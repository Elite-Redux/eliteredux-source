...
static void CreateApprenticeMenu(u8 menu)
{
    ...
    case APPRENTICE_ASK_3SPECIES:
        ...
        for (i = 0; i < MULTI_PARTY_SIZE; i++)
        {
            ...
            strings[i] = gSpecies[species].name;
        }
        break;
    case APPRENTICE_ASK_2SPECIES:
        ...
        if (PLAYER_APPRENTICE.questionsAnswered >= NUM_WHICH_MON_QUESTIONS)
            return;
        strings[1] = gSpecies[gApprenticeQuestionData->altSpeciesId].name;
        strings[0] = gSpecies[gApprenticeQuestionData->speciesId].name;
        break;
...
static void ApprenticeBufferString(void)
{
    ...
    case APPRENTICE_BUFF_SPECIES1:
        StringCopy(stringDst, gSpecies[gApprenticeQuestionData->speciesId].name);
        break;
    case APPRENTICE_BUFF_SPECIES2:
        StringCopy(stringDst, gSpecies[gApprenticeQuestionData->altSpeciesId].name);
        break;
    ...
    case APPRENTICE_BUFF_LEAD_MON_SPECIES:
        ...
        StringCopy(stringDst, gSpecies[gApprentices[PLAYER_APPRENTICE.id].species[speciesArrayId]].name);
        break;
    }
}
