...
int GetHpFormChangeSpecies(int battler, struct AiData* aiData)
{
    return SPECIES_NONE;
}
...
int ScoreSwitchInAbility(int ability, int battlerAtk, int battlerDef, int move, int moveType, struct AiData* aiData)
{
    ...
    case ABILITY_BUTTER_UP:
        return AI_SCORE_CURE_PARTY_STATUS(battlerAtk)
            - AI_SCORE_CURE_STATUS(BATTLE_PARTNER(battlerAtk))
    ...
    case ABILITY_MALICIOUS:
        return ScoreIntimidate(battlerDef, STAT_HIGHEST_ATTACKING, -1, ability, TRUE) + ScoreIntimidate(battlerDef, STAT_HIGHEST_DEFENDING, -1, ability, TRUE);
    ...
    case ABILITY_PRESSURE:
        return AI_SCORE_CLEAR_STAT_BUFFS(battlerDef) + AI_SCORE_CLEAR_STAT_BUFFS(BATTLE_PARTNER(battlerDef)) + AI_SCORE_CLEAR_STAT_BUFFS(BATTLE_PARTNER(battlerAtk));
    
    case ABILITY_PETRIFY:
        return AI_SCORE_CLEAR_STAT_BUFFS(battlerDef) + AI_SCORE_CLEAR_STAT_BUFFS(BATTLE_PARTNER(battlerDef)) + ScoreIntimidate(battlerDef, STAT_SPEED, -1, ability, TRUE);
    ...
    case ABILITY_PROTOSYNTHESIS:
        if (IsWeatherActive(WEATHER_SUN_ANY))
            return AI_SCORE_PARADOX_BOOST;
        
        if (GetBattlerHoldEffect(battlerAtk, TRUE) == HOLD_EFFECT_BOOSTER_ENERGY)
            return AI_SCORE_PARADOX_BOOST + AI_SCORE_LOSE_ITEM(battlerAtk);
        break;
    ...
    case ABILITY_LAWNMOWER:
        ...
        if (gFieldStatuses & (STATUS_FIELD_PSYCHIC_TERRAIN | STATUS_FIELD_MISTY_TERRAIN)) score = AI_SCORE_STAT(battlerAtk, STAT_SPDEF, 1);
        else score = AI_SCORE_STAT(battlerAtk, STAT_DEF, 1);
    ...
    case ABILITY_POWER_OF_ALCHEMY:
        if (!gBattleMons[battlerDef].item
            || gBattleMons[battlerDef].item == ITEM_BIG_NUGGET
        ...
        return AI_SCORE_LOSE_ITEM(battlerDef) + AI_SCORE_GIVE_ITEM(battlerDef, gBattleMons[battlerDef].item == ITEM_BLACK_SLUDGE ? ITEM_BIG_NUGGET : ITEM_BLACK_SLUDGE);
