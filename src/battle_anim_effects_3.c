...
const u16 gFacadeBlendColors[] = {
    RGB(28, 25, 1), RGB(28, 21, 5), RGB(27, 18, 8), RGB(27, 14, 11), RGB(26, 10, 15), RGB(26, 7, 18), RGB(25, 3, 21), RGB(25, 0, 25),
    RGB(25, 0, 23), RGB(25, 0, 20), RGB(25, 0, 16), RGB(25, 0, 13),  RGB(26, 0, 10),  RGB(26, 0, 6),  RGB(26, 0, 3),  RGB(27, 0, 0),
    RGB(27, 1, 0),  RGB(27, 5, 0),  RGB(27, 9, 0),  RGB(27, 12, 0),  RGB(28, 16, 0),  RGB(28, 19, 0), RGB(28, 23, 0), RGB(29, 27, 0),
};
...
void AnimTask_SmokescreenImpact(u8 taskId) {
    SmokescreenImpact(GetBattlerSpriteCoord(gBattleAnimTarget, 2) + 8, GetBattlerSpriteCoord(gBattleAnimTarget, 3) + 8, 0);
...
static void SetPsychicBackground_Step(u8 taskId) {
    ...
    if (++gTasks[taskId].data[5] == 4) {
        ...
        for (i = 10; i > 0; i--) gPlttBufferFaded[paletteIndex * 16 + i + 1] = gPlttBufferFaded[paletteIndex * 16 + i];
...
static void FadeScreenToWhite_Step(u8 taskId) {
    ...
    if (++gTasks[taskId].data[5] == 4) {
        ...
        for (i = 10; i > 0; i--) gPlttBufferUnfaded[paletteIndex * 16 + i + 1] = gPlttBufferUnfaded[paletteIndex * 16 + i];
...
static void AnimSpikes(struct Sprite* sprite) {
    ...
    if (GetBattlerSide(gBattleAnimAttacker) != B_SIDE_PLAYER) gBattleAnimArgs[2] = -gBattleAnimArgs[2];
...
static void AnimSpikes_Step2(struct Sprite* sprite) {
    if (sprite->data[1] & 1) sprite->invisible ^= 1;
...
static void AnimLetterZ(struct Sprite* sprite) {
    ...
    if ((u16)(sprite->x + sprite->x2) > DISPLAY_WIDTH) DestroyAnimSprite(sprite);
}
...
void AnimTask_RemoveSpotlight(u8 taskId) {
    ...
    gBattle_WIN1H = 0;
    gBattle_WIN1V = 0;
    if (!IsContest()) ClearGpuRegBits(REG_OFFSET_DISPCNT, DISPCNT_WIN1_ON);
...
static void AnimWishStar_Step(struct Sprite* sprite) {
    ...
    if (newX > 304) DestroyAnimSprite(sprite);
}
    ...
    switch (gTasks[taskId].data[0]) {
        ...
        case 1:
            if (gTasks[taskId].data[2]++ > 1) {
                ...
                if (stretch == 15) gTasks[taskId].data[0]++;
            }
            break;
        case 2:
        ...
        case 3:
            if (gTasks[taskId].data[2]++ > 1) {
                ...
                if (stretch == 0) gTasks[taskId].data[0]++;
            }
            break;
        case 4:
            SetGpuReg(REG_OFFSET_MOSAIC, 0);
    ...
    switch (gTasks[taskId].data[0]) {
        ...
        case 1:
            if (gTasks[taskId].data[4]++ > 0) {
                ...
                if (++gTasks[taskId].data[1] > 12) gTasks[taskId].data[1] = 12;

                SetGpuReg(REG_OFFSET_BLDALPHA, BLDALPHA_BLEND(gTasks[taskId].data[1], 16 - gTasks[taskId].data[1]));

                if (gTasks[taskId].data[1] == 12) gTasks[taskId].data[0]++;
            }
            break;
        case 2:
            if (--gTasks[taskId].data[1] < 0) gTasks[taskId].data[1] = 0;
...
void AnimTask_DoomDesireLightBeam(u8 taskId) {
    ...
    switch (gTasks[taskId].data[0]) {
        case 0:
            ...
            if (IsContest()) {
                ...
                if (IsDoubleBattle() == TRUE) {
                    if (position == B_POSITION_OPPONENT_LEFT) gBattle_BG1_X = -155;
                    if (position == B_POSITION_OPPONENT_RIGHT) gBattle_BG1_X = -115;
                    if (position == B_POSITION_PLAYER_LEFT) gBattle_BG1_X = 14;
                    if (position == B_POSITION_PLAYER_RIGHT) gBattle_BG1_X = -20;
                } else {
                    if (position == B_POSITION_OPPONENT_LEFT) gBattle_BG1_X = -135;
                    if (position == B_POSITION_PLAYER_LEFT) gBattle_BG1_X = -10;
                }
        ...
        case 1:
            ...
            break;
        case 2:
            if (--gTasks[taskId].data[1] <= 4) gTasks[taskId].data[1] = 5;

            SetGpuReg(REG_OFFSET_BLDALPHA, BLDALPHA_BLEND(3, gTasks[taskId].data[1]));
            if (gTasks[taskId].data[1] == 5) gTasks[taskId].data[0]++;
            break;
        case 3:
            if (++gTasks[taskId].data[3] > gDoomDesireLightBeamDelayTable[gTasks[taskId].data[2]]) gTasks[taskId].data[0]++;
            break;
        case 4:
            if (++gTasks[taskId].data[1] > 13) gTasks[taskId].data[1] = 13;

            SetGpuReg(REG_OFFSET_BLDALPHA, BLDALPHA_BLEND(3, gTasks[taskId].data[1]));
            if (gTasks[taskId].data[1] == 13) gTasks[taskId].data[0] = 1;
            break;
        case 5:
            GetBattleAnimBg1Data(&animBg);
            ClearBattleAnimBg(animBg.bgId);
...
static void AnimSweetScentPetal_Step(struct Sprite* sprite) {
    ...
    if (GetBattlerSide(gBattleAnimAttacker) == B_SIDE_PLAYER) {
        ...
        if (sprite->x > DISPLAY_WIDTH) DestroyAnimSprite(sprite);
    ...
    if (gTasks[taskId].data[0] == 0) {
        ...
        switch (gBattleAnimArgs[1]) {
            ...
            case 1:
                ...
                SetBattlerSpriteYOffsetFromYScale(spriteId);
                if (IsContest() || GetBattlerSide(gTasks[taskId].data[11]) == B_SIDE_PLAYER) gSprites[spriteId].y2 += 16;
                break;
            case 2:
...
void AnimTask_AcidArmor(u8 taskId) {
    ...
    if (GetBattlerSide(battler) == B_SIDE_OPPONENT) task->data[8] *= -1;
...
void AnimSmokeBallEscapeCloud(struct Sprite* sprite) {
    ...
    StartSpriteAffineAnim(sprite, gBattleAnimArgs[0]);
    if (GetBattlerSide(gBattleAnimTarget) != B_SIDE_PLAYER) gBattleAnimArgs[1] = -gBattleAnimArgs[1];
...
void AnimTask_SlideMonForFocusBand(u8 taskId) {
    ...
    if (gBattleAnimArgs[3]) gTasks[taskId].data[6] = gTasks[taskId].data[6] | -0x8000;
...
static void AnimRoarNoiseLine(struct Sprite* sprite) {
    if (GetBattlerSide(gBattleAnimAttacker) == B_SIDE_OPPONENT) gBattleAnimArgs[0] = -gBattleAnimArgs[0];
...
void AnimTask_GlareEyeDots(u8 taskId) {
    ...
    if (GetBattlerSide(gBattleAnimAttacker) == B_SIDE_PLAYER)
        task->data[11] =
            GetBattlerSpriteCoord(gBattleAnimAttacker, BATTLER_COORD_X_2) + GetBattlerSpriteCoordAttr(gBattleAnimAttacker, BATTLER_COORD_ATTR_HEIGHT) / 4;
    else
        task->data[11] =
            GetBattlerSpriteCoord(gBattleAnimAttacker, BATTLER_COORD_X_2) - GetBattlerSpriteCoordAttr(gBattleAnimAttacker, BATTLER_COORD_ATTR_HEIGHT) / 4;

    task->data[12] =
        GetBattlerSpriteCoord(gBattleAnimAttacker, BATTLER_COORD_Y_PIC_OFFSET) - GetBattlerSpriteCoordAttr(gBattleAnimAttacker, BATTLER_COORD_ATTR_HEIGHT) / 4;
...
void AnimTask_BarrageBall(u8 taskId) {
    ...
    task->data[13] = GetBattlerSpriteCoord(gBattleAnimTarget, BATTLER_COORD_X_2);
    task->data[14] =
        GetBattlerSpriteCoord(gBattleAnimTarget, BATTLER_COORD_Y_PIC_OFFSET) + GetBattlerSpriteCoordAttr(gBattleAnimTarget, BATTLER_COORD_ATTR_HEIGHT) / 4;
    ...
    if (task->data[15] != MAX_SPRITES) {
        ...
        if (GetBattlerSide(gBattleAnimAttacker) == B_SIDE_OPPONENT) StartSpriteAffineAnim(&gSprites[task->data[15]], 1);
...
static void AnimForesightMagnifyingGlass(struct Sprite* sprite) {
    ...
    if (GetBattlerSide(sprite->data[7]) == B_SIDE_OPPONENT) sprite->oam.matrixNum = ST_OAM_HFLIP;
...
void AnimTask_MonToSubstitute(u8 taskId) {
    ...
    if (gTasks[taskId].data[0] == 0) {
        ...
        for (i = 0; i < NUM_TASK_DATA; i++) gTasks[taskId].data[i] = 0;
...
static void AnimTask_MonToSubstituteDoll(u8 taskId) {
    ...
    switch (gTasks[taskId].data[0]) {
        ...
        case 1:
            ...
            if (gSprites[spriteId].y + gSprites[spriteId].y2 >= -32) gSprites[spriteId].x2 = 0;

            if (gSprites[spriteId].y2 > 0) gSprites[spriteId].y2 = 0;
        ...
        case 2:
            gTasks[taskId].data[10] -= 112;
            if (gTasks[taskId].data[10] < 0) gTasks[taskId].data[10] = 0;

            gSprites[spriteId].y2 -= gTasks[taskId].data[10] >> 8;
            if (gTasks[taskId].data[10] == 0) gTasks[taskId].data[0]++;
            break;
        case 3:
            gTasks[taskId].data[10] += 112;
            gSprites[spriteId].y2 += gTasks[taskId].data[10] >> 8;
...
static void MoveOdorSleuthClone(struct Sprite* sprite) {
    if (++sprite->data[1] > 1) {
        sprite->data[1] = 0;
        if (!gBattleSpritesDataPtr->battlerData[gBattleAnimTarget].invisible) sprite->invisible ^= 1;
    }
...
void AnimTask_GetReturnPowerLevel(u8 taskId) {
    ...
    if (gAnimFriendship < 60) gBattleAnimArgs[ARG_RET_ID] = 0;
    if (gAnimFriendship > 60 && gAnimFriendship < 92) gBattleAnimArgs[ARG_RET_ID] = 1;
    if (gAnimFriendship > 91 && gAnimFriendship < 201) gBattleAnimArgs[ARG_RET_ID] = 2;
    if (gAnimFriendship > 200) gBattleAnimArgs[ARG_RET_ID] = 3;
    ...
    switch (gTasks[taskId].data[0]) {
        ...
        case 1:
            ...
            spriteId2 = CreateAdditionalMonSpriteForMoveAnim(
                species, isBackPic, 0, x, GetBattlerSpriteCoord(gBattleAnimTarget, BATTLER_COORD_Y), subpriority, personality, otId, gBattleAnimAttacker);
            if (gBattleSpritesDataPtr->battlerData[gBattleAnimAttacker].transformSpecies != SPECIES_NONE)
        ...
        case 4:
            ...
            if (GetBattlerSide(gBattleAnimAttacker) == B_SIDE_PLAYER) {
                gSprites[spriteId].x2 += (gTasks[taskId].data[1] >> 8);
                if (gSprites[spriteId].x2 + gSprites[spriteId].x >= GetBattlerSpriteCoord(gBattleAnimAttacker, BATTLER_COORD_X)) gSprites[spriteId].x2 = 0;
            } else {
                gSprites[spriteId].x2 -= (gTasks[taskId].data[1] >> 8);
                if (gSprites[spriteId].x2 + gSprites[spriteId].x <= GetBattlerSpriteCoord(gBattleAnimAttacker, BATTLER_COORD_X)) gSprites[spriteId].x2 = 0;
            }
            ...
            gTasks[taskId].data[1] &= 0xFF;
            if (gSprites[spriteId].x2 == 0) DestroyAnimVisualTask(taskId);
            break;
    }
}
...
void AnimTask_SnatchPartnerMove(u8 taskId) {
    ...
    switch (gTasks[taskId].data[15]) {
        case 0:
            ...
            if (attackerX > targetX) gTasks[taskId].data[0] *= -1;
        ...
        case 1:
            ...
            if (gTasks[taskId].data[0] > 0) {
                if (gSprites[spriteId].x + gSprites[spriteId].x2 >= gTasks[taskId].data[2]) gTasks[taskId].data[15]++;
            } else {
                if (gSprites[spriteId].x + gSprites[spriteId].x2 <= gTasks[taskId].data[2]) gTasks[taskId].data[15]++;
            }
            break;
        ...
        case 3:
            ...
            gSprites[spriteId].x2 += gTasks[taskId].data[0];
            if (gTasks[taskId].data[0] < 0) {
                if (gSprites[spriteId].x + gSprites[spriteId].x2 <= gTasks[taskId].data[1]) gTasks[taskId].data[15]++;
            } else {
                if (gSprites[spriteId].x + gSprites[spriteId].x2 >= gTasks[taskId].data[1]) gTasks[taskId].data[15]++;
            }
            break;
...
void AnimTask_GetWeather(u8 taskId) {
    ...
    if (!hasChloroplast && !hasAuroraBorealis && !WEATHER_HAS_EFFECT) return;
    if (hasChloroplast)
        gBattleAnimArgs[ARG_RET_ID] = ANIM_WEATHER_SUN;
    else if (hasAuroraBorealis)
