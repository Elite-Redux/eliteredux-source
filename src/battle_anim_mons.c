...
u8 GetBattlerSpriteCoord(u8 battlerId, u8 coordType) {
    ...
    if (IsContest()) {
        if (coordType == BATTLER_COORD_Y_PIC_OFFSET && battlerId == 3) coordType = BATTLER_COORD_Y;
    }
...
u8 GetBattlerYDelta(u8 battlerId, SpeciesEnum species) {
    u32 personality;
    struct BattleSpriteInfo *spriteInfo;
    u8 ret;
    u16 coordSpecies;
...
u8 GetBattlerElevation(u8 battlerId, SpeciesEnum species) {
    u8 ret = 0;
...
u8 GetBattlerSpriteFinal_Y(u8 battlerId, SpeciesEnum species, bool8 a3) {
    u16 offset;
    u8 y;
    ...
    if (a3) {
        if (GetBattlerSide(battlerId) == B_SIDE_PLAYER) y += 8;
        if (y > 104) y = 104;
    }
    return y;
}
...
u8 GetBattlerSpriteDefault_Y(u8 battlerId) { return GetBattlerSpriteCoord(battlerId, BATTLER_COORD_Y_PIC_OFFSET_DEFAULT); }
...
u8 GetBattlerYCoordWithElevation(u8 battlerId) {
    ...
    if (!IsContest()) {
        ...
        if (GetBattlerSide(battlerId) != B_SIDE_PLAYER) y -= GetBattlerElevation(battlerId, species);
    }
    return y;
}
...
void TranslateSpriteLinearAndFlicker(struct Sprite *sprite) {
    if (sprite->data[0] > 0) {
        ...
        if (sprite->data[0] % sprite->data[5] == 0) {
            if (sprite->data[5]) sprite->invisible ^= 1;
        }
    } else {
        SetCallbackToStoredInData6(sprite);
    }
...
u8 GetOppositeSide(u8 battlerId) { return BATTLE_OPPOSITE(GetBattlerSide(battlerId)); }

u8 GetBattlerSide(u8 battlerId) { return GET_BATTLER_SIDE2(battlerId); }

u8 GetBattlerPosition(u8 battlerId) { return GET_BATTLER_POSITION(battlerId); }

u8 GetBattlerAtPosition(u8 position) {
    u8 i;

    for (i = 0; i < gBattlersCount; i++) {
        if (gBattlerPositions[i] == position) break;
    }
    return i;
}

struct Pokemon *GetSideParty(u32 side) { return (side == B_SIDE_PLAYER) ? gPlayerParty : gEnemyParty; }

struct Pokemon *GetBattlerParty(u32 battler) { return GetSideParty(GetBattlerSide(battler)); }
...
bool8 IsBattlerSpritePresent(u8 battlerId) {
    if (IsContest()) {
        ...
        if (!gBattleStruct->spriteIgnore0Hp) {
            if (GetBattlerSide(battlerId) == B_SIDE_OPPONENT) {
                if (GetMonData(&gEnemyParty[gBattlerPartyIndexes[battlerId]], MON_DATA_HP) == 0) return FALSE;
            } else {
                if (GetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerId]], MON_DATA_HP) == 0) return FALSE;
            }
        }
        return TRUE;
    }
...
void AnimLoadCompressedBgTilemapHandleContest(struct BattleAnimBgData *unk, const void *src, u32 arg2) {
    ...
    if (IsContest() == TRUE) sub_80A4720(unk->paletteId, unk->bgTilemap, 0, arg2);
...
void SetSpriteRotScale(u8 spriteId, s16 xScale, s16 yScale, u16 rotation) {
    ...
    if (sub_80A7238()) src.xScale = -src.xScale;
...
void PrepareBattlerSpriteForRotScale(u8 spriteId, u8 objMode) {
    ...
    if (IsContest() || IsBattlerSpriteVisible(battlerId)) gSprites[spriteId].invisible = FALSE;
    gSprites[spriteId].oam.objMode = objMode;
    gSprites[spriteId].affineAnimPaused = TRUE;
    if (!IsContest() && !gSprites[spriteId].oam.affineMode) gSprites[spriteId].oam.matrixNum = gBattleSpritesDataPtr->healthBoxesData[battlerId].matrixNum;
...
u32 GetBattleBgPalettesMask(u8 battleBackground, u8 attacker, u8 target, u8 attackerPartner, u8 targetPartner, u8 a6, u8 a7) {
    ...
    if (a7) {
        if (!IsContest()) selectedPalettes |= 0x200;
    }
...
u8 AnimDummyReturnArg(u8 battler) { return battler; }
...
void AnimTask_AlphaFadeIn(u8 taskId) {
    ...
    if (gBattleAnimArgs[2] > gBattleAnimArgs[0]) v2 = 1;
    if (gBattleAnimArgs[2] < gBattleAnimArgs[0]) v2 = -1;
    if (gBattleAnimArgs[3] > gBattleAnimArgs[1]) v1 = 1;
    if (gBattleAnimArgs[3] < gBattleAnimArgs[1]) v1 = -1;
...
void *LoadPointerFromVars(s16 lo, s16 hi) { return (void *)((u16)lo | ((u16)hi << 16)); }
...
u8 CreateAdditionalMonSpriteForMoveAnim(SpeciesEnum species, bool8 isBackpic, u8 id, s16 x, s16 y, u8 subpriority, u32 personality, u32 trainerId, u32 battlerId) {
    u8 spriteId;
    ...
    if (gMonSpritesGfxPtr != NULL && gMonSpritesGfxPtr->buffer == NULL) gMonSpritesGfxPtr->buffer = AllocZeroed(0x2000);
    if (!isBackpic) {
        LoadHueShiftedMonPalette(GetMonSpritePal(species, personality, isShiny), (palette * 0x10) + 0x100, 0x20, personality, isAlpha);
        LoadSpecialPokePic(&gMonFrontPicTable[species], gMonSpritesGfxPtr->buffer, species, personality, TRUE);
    } else {
        LoadHueShiftedMonPalette(GetMonSpritePal(species, personality, isShiny), (palette * 0x10) + 0x100, 0x20, personality, isAlpha);
        LoadSpecialPokePic(&gMonBackPicTable[species], gMonSpritesGfxPtr->buffer, species, personality, FALSE);
    }
...
void DestroySpriteAndFreeResources_(struct Sprite *sprite) { DestroySpriteAndFreeResources(sprite); }
