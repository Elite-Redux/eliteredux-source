...
static u32 GetOpponentMonData(u8 monId, u8 *dst)
{
    ...
    case REQUEST_ALL_BATTLE:
        ...
        battleMon.speedDown = !GetMonData(&gEnemyParty[monId], MON_DATA_SPEED_IV);
        ...
        battleMon.hpType = GetMonData(&gEnemyParty[monId], MON_DATA_HP_TYPE);
        ...
        for(i = 0; i < HELL_MODE_EXTRA_ABILITIES; i++)
            battleMon.extraAbilities[i] = GetExtraAbilityToSetToBattler(i, TRUE);
    ...
    case REQUEST_ALL_BATTLE:
        {
            ...
            SetMonData(&gEnemyParty[monId], MON_DATA_HP_TYPE, &battlePokemon->hpType);
        }
        break;
    case REQUEST_SPECIES_BATTLE:
