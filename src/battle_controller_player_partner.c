...
static void Task_GiveExpToMon(u8 taskId)
{
    ...
    if (IsDoubleBattle() == TRUE || monId != gBattlerPartyIndexes[battlerId]) // give exp without the expbar
    {
        ...
        u32 nextLvlExp = gExperienceTables[gSpecies[species].growthRate][level + 1];
...
static void Task_PrepareToGiveExpWithExpBar(u8 taskId)
{
    ...
    u32 currLvlExp = gExperienceTables[gSpecies[species].growthRate][level];
    u32 expToNextLvl;

    exp -= currLvlExp;
    expToNextLvl = gExperienceTables[gSpecies[species].growthRate][level + 1] - currLvlExp;
...
static void Task_GiveExpWithExpBar(u8 taskId)
{
    ...
    else
    {
        ...
        if (r4 == -1)
        {
            ...
            expOnNextLvl = gExperienceTables[gSpecies[species].growthRate][level + 1];
...
static u32 CopyPlayerPartnerMonData(u8 monId, u8 *dst)
{
    ...
    case REQUEST_ALL_BATTLE:
        ...
        battleMon.abilities[0] = GetAbilityBySpecies(battleMon.species, battleMon.abilityNum);
        battleMon.abilities[1] = GetInnateInSlot(battleMon.level, battleMon.species, 0, battleMon.personality, FALSE);
        battleMon.abilities[2] = GetInnateInSlot(battleMon.level, battleMon.species, 1, battleMon.personality, FALSE);
        battleMon.abilities[3] = GetInnateInSlot(battleMon.level, battleMon.species, 2, battleMon.personality, FALSE);
