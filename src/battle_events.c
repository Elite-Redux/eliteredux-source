...
u8 BattleEventExec(struct BattleEvent* battleEvent) {
    if (battleEvent->id == BATTLE_EVENT_NONE) return EXEC_BATTLE_EVENTS_ALL_CLEAR;

    gLastBattleEvent = battleEvent->id;

    switch (gBattleResults.battleTurnCounter) {
        case 0:
            if (BattleEventBeforeFirstTurnExec(battleEvent) == EXEC_BATTLE_EVENTS_NEEDS_SCRIPT_CALL) return EXEC_BATTLE_EVENTS_NEEDS_SCRIPT_CALL;
            break;
        default:
            if (BattleEventStartTurnExec(battleEvent) == EXEC_BATTLE_EVENTS_NEEDS_SCRIPT_CALL) return EXEC_BATTLE_EVENTS_NEEDS_SCRIPT_CALL;
            break;
    }
    return EXEC_BATTLE_EVENTS_ALL_CLEAR;
}
...
#define RUN_BATTLESCRIPT(bs) \
    BattleScriptExecute(bs); \
    return EXEC_BATTLE_EVENTS_NEEDS_SCRIPT_CALL;
#define RUN_BATTLESCRIPT_UNREGISTER(bs) \
    UnregisterCurrentBattleEvent();     \
    BattleScriptExecute(bs);            \
    return EXEC_BATTLE_EVENTS_NEEDS_SCRIPT_CALL;
#define SET_STR1(str) StringExpandPlaceholders(gStringVar1, str);
#define SET_STR2(str1, str2)                     \
    StringExpandPlaceholders(gStringVar1, str1); \
...
bool8 IsBattleEventForbiddenOnSwitchIn(u8 battleEventID) {
    ...
    for (i = 0; i < ARRAY_COUNT(sOnSwitchInForbiddenBattleEvent); i++) {
        if (battleEventID == sOnSwitchInForbiddenBattleEvent[i]) return TRUE;
    }
    return FALSE;
}
...
bool8 DepleteTeamPowerPointOfMove(u16 moveId) {
    ...
    for (i = 0; i < gPlayerPartyCount; i++) {
        ...
        for (j = 0; j < MAX_MON_MOVES; j++) {
            ...
            if (i == 0) gBattleMons[B_POSITION_PLAYER_LEFT].pp[j] = 0;
            if (i == 1) gBattleMons[B_POSITION_PLAYER_RIGHT].pp[j] = 0;
...
u8 BattleEventBeforeFirstTurnExec(struct BattleEvent* battleEvent) {
    switch (battleEvent->id) {
        ...
        case BATTLE_EVENT_POSTURE_OFFENSE:
            ...
            if (battleEvent->data0 == 0) battleEvent->data0 = 4;
        ...
        case BATTLE_EVENT_LAST_PARALYZED:
            if (AffectNStatusOnTeamFromLastToFirst(STATUS1_PARALYSIS, battleEvent->data0)) PlaySE(SE_M_THUNDERBOLT2);
            SET_STR1(gText_Paralysis)
            RUN_BATTLESCRIPT_UNREGISTER(BattleScript_ExtraSkillStatusOnTeam)
        case BATTLE_EVENT_LAST_BURNED:
            if (AffectNStatusOnTeamFromLastToFirst(STATUS1_BURN, battleEvent->data0)) PlaySE(SE_M_FLAME_WHEEL);
            SET_STR1(gText_Burn)
            RUN_BATTLESCRIPT_UNREGISTER(BattleScript_ExtraSkillStatusOnTeam)
        case BATTLE_EVENT_LAST_SLEEP:
            if (AffectNStatusOnTeamFromLastToFirst(STATUS1_SLEEP, battleEvent->data0)) PlaySE(SE_M_SNORE);
            SET_STR1(gText_Sleep)
            RUN_BATTLESCRIPT_UNREGISTER(BattleScript_ExtraSkillStatusOnTeam)
        case BATTLE_EVENT_LAST_FROSTBITE:
            if (AffectNStatusOnTeamFromLastToFirst(STATUS1_FROSTBITE, battleEvent->data0)) PlaySE(SE_M_ICY_WIND);  // TODO PROBABLY WRONG SE
            SET_STR1(sText_Frostbite)
            RUN_BATTLESCRIPT_UNREGISTER(BattleScript_ExtraSkillStatusOnTeam)
        case BATTLE_EVENT_LAST_BLEED:
            if (AffectNStatusOnTeamFromLastToFirst(STATUS1_BLEED, battleEvent->data0)) PlaySE(SE_M_BUBBLE);
            SET_STR1(gText_Bleed)
            RUN_BATTLESCRIPT_UNREGISTER(BattleScript_ExtraSkillStatusOnTeam)
        case BATTLE_EVENT_LAST_POISONED:
            if (AffectNStatusOnTeamFromLastToFirst(STATUS1_POISON, battleEvent->data0)) PlaySE12WithPanning(SE_M_TOXIC, 13);
        ...
        case BATTLE_EVENT_PERMA_WIDE_GUARD:
            if (gSideStatuses[B_SIDE_OPPONENT] & SIDE_STATUS_WIDE_GUARD) return EXEC_BATTLE_EVENTS_ALL_CLEAR;
        ...
        case BATTLE_EVENT_PERMA_STICKY_WEB:
            if (gSideStatuses[B_SIDE_PLAYER] & SIDE_STATUS_STICKY_WEB) return EXEC_BATTLE_EVENTS_ALL_CLEAR;
            gSideStatuses[B_SIDE_PLAYER] |= SIDE_STATUS_STICKY_WEB;
            RUN_BATTLESCRIPT(BattleScript_ExtraSkillPermaStickyWebOld);

        case BATTLE_EVENT_NO_PROTECT:
            if (!DepleteTeamPowerPointOfMove(MOVE_PROTECT)) return EXEC_BATTLE_EVENTS_ALL_CLEAR;
...
u8 BattleEventStartTurnExec(struct BattleEvent* battleEvent) {
    ...
    if (gVolatileStructs[B_POSITION_OPPONENT_LEFT].isFirstTurn == 1 && IsBattleEventForbiddenOnSwitchIn(battleEvent->id)) return EXEC_BATTLE_EVENTS_ALL_CLEAR;
    switch (battleEvent->id) {
        case BATTLE_EVENT_NONE:
            break;
        ...
        case BATTLE_EVENT_LAST_STAND:
            if (gFaintedMonCount[1] != battleEvent->data0) return EXEC_BATTLE_EVENTS_ALL_CLEAR;
        ...
        case BATTLE_EVENT_PERMA_HEAL_BLOCK:
            if (gStatuses3[B_SIDE_PLAYER] & STATUS3_HEAL_BLOCK) return EXEC_BATTLE_EVENTS_ALL_CLEAR;
        ...
        case BATTLE_EVENT_PERMA_NIGHTMARE:
            if (gBattleMons[B_SIDE_PLAYER].status2 & STATUS2_NIGHTMARE) return EXEC_BATTLE_EVENTS_ALL_CLEAR;
            if (!(gBattleMons[B_SIDE_PLAYER].status1 & STATUS1_SLEEP)) return EXEC_BATTLE_EVENTS_ALL_CLEAR;
...
u16 GetTrainerBattleEventData(u16 trainerId, u8 number, u8 dataID) {
    if (sTrainerSkillList[trainerId][number][BATTLE_EVENT_ID] == BATTLE_EVENT_NONE) return 0;
