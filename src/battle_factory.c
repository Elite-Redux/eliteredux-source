...
static const u8 sRequiredMoveCounts[FACTORY_NUM_STYLES - 1] = {[FACTORY_STYLE_PREPARATION - 1] = 3,
...
static const u16 sMoves_TotalPreparation[] = {MOVE_SWORDS_DANCE, MOVE_GROWTH,    MOVE_MEDITATE,     MOVE_AGILITY,      MOVE_DOUBLE_TEAM,  MOVE_HARDEN,
...
static const u16 sMoves_WeakeningTheFoe[] = {MOVE_SAND_ATTACK,   MOVE_TAIL_WHIP,   MOVE_LEER,        MOVE_GROWL,     MOVE_STRING_SHOT,
                                             MOVE_SCREECH,       MOVE_SMOKESCREEN, MOVE_KINESIS,     MOVE_FLASH,     MOVE_COTTON_SPORE,
                                             MOVE_SPITE,         MOVE_SCARY_FACE,  MOVE_CHARM,       MOVE_KNOCK_OFF, MOVE_SWEET_SCENT,
                                             MOVE_FEATHER_DANCE, MOVE_FAKE_TEARS,  MOVE_METAL_SOUND, MOVE_TICKLE,    MOVE_NONE};

static const u16 sMoves_HighRiskHighReturn[] = {MOVE_GUILLOTINE,   MOVE_HORN_DRILL,    MOVE_DOUBLE_EDGE, MOVE_HYPER_BEAM,   MOVE_COUNTER,  MOVE_FISSURE,
...
static const u16 sMoves_Endurance[] = {MOVE_MIST,      MOVE_RECOVER,      MOVE_LIGHT_SCREEN, MOVE_HAZE,      MOVE_REFLECT,    MOVE_SOFT_BOILED,
                                       MOVE_REST,      MOVE_PROTECT,      MOVE_DETECT,       MOVE_ENDURE,    MOVE_MILK_DRINK, MOVE_HEAL_BELL,
                                       MOVE_SAFEGUARD, MOVE_BATON_PASS,   MOVE_MORNING_SUN,  MOVE_SYNTHESIS, MOVE_MOONLIGHT,  MOVE_SWALLOW,
                                       MOVE_WISH,      MOVE_INGRAIN,      MOVE_MAGIC_COAT,   MOVE_RECYCLE,   MOVE_REFRESH,    MOVE_MUD_SPORT,
                                       MOVE_SLACK_OFF, MOVE_AROMATHERAPY, MOVE_WATER_SPORT,  MOVE_NONE};

static const u16 sMoves_SlowAndSteady[] = {MOVE_SING,          MOVE_SUPERSONIC,   MOVE_DISABLE, MOVE_LEECH_SEED, MOVE_POISON_POWDER, MOVE_STUN_SPORE,
...
static const u16 sMoves_DependsOnTheBattlesFlow[] = {MOVE_SANDSTORM, MOVE_RAIN_DANCE, MOVE_SUNNY_DAY, MOVE_HAIL, MOVE_WEATHER_BALL, MOVE_NONE};
...
// code
void CallBattleFactoryFunction(void) { sBattleFactoryFunctions[gSpecialVar_0x8004](); }
...
static void InitFactoryChallenge(void) {
    ...
    for (i = 0; i < 6; i++) gSaveBlock2Ptr->frontier.rentalMons[i].monId = 0xFFFF;
    for (i = 0; i < getPartySize(); i++) gFrontierTempParty[i] = 0xFFFF;
...
static void GenerateOpponentMons(void) {
    ...
    if (gSaveBlock2Ptr->frontier.curChallengeBattleNum < 6) gSaveBlock2Ptr->frontier.trainerIds[gSaveBlock2Ptr->frontier.curChallengeBattleNum] = trainerId;

    i = 0;
    while (i != getPartySize()) {
        u16 monId = GetFactoryMonId(lvlMode, challengeNum, FALSE);
        if (gFacilityTrainerMons[monId].species == SPECIES_UNOWN) continue;

        for (j = 0; j < 6; j++) {
            if (gFacilityTrainerMons[monId].species == gFacilityTrainerMons[gSaveBlock2Ptr->frontier.rentalMons[j].monId].species) break;
        }
        if (j != 6) continue;

        if (lvlMode == FRONTIER_LVL_50 && monId > FRONTIER_MONS_HIGH_TIER) continue;

        for (k = firstMonId; k < firstMonId + i; k++) {
            if (species[k] == gFacilityTrainerMons[monId].species) break;
        }
        if (k != firstMonId + i) continue;

        for (k = firstMonId; k < firstMonId + i; k++) {
            if (heldItems[k] != 0 && heldItems[k] == gFacilityTrainerMons[monId].item) break;
        }
        if (k != firstMonId + i) continue;
...
static u8 getPartySize() { return FlagGet(FLAG_FRONTIER_6V6) ? FRONTIER_PARTY_SIZE_6 : FRONTIER_PARTY_SIZE; }

static void SetOpponentGfxVar(void) { SetBattleFacilityTrainerGfxId(gTrainerBattleOpponent_A, 0); }
...
static void SetRentalsToOpponentParty(void) {
    ...
    for (i = 0; i < getPartySize(); i++) {
        gSaveBlock2Ptr->frontier.rentalMons[i + getPartySize()].monId = gFrontierTempParty[i];
        gSaveBlock2Ptr->frontier.rentalMons[i + getPartySize()].ivs = GetBoxMonData(&gEnemyParty[i].box, MON_DATA_ATK_IV, NULL);
        gSaveBlock2Ptr->frontier.rentalMons[i + getPartySize()].personality = GetMonData(&gEnemyParty[i], MON_DATA_PERSONALITY, NULL);
        gSaveBlock2Ptr->frontier.rentalMons[i + getPartySize()].abilityNum = GetBoxMonData(&gEnemyParty[i].box, MON_DATA_ABILITY_NUM, NULL);
...
static void SetPlayerAndOpponentParties(void) {
    ...
    if (gSpecialVar_0x8005 < 2) {
        ...
        for (i = 0; i < getPartySize(); i++) {
            ...
            bits = 1;
            for (j = 0; j < NUM_STATS; bits <<= 1, j++) {
                if (gFacilityTrainerMons[monId].evSpread & bits) SetMonData(&gPlayerParty[i], MON_DATA_HP_EV + j, &evs);
            }

            CalculateMonStats(&gPlayerParty[i]);
            friendship = 0;
            for (k = 0; k < MAX_MON_MOVES; k++) SetMonMoveAvoidReturn(&gPlayerParty[i], gFacilityTrainerMons[monId].moves[k], k);
            ...
            SetMonData(&gPlayerParty[i], MON_DATA_HP_TYPE, &gFacilityTrainerMons[monId].hpType);
        }
    }
    ...
    switch (gSpecialVar_0x8005) {
        ...
        case 2:
            for (i = 0; i < getPartySize(); i++) {
                monId = gSaveBlock2Ptr->frontier.rentalMons[i + getPartySize()].monId;
                ivs = gSaveBlock2Ptr->frontier.rentalMons[i + getPartySize()].ivs;
                CreateMon(&gEnemyParty[i],
                          gFacilityTrainerMons[monId].species,
                          monLevel,
                          ivs,
                          TRUE,
                          gSaveBlock2Ptr->frontier.rentalMons[i + getPartySize()].personality,
                          OT_ID_PLAYER_ID,
                          0);
                ...
                bits = 1;
                for (j = 0; j < NUM_STATS; bits <<= 1, j++) {
                    if (gFacilityTrainerMons[monId].evSpread & bits) SetMonData(&gEnemyParty[i], MON_DATA_HP_EV + j, &evs);
                }

                CalculateMonStats(&gEnemyParty[i]);
                for (k = 0; k < MAX_MON_MOVES; k++) SetMonMoveAvoidReturn(&gEnemyParty[i], gFacilityTrainerMons[monId].moves[k], k);
                SetMonData(&gEnemyParty[i], MON_DATA_HELD_ITEM, &gFacilityTrainerMons[monId].item);
                SetMonData(&gEnemyParty[i], MON_DATA_ABILITY_NUM, &gSaveBlock2Ptr->frontier.rentalMons[i + getPartySize()].abilityNum);
            SetMonData(&gEnemyParty[i], MON_DATA_HP_TYPE, &gFacilityTrainerMons[monId].hpType);
            }
            break;
    }
}
...
static void GetOpponentMostCommonMonType(void) {
    ...
    for (i = TYPE_NORMAL; i < NUMBER_OF_MON_TYPES; i++) typeCounts[i] = 0;
    for (i = 0; i < getPartySize(); i++) {
        u32 species = gFacilityTrainerMons[gFrontierTempParty[i]].species;
        typeCounts[gBaseStats[species].type1]++;
        if (gBaseStats[species].type1 != gBaseStats[species].type2) typeCounts[gBaseStats[species].type2]++;
    }
...
static void GetOpponentBattleStyle(void) {
    ...
    count = 0;
    gFacilityTrainerMons = gBattleFrontierMons;
    for (i = 0; i < FACTORY_NUM_STYLES; i++) stylePoints[i] = 0;
    ...
    if (count > 2) gSpecialVar_Result = FACTORY_NUM_STYLES;
}
...
static void RestorePlayerPartyHeldItems(void) {
    ...
    for (i = 0; i < getPartySize(); i++) {
        SetMonData(&gPlayerParty[i], MON_DATA_HELD_ITEM, &gFacilityTrainerMons[gSaveBlock2Ptr->frontier.rentalMons[i].monId].item);
    }
}
...
void FillFactoryBrainParty(void) {
    ...
    while (i != getPartySize()) {
        ...
        if (gFacilityTrainerMons[monId].species == SPECIES_UNOWN) continue;
        if (monLevel == 50 && monId > FRONTIER_MONS_HIGH_TIER) continue;

        for (j = 0; j < 6; j++) {
            if (monId == gSaveBlock2Ptr->frontier.rentalMons[j].monId) break;
        }
        if (j != 6) continue;
...
static u16 GetFactoryMonId(u8 lvlMode, u8 challengeNum, bool8 useBetterRange) {
    ...
    if (challengeNum < 7) {
        if (useBetterRange) {
            numMons = ARRAY_COUNT(gBattleFrontierMons);
        ...
        if (challenge != 7) challenge = 7;  // why bother assigning it above at all
...
void SetMonMoveAvoidReturn(struct Pokemon* mon, u16 moveArg, u8 moveSlot) {
    u16 move = moveArg;
    if (moveArg == MOVE_RETURN) move = MOVE_FRUSTRATION;
