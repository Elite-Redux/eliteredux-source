...
{
    // Level 50
    {FRONTIER_MON_VENUSAUR_1,     NUM_FRONTIER_MONS - 1}, 
    {FRONTIER_MON_VENUSAUR_1,     NUM_FRONTIER_MONS - 1}, 
    {FRONTIER_MON_VENUSAUR_1,     NUM_FRONTIER_MONS - 1}, 
    {FRONTIER_MON_VENUSAUR_1,     NUM_FRONTIER_MONS - 1}, 
    {FRONTIER_MON_VENUSAUR_1,     NUM_FRONTIER_MONS - 1}, 
...
{
    int i, j, k;
    u16 species[(FlagGet(FLAG_FRONTIER_6V6) ? FRONTIER_PARTY_SIZE_6 : FRONTIER_PARTY_SIZE)];
    u16 heldItems[(FlagGet(FLAG_FRONTIER_6V6) ? FRONTIER_PARTY_SIZE_6 : FRONTIER_PARTY_SIZE)];
    int firstMonId = 0;
    u16 trainerId = 0;
    u32 lvlMode = gSaveBlock2Ptr->frontier.lvlMode;
    u32 battleMode = VarGet(VAR_FRONTIER_BATTLE_MODE);
    ...
    i = 0;
    while (i != (FlagGet(FLAG_FRONTIER_6V6) ? FRONTIER_PARTY_SIZE_6 : FRONTIER_PARTY_SIZE))
    {
...
static void SetRentalsToOpponentParty(void)
{
    ...
    for (i = 0; i < (FlagGet(FLAG_FRONTIER_6V6) ? FRONTIER_PARTY_SIZE_6 : FRONTIER_PARTY_SIZE); i++)
    {
        gSaveBlock2Ptr->frontier.rentalMons[i + (FlagGet(FLAG_FRONTIER_6V6) ? FRONTIER_PARTY_SIZE_6 : FRONTIER_PARTY_SIZE)].monId = gFrontierTempParty[i];
        gSaveBlock2Ptr->frontier.rentalMons[i + (FlagGet(FLAG_FRONTIER_6V6) ? FRONTIER_PARTY_SIZE_6 : FRONTIER_PARTY_SIZE)].ivs = GetBoxMonData(&gEnemyParty[i].box, MON_DATA_ATK_IV, NULL);
        gSaveBlock2Ptr->frontier.rentalMons[i + (FlagGet(FLAG_FRONTIER_6V6) ? FRONTIER_PARTY_SIZE_6 : FRONTIER_PARTY_SIZE)].personality = GetMonData(&gEnemyParty[i], MON_DATA_PERSONALITY, NULL);
        gSaveBlock2Ptr->frontier.rentalMons[i + (FlagGet(FLAG_FRONTIER_6V6) ? FRONTIER_PARTY_SIZE_6 : FRONTIER_PARTY_SIZE)].abilityNum = GetBoxMonData(&gEnemyParty[i].box, MON_DATA_ABILITY_NUM, NULL);
...
static void SetPlayerAndOpponentParties(void)
{
    ...
    case 2:
        for (i = 0; i < (FlagGet(FLAG_FRONTIER_6V6) ? FRONTIER_PARTY_SIZE_6 : FRONTIER_PARTY_SIZE); i++)
        {
            monId = gSaveBlock2Ptr->frontier.rentalMons[i + (FlagGet(FLAG_FRONTIER_6V6) ? FRONTIER_PARTY_SIZE_6 : FRONTIER_PARTY_SIZE)].monId;
            ivs = gSaveBlock2Ptr->frontier.rentalMons[i + (FlagGet(FLAG_FRONTIER_6V6) ? FRONTIER_PARTY_SIZE_6 : FRONTIER_PARTY_SIZE)].ivs;
            CreateMon(&gEnemyParty[i],
                      gFacilityTrainerMons[monId].species,
                      monLevel,
                      ivs,
                      TRUE, gSaveBlock2Ptr->frontier.rentalMons[i + (FlagGet(FLAG_FRONTIER_6V6) ? FRONTIER_PARTY_SIZE_6 : FRONTIER_PARTY_SIZE)].personality,
                      OT_ID_PLAYER_ID, 0);
            ...
            SetMonData(&gEnemyParty[i], MON_DATA_ABILITY_NUM, &gSaveBlock2Ptr->frontier.rentalMons[i + (FlagGet(FLAG_FRONTIER_6V6) ? FRONTIER_PARTY_SIZE_6 : FRONTIER_PARTY_SIZE)].abilityNum);
        }
        break;
    }
}
