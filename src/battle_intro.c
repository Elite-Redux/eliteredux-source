...
static EWRAM_DATA BgCnt sBgCnt = { 0 };
...
void SetAnimBgAttribute(u8 bgId, u8 attributeId, u8 value)
{
    if (bgId < 4)
    {
        sBgCnt.bits = GetGpuReg(gBattleAnimBgCntSet[bgId]);
        switch (attributeId)
        {
        ...
        SetGpuReg(gBattleAnimBgCntSet[bgId], sBgCnt.bits);
    }
}
...
int GetAnimBgAttribute(u8 bgId, u8 attributeId)
{
    ...
    if (bgId < 4)
    {
        bgCnt.bits = GetGpuReg(gBattleAnimBgCntGet[bgId]);
        switch (attributeId)
        {
        case BG_ANIM_SCREEN_SIZE:
