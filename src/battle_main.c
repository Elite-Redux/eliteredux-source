        ...
        for (i = 0; i < monsCount; i++) {
            ...
            switch (gTrainers[trainerNum].partyFlags) {
                case 0: {
                    ...
                    for (j = 0; gSpecies[partyData[i].species].name[j] != EOS; j++) nameHash += gSpecies[partyData[i].species].name[j];
...
u32 GetBattlerTotalSpeedStat(u8 battlerId, u8 calcType) {
    ...
    if (calcType != TOTAL_SPEED_QUASH) {
        ...
        if (GET_BATTLER_SIDE(battlerId) != B_SIDE_PLAYER && getMonotypeChampType() == TYPE_ICE) speed *= 2;
