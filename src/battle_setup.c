...
static const u8 sBattleTransitionTable_BattlePyramid[] = {
    B_TRANSITION_FRONTIER_SQUARES, B_TRANSITION_FRONTIER_SQUARES_SCROLL, B_TRANSITION_FRONTIER_SQUARES_SPIRAL};

static const u8 sBattleTransitionTable_BattleDome[] = {
    B_TRANSITION_FRONTIER_LOGO_WIGGLE, B_TRANSITION_FRONTIER_SQUARES, B_TRANSITION_FRONTIER_SQUARES_SCROLL, B_TRANSITION_FRONTIER_SQUARES_SPIRAL};
...
const struct RematchTrainer gRematchTable[REMATCH_TABLE_ENTRIES] = {
    ...
    [REMATCH_WALLACE] = REMATCH(TRAINER_WALLACE, TRAINER_WALLACE_2, TRAINER_WALLACE_3, TRAINER_WALLACE_4, SOOTOPOLIS_CITY),
    ...
    [REMATCH_STEVEN] = REMATCH(TRAINER_STEVEN, TRAINER_STEVEN_LEGENDS, TRAINER_STEVEN_DOUBLES, TRAINER_STEVEN_DOUBLES_LEGENDS, EVER_GRANDE_CITY),
};
...
void BattleSetup_StartDoubleWildBattle(void) { DoStandardWildBattle(TRUE); }
...
u8 BattleSetup_GetTerrainId(void) {
    ...
    if (MetatileBehavior_IsTallGrass(tileBehavior)) return BATTLE_TERRAIN_GRASS;
    if (MetatileBehavior_IsLongGrass(tileBehavior)) return BATTLE_TERRAIN_LONG_GRASS;
    if (MetatileBehavior_IsSandOrDeepSand(tileBehavior)) return BATTLE_TERRAIN_SAND;
    ...
    switch (gMapHeader.mapType) {
        ...
        case MAP_TYPE_UNDERGROUND:
            if (MetatileBehavior_IsIndoorEncounter(tileBehavior)) return BATTLE_TERRAIN_BUILDING;
            if (MetatileBehavior_IsSurfableWaterOrUnderwater(tileBehavior)) return BATTLE_TERRAIN_POND;
        ...
        case MAP_TYPE_OCEAN_ROUTE:
            if (MetatileBehavior_IsSurfableWaterOrUnderwater(tileBehavior)) return BATTLE_TERRAIN_WATER;
            return BATTLE_TERRAIN_PLAIN;
    }
    if (MetatileBehavior_IsDeepOrOceanWater(tileBehavior)) return BATTLE_TERRAIN_WATER;
    if (MetatileBehavior_IsSurfableWaterOrUnderwater(tileBehavior)) return BATTLE_TERRAIN_POND;
    if (MetatileBehavior_IsMountain(tileBehavior)) return BATTLE_TERRAIN_MOUNTAIN;
    if (TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_SURFING)) {
        if (MetatileBehavior_GetBridgeType(tileBehavior)) return BATTLE_TERRAIN_POND;
        if (MetatileBehavior_IsBridge(tileBehavior) == TRUE) return BATTLE_TERRAIN_WATER;
    }
    if (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(ROUTE113) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(ROUTE113)) return BATTLE_TERRAIN_SAND;
    if (GetSav1Weather() == 8) return BATTLE_TERRAIN_SAND;
...
static u8 GetBattleTransitionTypeByMap(void) {
    ...
    PlayerGetDestCoords(&x, &y);
    tileBehavior = MapGridGetMetatileBehaviorAt(x, y);
    if (Overworld_GetFlashLevel()) return B_TRANSITION_SHUFFLE;
...
static u8 GetSumOfEnemyPartyLevel(u16 opponentId, u8 numMons) {
    ...
    if (gTrainers[opponentId].partySize < count) count = gTrainers[opponentId].partySize;
...
u8 GetTrainerBattleTransition(void) {
    ...
    if (gTrainerBattleOpponent_A == TRAINER_SECRET_BASE) return B_TRANSITION_CHAMPION;
...
static void TryUpdateGymLeaderRematchFromWild(void) {
    if (GetGameStat(GAME_STAT_WILD_BATTLES) % 60 == 0) UpdateGymLeaderRematch();
}

static void TryUpdateGymLeaderRematchFromTrainer(void) {
    if (GetGameStat(GAME_STAT_TRAINER_BATTLES) % 20 == 0) UpdateGymLeaderRematch();
}
...
const u8 *BattleSetup_ConfigureTrainerBattle(const u8 *data) {
    ...
    switch (sTrainerBattleMode) {
        ...
        case TRAINER_BATTLE_DOUBLE_NO_INTRO_TEXT:
            ...
            SetMapVarsToTrainer();
            return EventScript_TryDoDoubleTrainerBattleNoIntro;
...
u8 GetTrainerBattleMode(void) { return sTrainerBattleMode; }
...
u16 GetTrainerBattleWins(void) { return (VarGet(VAR_TRACKCOUNT_WINS_HIGH) << 8) | (VarGet(VAR_TRACKCOUNT_WINS_LOW)); }
...
static void SetBattledTrainerFlag(void) { FlagSet(GetTrainerAFlag()); }
    ...
    if (InBattlePyramid()) {
        ...
        if (gNoOfApproachingTrainers == 0 || gNoOfApproachingTrainers == 1)
            ...
            CopyTrainerHillTrainerText(TRAINER_HILL_TEXT_INTRO,
                                       LocalIdToHillTrainerId(gObjectEvents[gApproachingTrainers[gApproachingTrainerId].objectEventId].localId));
    ...
    for (i = 0; i <= REMATCH_SPECIAL_TRAINER_START; i++) {
        if (table[i].mapGroup == mapGroup && table[i].mapNum == mapNum && !sub_80B1D94(i)) {
            if (gSaveBlock1Ptr->trainerRematches[i] != 0) {
                ...
                ret = TRUE;
            } else if (FlagGet(FLAG_MATCH_CALL_REGISTERED + i) && (Random() % 100) <= 30)  // 31% chance of getting a rematch.
            {
...
static u32 GetTrainerMatchCallFlag(u32 trainerId) {
    ...
    for (i = 0; i < REMATCH_TABLE_ENTRIES; i++) {
        if (gRematchTable[i].trainerIds[0] == trainerId) return FLAG_MATCH_CALL_REGISTERED + i;
    }
...
#ifndef FREE_MATCH_CALL
    if (IsRematchStepCounterMaxed() && UpdateRandomTrainerRematches(gRematchTable, mapGroup, mapNum) == TRUE) gSaveBlock1Ptr->trainerRematchStepCounter = 0;
#endif
}

bool32 DoesSomeoneWantRematchIn(u16 mapGroup, u16 mapNum) { return DoesSomeoneWantRematchIn_(gRematchTable, mapGroup, mapNum); }

bool32 IsRematchTrainerIn(u16 mapGroup, u16 mapNum) { return IsRematchTrainerIn_(gRematchTable, mapGroup, mapNum); }

static u16 GetRematchTrainerId(u16 trainerId) { return GetRematchTrainerIdFromTable(gRematchTable, trainerId); }

u16 GetLastBeatenRematchTrainerId(u16 trainerId) { return GetLastBeatenRematchTrainerIdFromTable(gRematchTable, trainerId); }

bool8 ShouldTryRematchBattle(void) {
    if (IsFirstTrainerIdReadyForRematch(gRematchTable, gTrainerBattleOpponent_A)) return TRUE;

    return WasSecondRematchWon(gRematchTable, gTrainerBattleOpponent_A);
}

bool8 IsTrainerReadyForRematch(void) { return IsTrainerReadyForRematch_(gRematchTable, gTrainerBattleOpponent_A); }
...
u16 CountBattledRematchTeams(u16 trainerId) {
    ...
    if (HasTrainerBeenFought(gRematchTable[trainerId].trainerIds[0]) != TRUE) return 0;

    for (i = 1; i < REMATCHES_COUNT; i++) {
        if (gRematchTable[trainerId].trainerIds[i] == 0) break;
        if (!HasTrainerBeenFought(gRematchTable[trainerId].trainerIds[i])) break;
    }
