...
#define PARTNER_TEXTS(name)                                                                                                              \
    BattleFrontier_BattleTowerMultiPartnerRoom_Text_##name##Intro, BattleFrontier_BattleTowerMultiPartnerRoom_Text_##name##Mon1,         \
...
static const u16 sRecordTrainerSpeechWon[] = {EC_WORD_YAY, EC_WORD_YAY, EC_WORD_EXCL_EXCL, EC_WORD_I_VE, EC_WORD_WON, EC_WORD_EXCL_EXCL};

static const u16 sRecordTrainerSpeechLost[] = {EC_WORD_TOO, EC_WORD_BAD, EC_WORD_ELLIPSIS, EC_WORD_WE, EC_WORD_LOST, EC_WORD_ELLIPSIS};
...
static void SetTowerBattleWon(void) {
    ...
    if (gTrainerBattleOpponent_A == TRAINER_EREADER) ClearEReaderTrainer(&gSaveBlock2Ptr->frontier.ereaderTrainer);
#endif

    // towerNumWins is never read outside this conditional
    if (gSaveBlock2Ptr->frontier.towerNumWins < MAX_STREAK) gSaveBlock2Ptr->frontier.towerNumWins++;
...
static bool8 ChooseSpecialBattleTowerTrainer(void) {
    ...
    if (VarGet(VAR_FRONTIER_FACILITY) != FRONTIER_FACILITY_TOWER) return FALSE;
    ...
    for (i = 0; i < BATTLE_TOWER_RECORD_COUNT; i++) {
        ...
        if (validMons >= sBattleTowerPartySizes2[battleMode] && gSaveBlock2Ptr->frontier.towerRecords[i].winStreak == winStreak &&
            gSaveBlock2Ptr->frontier.towerRecords[i].lvlMode == lvlMode && recordHasData && gSaveBlock2Ptr->frontier.towerRecords[i].checksum == checksum) {
    ...
    if (battleMode == FRONTIER_MODE_SINGLES) {
        ...
        for (i = 0; i < APPRENTICE_COUNT; i++) {
            if (gSaveBlock2Ptr->apprentices[i].lvlMode != 0 && sApprenticeChallengeThreshold[gSaveBlock2Ptr->apprentices[i].numQuestions] == winStreak &&
        ...
        if (battleMode == FRONTIER_MODE_MULTIS || battleMode == FRONTIER_MODE_LINK_MULTIS) {
            ...
            while (1) {
                ...
                for (i = 0; i < gSaveBlock2Ptr->frontier.curChallengeBattleNum; i++) {
                    if (gSaveBlock2Ptr->frontier.trainerIds[i] == id) break;
                }
                if (i == gSaveBlock2Ptr->frontier.curChallengeBattleNum) break;
            }
...
void SetBattleFacilityTrainerGfxId(u16 trainerId, u8 tempVarId) {
    ...
    for (i = 0; i < ARRAY_COUNT(gTowerMaleFacilityClasses); i++) {
        if (gTowerMaleFacilityClasses[i] == facilityClass) break;
    }
    ...
    for (i = 0; i < ARRAY_COUNT(gTowerFemaleFacilityClasses); i++) {
        if (gTowerFemaleFacilityClasses[i] == facilityClass) break;
    }
...
void SetEReaderTrainerGfxId(void) { SetBattleFacilityTrainerGfxId(TRAINER_EREADER, 0); }
...
void PutNewBattleTowerRecord(struct EmeraldBattleTowerRecord *newRecordEm) {
    ...
    for (i = 0; i < BATTLE_TOWER_RECORD_COUNT; i++) {
        ...
        for (j = 0; j < TRAINER_ID_LENGTH; j++) {
            if (gSaveBlock2Ptr->frontier.towerRecords[i].trainerId[j] != newRecord->trainerId[j]) break;
        }
        if (j == 4) {
            for (k = 0; k < PLAYER_NAME_LENGTH; k++) {
#ifdef BUGFIX
                if (gSaveBlock2Ptr->frontier.towerRecords[i].name[k] != newRecord->name[k]) break;
                if (newRecord->name[k] == EOS)
#else
                if (gSaveBlock2Ptr->frontier.towerRecords[i].name[j] != newRecord->name[j]) break;
    ...
    for (i = 0; i < BATTLE_TOWER_RECORD_COUNT; i++) {
        if (gSaveBlock2Ptr->frontier.towerRecords[i].winStreak == 0) break;
    }
    if (i < BATTLE_TOWER_RECORD_COUNT) {
        gSaveBlock2Ptr->frontier.towerRecords[i] = *newRecord;
        return;
...
u8 GetFrontierOpponentClass(u16 trainerId) {
...
#endif
    {
        ...
        if (gBattleTypeFlags & BATTLE_TYPE_RECORDED) {
            ...
            trainerClass =
                gFacilityClassToTrainerClass[gApprentices[gSaveBlock2Ptr->apprentices[trainerId - TRAINER_RECORD_MIXING_APPRENTICE].id].facilityClass];
        }
    }
...
void GetFrontierTrainerName(u8 *dst, u16 trainerId) {
    ...
    if (trainerId == TRAINER_EREADER) {
#ifndef FREE_BATTLE_TOWER_E_READER
        for (i = 0; i < PLAYER_NAME_LENGTH; i++) dst[i] = gSaveBlock2Ptr->frontier.ereaderTrainer.name[i];
#endif
    } else if (trainerId == TRAINER_FRONTIER_BRAIN) {
        CopyFrontierBrainTrainerName(dst);
        return;
    } else if (trainerId == TRAINER_STEVEN_PARTNER) {
        for (i = 0; i < PLAYER_NAME_LENGTH; i++) dst[i] = gTrainers[TRAINER_STEVEN].trainerName[i];
    } else if (trainerId >= TRAINER_CUSTOM_PARTNER) {
        ...
        for (i = 0; i < PLAYER_NAME_LENGTH; i++) dst[i] = gFacilityTrainers[trainerId].trainerName[i];
    } else if (trainerId < TRAINER_RECORD_MIXING_APPRENTICE) {
        if (gBattleTypeFlags & BATTLE_TYPE_RECORDED) {
...
static void FillTrainerParty(u16 trainerId, u8 firstMonId, u8 monCount) {
...
#ifndef FREE_BATTLE_TOWER_E_READER
        for (i = firstMonId; i < firstMonId + 3; i++) CreateBattleTowerMon(&gEnemyParty[i], &gSaveBlock2Ptr->frontier.ereaderTrainer.party[i - firstMonId]);
#endif
        return;
    } else if (trainerId == TRAINER_FRONTIER_BRAIN) {
    ...
    while (i != monCount) {
        ...
        if ((level == 50 || level == 20) && monId > FRONTIER_MONS_HIGH_TIER) continue;

        // Ensure this pokemon species isn't a duplicate.
        for (j = 0; j < i + firstMonId; j++) {
            if (GetMonData(&gEnemyParty[j], MON_DATA_SPECIES, NULL) == gFacilityTrainerMons[monId].species) break;
        }
        ...
        for (j = 0; j < i; j++) {
            if (chosenMonIndices[j] == monId) break;
        }
        if (j != i) continue;
...
static void Unused_CreateApprenticeMons(u16 trainerId, u8 firstMonId) {
    ...
    for (i = 0; i != 3; i++) {
        ...
        friendship = MAX_FRIENDSHIP;
        for (j = 0; j < MAX_MON_MOVES; j++) {
            if (apprentice->party[i].moves[j] == MOVE_FRUSTRATION) friendship = 0;
        }
...
void FrontierSpeechToString(const u16 *words) {
    ...
    if (GetStringWidth(1, gStringVar4, -1) > 204u) {
        ...
        while (gStringVar4[i] != CHAR_NEWLINE) i++;
    ...
    switch (gBattleScripting.specialTrainerBattleType) {
        ...
        case SPECIAL_BATTLE_PYRAMID:
            if (gSaveBlock2Ptr->frontier.battlesCount < 0xFFFFFF) {
                ...
                if (gSaveBlock2Ptr->frontier.battlesCount % 20 == 0) UpdateGymLeaderRematch();
            } else {
        ...
        case SPECIAL_BATTLE_MULTI:
            for (i = 0; i < 3; i++) {
                if (GetMonData(&gPlayerParty[i], MON_DATA_SPECIES)) gSaveBlock1Ptr->playerParty[i] = gPlayerParty[i];
            }
            break;
    }
...
void DoSpecialTrainerBattle(void) {
    ...
    switch (gSpecialVar_0x8004) {
...
#ifndef FREE_BATTLE_TOWER_E_READER
            ZeroEnemyPartyMons();
            for (i = 0; i < 3; i++) CreateBattleTowerMon(&gEnemyParty[i], &gSaveBlock2Ptr->frontier.ereaderTrainer.party[i]);
        ...
        case SPECIAL_BATTLE_DOME:
            ...
            if (VarGet(VAR_FRONTIER_BATTLE_MODE) == FRONTIER_MODE_DOUBLES) gBattleTypeFlags |= BATTLE_TYPE_DOUBLE;
            if (gTrainerBattleOpponent_A == TRAINER_FRONTIER_BRAIN) FillFrontierTrainerParty(DOME_BATTLE_PARTY_SIZE);
            CreateTask(Task_StartBattleAfterTransition, 1);
...
static void SaveCurrentWinStreak(void) {
    ...
    if (gSaveBlock2Ptr->frontier.towerWinStreaks[battleMode][lvlMode] < winStreak) gSaveBlock2Ptr->frontier.towerWinStreaks[battleMode][lvlMode] = winStreak;
}
...
static void SaveBattleTowerRecord(void) {
    ...
    if (gSaveBlock2Ptr->playerGender != MALE) {
        class = gTowerFemaleFacilityClasses[(gSaveBlock2Ptr->playerTrainerId[0] + gSaveBlock2Ptr->playerTrainerId[1] + gSaveBlock2Ptr->playerTrainerId[2] +
                                             gSaveBlock2Ptr->playerTrainerId[3]) %
                                            ARRAY_COUNT(gTowerFemaleFacilityClasses)];
    } else {
        class = gTowerMaleFacilityClasses[(gSaveBlock2Ptr->playerTrainerId[0] + gSaveBlock2Ptr->playerTrainerId[1] + gSaveBlock2Ptr->playerTrainerId[2] +
                                           gSaveBlock2Ptr->playerTrainerId[3]) %
...
static void SaveTowerChallenge(void) {
    ...
    if (gSpecialVar_0x8005 == 0 && (challengeNum > 1 || gSaveBlock2Ptr->frontier.curChallengeBattleNum != 0)) SaveBattleTowerRecord();
...
static void LoadMultiPartnerCandidatesData(void) {
    ...
    do {
        do {
            ...
            for (i = 0; i < j; i++) {
                if (gSaveBlock2Ptr->frontier.trainerIds[i] == trainerId) break;
                if (gFacilityTrainers[gSaveBlock2Ptr->frontier.trainerIds[i]].facilityClass == gFacilityTrainers[trainerId].facilityClass) break;
            }
        } while (i != j);
        ...
        for (j = 0; j < 2; j++) {
            while (1) {
                ...
                for (k = 8; k < r10; k++) {
                    ...
                    if (species1 == gFacilityTrainerMons[monId].species) break;
                    if (species2 == gFacilityTrainerMons[monId].species) break;
                }
                if (k == r10) break;
            }
    ...
    ValidateApprenticesChecksums();
    for (i = 0; i < APPRENTICE_COUNT; i++) {
        if (gSaveBlock2Ptr->apprentices[i].lvlMode != 0 && sApprenticeChallengeThreshold[gSaveBlock2Ptr->apprentices[i].numQuestions] / 7 <= challengeNum &&
            gSaveBlock2Ptr->apprentices[i].lvlMode - 1 == lvlMode) {
            k = 0;
    ...
    for (i = 0; i < BATTLE_TOWER_RECORD_COUNT; i++) {
        ...
        if (gSaveBlock2Ptr->frontier.towerRecords[i].winStreak / 7 <= challengeNum && gSaveBlock2Ptr->frontier.towerRecords[i].lvlMode == lvlMode &&
            recordHasData && gSaveBlock2Ptr->frontier.towerRecords[i].checksum == checksum) {
            k = 0;
            for (j = 0; j < MAX_FRONTIER_PARTY_SIZE; j++) {
...
static void GetPotentialPartnerMoveAndSpecies(u16 trainerId, u16 monId) {
    ...
    if (trainerId != TRAINER_EREADER) {
        if (trainerId < FRONTIER_TRAINERS_COUNT) {
            ...
            for (i = 0; i < PLAYER_NAME_LENGTH; i++) gStringVar3[i] = gSaveBlock2Ptr->apprentices[trainerId - TRAINER_RECORD_MIXING_APPRENTICE].playerName[i];
            gStringVar3[i] = EOS;
...
static void ShowPartnerCandidateMessage(void) {
    ...
    for (partnerId = 0; partnerId < ARRAY_COUNT(sPartnerTrainerTextTables); partnerId++) {
        if (sPartnerTrainerTextTables[partnerId].facilityClass == GetFrontierTrainerFacilityClass(trainerId)) break;
    }

    switch (gSpecialVar_0x8005) {
        case PARTNER_MSGID_INTRO:
            if (trainerId == TRAINER_EREADER) return;
            if (trainerId < FRONTIER_TRAINERS_COUNT) {
                ...
                ConvertIntToDecimalStringN(
                    gStringVar2, gSaveBlock2Ptr->apprentices[trainerId - TRAINER_RECORD_MIXING_APPRENTICE].number, STR_CONV_MODE_LEFT_ALIGN, 3);
        ...
        case PARTNER_MSGID_ACCEPT:
            ...
            for (k = 0; k < 14; k++) {
                while (1) {
                    ...
                    if (gPartnerTrainerId == i) continue;

                    for (j = 0; j < k; j++) {
                        if (gSaveBlock2Ptr->frontier.trainerIds[j] == i) break;
                    }
                    if (j == k) break;
                }
            ...
            if ((GetBlockReceivedStatus() & 3) == 3) {
                ...
                for (i = 0; i < 14; i++) {
                    do {
                        ...
                        for (j = 0; j < i; j++) {
                            if (gSaveBlock2Ptr->frontier.trainerIds[j] == trainerId) break;
                        }
                    } while (i != j);
...
static void SetTowerInterviewData(void) {
    ...
    if (VarGet(VAR_FRONTIER_BATTLE_MODE) != FRONTIER_MODE_SINGLES) return;
    ...
    for (i = 0; i < POKEMON_NAME_LENGTH + 1; i++) gSaveBlock2Ptr->frontier.towerInterview.opponentMonNickname[i] = gBattleMons[0].nickname[i];
...
static void ValidateBattleTowerRecordChecksums(void) {
    ...
    for (i = 0; i < BATTLE_TOWER_RECORD_COUNT; i++) {
        ...
        if (gSaveBlock2Ptr->frontier.towerRecords[i].checksum != checksum) ClearBattleTowerRecord(&gSaveBlock2Ptr->frontier.towerRecords[i]);
    }
}
...
static void FillEReaderTrainerWithPlayerData(void) {
    ...
    for (i = 0; i < 3; i++) ConvertPokemonToBattleTowerPokemon(&gPlayerParty[i], &ereaderTrainer->party[i]);
...
void GetEreaderTrainerName(u8 *dst) {
    ...
    for (i = 0; i < 5; i++) dst[i] = gSaveBlock2Ptr->frontier.ereaderTrainer.name[i];
...
void TryHideBattleTowerReporter(void) {
    if (gSaveBlock2Ptr->frontier.challengeStatus == CHALLENGE_STATUS_SAVING) HideBattleTowerReporter();
    ...
    if (trainerId == TRAINER_STEVEN_PARTNER) {
        for (i = 0; i < MULTI_PARTY_SIZE; i++) {
            ...
            CreateMon(&gPlayerParty[MULTI_PARTY_SIZE + i],
                      ...
                      OT_ID_PRESET,
                      STEVEN_OTID);
            for (j = 0; j < PARTY_SIZE; j++) SetMonData(&gPlayerParty[MULTI_PARTY_SIZE + i], MON_DATA_HP_EV + j, &sStevenMons[i].evs[j]);
            for (j = 0; j < MAX_MON_MOVES; j++) SetMonMoveSlot(&gPlayerParty[MULTI_PARTY_SIZE + i], sStevenMons[i].moves[j], j);
        ...
        for (i = 0; i < 3; i++) ZeroMonData(&gPlayerParty[i + 3]);
        ...
        for (i = 0; i < 3 && i < gTrainers[trainerId - TRAINER_CUSTOM_PARTNER].partySize; i++) {
            ...
            for (j = 0; gTrainerBallTable[j].classId != 0xFF; j++) {
                if (gTrainerBallTable[j].classId == gTrainers[trainerId - TRAINER_CUSTOM_PARTNER].trainerClass) break;
            }
        ...
        for (i = 0; i < FRONTIER_MULTI_PARTY_SIZE; i++) {
            ...
            if (record->language == LANGUAGE_JAPANESE) {
                ...
                if (monData.nickname[0] == EXT_CTRL_CODE_BEGIN && monData.nickname[1] == EXT_CTRL_CODE_JPN) trainerName[5] = EOS;
            }
...
bool32 RubyBattleTowerRecordToEmerald(struct RSBattleTowerRecord *src, struct EmeraldBattleTowerRecord *dst) {
    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++) {
        if (src->party[i].species) validMons++;
    }
...
// UB: Reading outside the array. sRubyFacilityClassToEmerald has less than FACILITY_CLASSES_COUNT entries.
...
#endif
        {
            if (sRubyFacilityClassToEmerald[i][0] == src->facilityClass) break;
        }
        ...
        for (i = 0; i < PLAYER_NAME_LENGTH + 1; i++) dst->name[i] = src->name[i];
        for (i = 0; i < TRAINER_ID_LENGTH; i++) dst->trainerId[i] = src->trainerId[i];
        for (i = 0; i < EASY_CHAT_BATTLE_WORDS_COUNT; i++) dst->greeting[i] = src->greeting[i];
        for (i = 0; i < EASY_CHAT_BATTLE_WORDS_COUNT; i++) dst->speechWon[i] = sRecordTrainerSpeechWon[i];
        for (i = 0; i < EASY_CHAT_BATTLE_WORDS_COUNT; i++) dst->speechLost[i] = sRecordTrainerSpeechLost[i];
        for (i = 0; i < FRONTIER_PARTY_SIZE; i++) dst->party[i] = src->party[i];
...
#endif
        {
            if (sRubyFacilityClassToEmerald[i][1] == src->facilityClass) break;
        }
        if (i != FACILITY_CLASSES_COUNT)
