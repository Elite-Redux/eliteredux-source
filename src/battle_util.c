...
void TryToApplyMimicry(u8 battlerId, bool8 various) {
    ...
    if (moveType != 0 && !IS_BATTLER_OF_TYPE(battlerId, moveType)) {
        ...
        if (!various) BattleScriptPushCursorAndCallback(BattleScript_MimicryActivatesEnd3);
    }
}
...
void TryToRevertMimicry(void) {
    ...
    for (i = 0; i < gBattlersCount; i++) {
        if (BattlerHasAbility(i, ABILITY_MIMICRY, FALSE)) RestoreBattlerOriginalTypes(i);
    }
}
...
bool32 ShouldChangeFormHpBased(u32 battler) {
    ...
    for (i = 0; i < ARRAY_COUNT(sHpTransformations); i++) {
        if (BattlerHasAbility(battler, sHpTransformations[i][0], FALSE)) {
            ...
            if (species == sHpTransformations[i][2] && gBattleMons[battler].hp > gBattleMons[battler].maxHP / sHpTransformations[i][3]) {
            ...
            if (species == sHpTransformations[i][1] && gBattleMons[battler].hp <= gBattleMons[battler].maxHP / sHpTransformations[i][3]) {
                gBattleScripting.abilityPopupOverwrite = sHpTransformations[i][0];
...
AbilityStates GetAbilityStateAs(u8 battler, u16 ability) { return (AbilityStates){.intValue = GetAbilityState(battler, ability)}; }
...
void SetAbilityStateAs(u8 battler, u16 ability, AbilityStates value) { SetAbilityState(battler, ability, value.intValue); }
...
void IncrementAbilityState(u8 battler, u16 ability, u32 value) { SetAbilityState(battler, ability, GetAbilityState(battler, ability) + value); }
...
bool32 IsUnsuppressableAbility(u32 ability) { return gAbilities[ability].unsuppressable; }

int IsPersistentOrUnsuppressableAbility(int ability) { return gAbilities[ability].unsuppressable || gAbilities[ability].persistent; }
...
u32 IsAbilityOnOpposingSide(u32 battlerId, u32 ability) { return IsAbilityOnSide(BATTLE_OPPOSITE(battlerId), ability); }
...
const struct TypePower gNaturalGiftTable[] = {
    [ITEM_TO_BERRY(ITEM_CHERI_BERRY)] = {TYPE_FIRE, 80},      [ITEM_TO_BERRY(ITEM_CHESTO_BERRY)] = {TYPE_WATER, 80},
    [ITEM_TO_BERRY(ITEM_PECHA_BERRY)] = {TYPE_ELECTRIC, 80},  [ITEM_TO_BERRY(ITEM_RAWST_BERRY)] = {TYPE_GRASS, 80},
    [ITEM_TO_BERRY(ITEM_ASPEAR_BERRY)] = {TYPE_ICE, 80},      [ITEM_TO_BERRY(ITEM_LEPPA_BERRY)] = {TYPE_FIGHTING, 80},
    [ITEM_TO_BERRY(ITEM_ORAN_BERRY)] = {TYPE_POISON, 80},     [ITEM_TO_BERRY(ITEM_PERSIM_BERRY)] = {TYPE_GROUND, 80},
    [ITEM_TO_BERRY(ITEM_LUM_BERRY)] = {TYPE_FLYING, 80},      [ITEM_TO_BERRY(ITEM_SITRUS_BERRY)] = {TYPE_PSYCHIC, 80},
    [ITEM_TO_BERRY(ITEM_FIGY_BERRY)] = {TYPE_BUG, 80},        [ITEM_TO_BERRY(ITEM_WIKI_BERRY)] = {TYPE_ROCK, 80},
    [ITEM_TO_BERRY(ITEM_MAGO_BERRY)] = {TYPE_GHOST, 80},      [ITEM_TO_BERRY(ITEM_AGUAV_BERRY)] = {TYPE_DRAGON, 80},
    [ITEM_TO_BERRY(ITEM_IAPAPA_BERRY)] = {TYPE_DARK, 80},     [ITEM_TO_BERRY(ITEM_RAZZ_BERRY)] = {TYPE_STEEL, 80},
    [ITEM_TO_BERRY(ITEM_OCCA_BERRY)] = {TYPE_FIRE, 80},       [ITEM_TO_BERRY(ITEM_PASSHO_BERRY)] = {TYPE_WATER, 80},
    [ITEM_TO_BERRY(ITEM_WACAN_BERRY)] = {TYPE_ELECTRIC, 80},  [ITEM_TO_BERRY(ITEM_RINDO_BERRY)] = {TYPE_GRASS, 80},
    [ITEM_TO_BERRY(ITEM_YACHE_BERRY)] = {TYPE_ICE, 80},       [ITEM_TO_BERRY(ITEM_CHOPLE_BERRY)] = {TYPE_FIGHTING, 80},
    [ITEM_TO_BERRY(ITEM_KEBIA_BERRY)] = {TYPE_POISON, 80},    [ITEM_TO_BERRY(ITEM_SHUCA_BERRY)] = {TYPE_GROUND, 80},
    [ITEM_TO_BERRY(ITEM_COBA_BERRY)] = {TYPE_FLYING, 80},     [ITEM_TO_BERRY(ITEM_PAYAPA_BERRY)] = {TYPE_PSYCHIC, 80},
    [ITEM_TO_BERRY(ITEM_TANGA_BERRY)] = {TYPE_BUG, 80},       [ITEM_TO_BERRY(ITEM_CHARTI_BERRY)] = {TYPE_ROCK, 80},
    [ITEM_TO_BERRY(ITEM_KASIB_BERRY)] = {TYPE_GHOST, 80},     [ITEM_TO_BERRY(ITEM_HABAN_BERRY)] = {TYPE_DRAGON, 80},
    [ITEM_TO_BERRY(ITEM_COLBUR_BERRY)] = {TYPE_DARK, 80},     [ITEM_TO_BERRY(ITEM_BABIRI_BERRY)] = {TYPE_STEEL, 80},
    [ITEM_TO_BERRY(ITEM_CHILAN_BERRY)] = {TYPE_NORMAL, 80},   [ITEM_TO_BERRY(ITEM_ROSELI_BERRY)] = {TYPE_FAIRY, 80},
    [ITEM_TO_BERRY(ITEM_BLUK_BERRY)] = {TYPE_FIRE, 90},       [ITEM_TO_BERRY(ITEM_NANAB_BERRY)] = {TYPE_WATER, 90},
    [ITEM_TO_BERRY(ITEM_WEPEAR_BERRY)] = {TYPE_ELECTRIC, 90}, [ITEM_TO_BERRY(ITEM_PINAP_BERRY)] = {TYPE_GRASS, 90},
    [ITEM_TO_BERRY(ITEM_POMEG_BERRY)] = {TYPE_ICE, 90},       [ITEM_TO_BERRY(ITEM_KELPSY_BERRY)] = {TYPE_FIGHTING, 90},
    [ITEM_TO_BERRY(ITEM_QUALOT_BERRY)] = {TYPE_POISON, 90},   [ITEM_TO_BERRY(ITEM_HONDEW_BERRY)] = {TYPE_GROUND, 90},
    [ITEM_TO_BERRY(ITEM_GREPA_BERRY)] = {TYPE_FLYING, 90},    [ITEM_TO_BERRY(ITEM_TAMATO_BERRY)] = {TYPE_PSYCHIC, 90},
    [ITEM_TO_BERRY(ITEM_CORNN_BERRY)] = {TYPE_BUG, 90},       [ITEM_TO_BERRY(ITEM_MAGOST_BERRY)] = {TYPE_ROCK, 90},
    [ITEM_TO_BERRY(ITEM_RABUTA_BERRY)] = {TYPE_GHOST, 90},    [ITEM_TO_BERRY(ITEM_NOMEL_BERRY)] = {TYPE_DRAGON, 90},
    [ITEM_TO_BERRY(ITEM_SPELON_BERRY)] = {TYPE_DARK, 90},     [ITEM_TO_BERRY(ITEM_PAMTRE_BERRY)] = {TYPE_STEEL, 90},
    [ITEM_TO_BERRY(ITEM_WATMEL_BERRY)] = {TYPE_FIRE, 100},    [ITEM_TO_BERRY(ITEM_DURIN_BERRY)] = {TYPE_WATER, 100},
    [ITEM_TO_BERRY(ITEM_BELUE_BERRY)] = {TYPE_ELECTRIC, 100}, [ITEM_TO_BERRY(ITEM_LIECHI_BERRY)] = {TYPE_GRASS, 100},
    [ITEM_TO_BERRY(ITEM_GANLON_BERRY)] = {TYPE_ICE, 100},     [ITEM_TO_BERRY(ITEM_SALAC_BERRY)] = {TYPE_FIGHTING, 100},
    [ITEM_TO_BERRY(ITEM_PETAYA_BERRY)] = {TYPE_POISON, 100},  [ITEM_TO_BERRY(ITEM_APICOT_BERRY)] = {TYPE_GROUND, 100},
    [ITEM_TO_BERRY(ITEM_LANSAT_BERRY)] = {TYPE_FLYING, 100},  [ITEM_TO_BERRY(ITEM_STARF_BERRY)] = {TYPE_PSYCHIC, 100},
    [ITEM_TO_BERRY(ITEM_ENIGMA_BERRY)] = {TYPE_BUG, 100},     [ITEM_TO_BERRY(ITEM_MICLE_BERRY)] = {TYPE_ROCK, 100},
    [ITEM_TO_BERRY(ITEM_CUSTAP_BERRY)] = {TYPE_GHOST, 100},   [ITEM_TO_BERRY(ITEM_JABOCA_BERRY)] = {TYPE_DRAGON, 100},
    [ITEM_TO_BERRY(ITEM_ROWAP_BERRY)] = {TYPE_DARK, 100},     [ITEM_TO_BERRY(ITEM_KEE_BERRY)] = {TYPE_FAIRY, 100},
...
static void CalculateOffensiveAbilityMultiplier(int ability, int battlerAtk, int battlerDef, int move, int moveType, int basePower,
    ...
    switch (ability) {
        ...
        case ABILITY_COMBAT_SPECIALIST:
            CalculateOffensiveAbilityMultiplier(
                ABILITY_IRON_FIST, battlerAtk, battlerDef, move, moveType, basePower, typeEffectivenessMultiplier, isCrit, resistance, modifier);
            CalculateOffensiveAbilityMultiplier(
                ABILITY_STRIKER, battlerAtk, battlerDef, move, moveType, basePower, typeEffectivenessMultiplier, isCrit, resistance, modifier);
            return;
...
#undef MUL_ATE
        ...
        case ABILITY_RIVALRY: {
            ...
            if (genderAtk != MON_GENDERLESS &&
                genderAtk == GetGenderFromSpeciesAndPersonality(gBattleMons[battlerDef].species, gBattleMons[battlerDef].personality))
                MUL(1.25);
        }
            return;
...
#undef SUPER_SWARM_LIKE
        ...
        case ABILITY_LETHARGY:
            if (gVolatileStructs[battlerAtk].slowStartTimer == 0 || gVolatileStructs[battlerAtk].slowStartTimer == 1)
                MUL(.2);
            else if (gVolatileStructs[battlerAtk].slowStartTimer == 2)
                MUL(.4);
            else if (gVolatileStructs[battlerAtk].slowStartTimer == 3)
                MUL(.6);
            else if (gVolatileStructs[battlerAtk].slowStartTimer == 4)
                MUL(.8);
            return;
        ...
        case ABILITY_AERIALIST:
            CalculateOffensiveAbilityMultiplier(
                ABILITY_LEVITATE, battlerAtk, battlerDef, move, moveType, basePower, typeEffectivenessMultiplier, isCrit, resistance, modifier);
            CalculateOffensiveAbilityMultiplier(
                ABILITY_FLOCK, battlerAtk, battlerDef, move, moveType, basePower, typeEffectivenessMultiplier, isCrit, resistance, modifier);
            return;
...
static void CalculateDefensiveAbilityMultiplier(int ability, int battlerAtk, int battlerDef, int move, int moveType, int typeEffectivenessModifier, int isCrit,
    ...
    switch (ability) {
        ...
        case ABILITY_PRETTY_PRINCESS:
            if (!IsUnaware(battlerAtk) && HasAnyLoweredStat(battlerDef)) MUL(1.5);
            return;
...
u16 CalculateAbilityMultipliers(int battlerAtk, int battlerDef, int move, int moveType, int basePower, int typeEffectivenessMultiplier, int isCrit,
    ...
    for (i = 0; i < NUM_ABILITY_SLOTS + 1; i++) {
        CalculateDefensiveAbilityMultiplier(GetAbilityAtIndex(battlerDef, i, TRUE),
                                            battlerAtk,
                                            battlerDef,
                                            move,
                                            moveType,
    ...
    if (IsBattlerAlive(BATTLE_PARTNER(battlerAtk))) {
        ...
        if (BattlerHasAbility(BATTLE_PARTNER(battlerAtk), ABILITY_MOSH_PIT, TRUE)) {
            if (gBattleMoves[move].flags & FLAG_RECKLESS_BOOST)
                MUL_MODIFIER(&modifier, 1.25);
            else
                MUL_MODIFIER(&modifier, 1.5);
        }
    }
...
int HandleAttackerAbility(int abilityNumber, int battler, int target, int move) {
    ...
    switch (ability) {
        case ABILITY_SHIELDS_DOWN:
            ...
            for (i = 0; i < ARRAY_COUNT(sHpTransformations); i++) {
                if (sHpTransformations[i][0] == ability && gBattleMons[battler].species == sHpTransformations[i][1]) break;
            }
        ...
        case ABILITY_GULP_MISSILE:
            ...
            REQUIRE(((gCurrentMove == MOVE_SURF || gCurrentMove == MOVE_TRIPLE_DIVE) && TARGET_TURN_DAMAGED) || gStatuses3[battler] & STATUS3_UNDERWATER ||
        ...
        HANDLE_ABILITY_RECOIL_EFFECT:
            ...
            if (!gBattleMoveDamage)
                gBattleMoveDamage = 1;
            else if (gBattleMoveDamage >= gBattleMons[battler].hp)
        ...
        case ABILITY_ANGELS_WRATH:
            switch (move) {
                ...
                case MOVE_STRING_SHOT:
                    ...
                    if (gSideStatuses[i] & SIDE_STATUS_STEALTH_ROCK && gSideStatuses[i] & SIDE_STATUS_TOXIC_SPIKES && gSideStatuses[i] & SIDE_STATUS_SPIKES &&
    ...
    switch (ability) {
        ...
        case ABILITY_DRY_SKIN:
            if (IsBattlerWeatherAffected(battler, WEATHER_SUN_ANY) && !BATTLER_HAS_MAGIC_GUARD(battler)) {
                ...
                return TRUE;
            } else if (HandleEndTurnAbilityAs(ABILITY_RAIN_DISH, battler))
                return TRUE;
            break;
