...
static const struct BgTemplate sBackgroundTemplates[] = {
    {.bg = 0, .charBaseIndex = 2, .mapBaseIndex = 28, .screenSize = 0, .paletteMode = 0, .priority = 0, .baseTile = 0},
};
...
void CB2_StartCreditsSequence(void) {
    ...
    while (TRUE) {
        if (LoadBikeScene(SCENE_OCEAN_MORNING, taskId)) break;
    }
...
static void Task_WaitPaletteFade(u8 taskId) {
    if (!gPaletteFade.active) gTasks[taskId].func = Task_CreditsMain;
}
...
static void Task_LoadShowMons(u8 taskId) {
    switch (gMain.state) {
        ...
        case 0: {
            ...
            for (i = 0; i < MON_PIC_SIZE; i++) gDecompressionBuffer[i] = 0x11;
            for (i = 0; i < MON_PIC_SIZE; i++) (gDecompressionBuffer + MON_PIC_SIZE)[i] = 0x22;
            for (i = 0; i < MON_PIC_SIZE; i++) (gDecompressionBuffer + MON_PIC_SIZE * 2)[i] = 0x33;
...
static void Task_CreditsTheEnd6(u8 taskId) {
    if (!gPaletteFade.active) {
        ...
        if (gTasks[taskId].tDelay == 7144) FadeOutBGM(8);

        if (gTasks[taskId].tDelay == 6840) m4aSongNumStart(MUS_END);
...
static void Task_CreditsSoftReset(u8 taskId) {
    if (!gPaletteFade.active) SoftReset(RESET_ALL);
}
            ...
            if (gTasks[gTasks[taskId].tMainTaskId].func == Task_CreditsMain) {
                if (gTasks[taskId].tCurrentPage < PAGE_COUNT) {
                    ...
                    for (i = 0; i < ENTRIES_PER_PAGE; i++)
                        PrintCreditsText(sCreditsEntryPointerTable[gTasks[taskId].tCurrentPage][i]->text,
                                         5 + i * 16,
...
static void Task_ShowMons(u8 taskId) {
    ...
    switch (gTasks[taskId].tState) {
        case 0:
            break;
        case 1:
            if (sCreditsData->nextImgPos == POS_LEFT && gTasks[gTasks[taskId].tMainTaskId].tPrintedPage == FALSE) break;
            gTasks[taskId].tState++;
            break;
        case 2:
            if (sCreditsData->imgCounter == NUM_MON_SLIDES || gTasks[gTasks[taskId].tMainTaskId].func != Task_CreditsMain) break;
...
static void SetBikeScene(u8 scene, u8 taskId) {
    ...
    if (scene == SCENE_FOREST_RIVAL_ARRIVE) gTasks[gTasks[taskId].tTaskId_BikeScene].tSinIdx = 69;
}
...
static void LoadTheEndScreen(u16 arg0, u16 arg1, u16 palOffset) {
    ...
    for (i = 0; i < 32 * 32; i++) ((u16 *)(VRAM + arg1))[i] = baseTile + 1;
}

static u16 GetLetterMapTile(u8 baseTiles) {
    u16 out = (baseTiles & 0x3F) + 80;

    if (baseTiles == 0xFF) return 1;

    if (baseTiles & (1 << 7)) out |= 1 << 11;
    if (baseTiles & (1 << 6)) out |= 1 << 10;
...
static void DrawLetterMapTiles(const u8 baseTiles[], u8 baseX, u8 baseY, u16 offset, u16 palette) {
    ...
    for (y = 0; y < 5; y++) {
        for (x = 0; x < 3; x++) ((u16 *)(VRAM + offset + (baseY + y) * 64))[baseX + x] = tileOffset + GetLetterMapTile(baseTiles[y * 3 + x]);
    }
}
...
static void DrawTheEnd(u16 offset, u16 palette) {
    ...
    for (pos = 0; pos < 32 * 32; pos++) ((u16 *)(VRAM + offset))[pos] = baseTile + 1;
    ...
    switch (sprite->sState) {
        ...
        case 1:
            if (sprite->x > 200)
                ...
                StartSpriteAnimIfDifferent(sprite, 2);
            if (sprite->x > -32) sprite->x -= 2;
            sprite->y2 = -gIntroCredits_MovingSceneryVOffset;
            break;
        case 2:
            sprite->data[7]++;
            StartSpriteAnimIfDifferent(sprite, 0);
            if ((sprite->data[7] & 3) == 0) sprite->x++;
            break;
        case 3:
            StartSpriteAnimIfDifferent(sprite, 0);
            if (sprite->x > -32) sprite->x--;
...
static void DeterminePokemonToShow(void) {
    ...
    for (dexNum = j; dexNum < NATIONAL_DEX_COUNT; dexNum++) sCreditsData->caughtMonIds[dexNum] = 0;
    ...
    if (sCreditsData->numMonToShow < NUM_MON_SLIDES) {
        for (j = sCreditsData->numMonToShow, page = 0; j < NUM_MON_SLIDES; j++) {
            ...
            page++;
            if (page == sCreditsData->numMonToShow) page = 0;
        }
