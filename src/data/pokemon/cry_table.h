...
// Here you can define directly what cry should use each species, if its not defined it will try to use one depending on its type, egg group and evolution
// stage, this causes more RAM usage when calculating so its advised to define all of them
...
static bool8 CanEvolve(u16 species) {
    ...
    for (i = 0; i < EVOS_PER_MON; i++) {
        if (gEvolutionTable[species][i].method && gEvolutionTable[species][i].method != EVO_MEGA_EVOLUTION) return TRUE;
    }
    return FALSE;
}
...
static u8 GetSpeciesEvolutionStage(u16 species, u16 firstStage) {
    ...
    switch (species) {
        ...
        case SPECIES_CERULEDGE:
            ...
            break;
        default:
            if (firstStage == SPECIES_EEVEE && firstStage != species) return 1;
            break;
    }
