...
static void DexNavGenerateMoveset(SpeciesEnum species, u8 searchLevel, u8 encounterLevel, u16* moveDst);
static u16 DexNavGenerateHeldItem(SpeciesEnum species, u8 searchLevel);
static u8 DexNavGetAbilityNum(SpeciesEnum species, u8 searchLevel);
static u8 DexNavGeneratePotential(u8 searchLevel);
static u8 DexNavTryGenerateMonLevel(SpeciesEnum species, u8 environment);
static u8 GetEncounterLevelFromMapData(SpeciesEnum species, u8 environment);
static void CreateDexNavWildMon(SpeciesEnum species, u8 potential, u8 level, u8 abilityNum, u16 item, u16* moves);
...
static const struct WindowTemplate sDexNavGuiWindowTemplates[] = {[WINDOW_INFO] = {
...
static const struct OamData sSearchIconOam = {.y = 0,
                                              .affineMode = 0,
                                              .objMode = 0,
                                              .mosaic = 0,
                                              .bpp = 0,
...
static const struct OamData sSelectionCursorOam = {.y = 0,
                                                   .affineMode = 0,
                                                   .objMode = 0,
                                                   .mosaic = 0,
                                                   .bpp = 0,
...
static const union AnimCmd sAnimCmdSight0[] = {ANIMCMD_FRAME(0, 1), ANIMCMD_END};
static const union AnimCmd sAnimCmdSight1[] = {ANIMCMD_FRAME(2, 1), ANIMCMD_END};
static const union AnimCmd sAnimCmdSight2[] = {ANIMCMD_FRAME(4, 1), ANIMCMD_END};
...
//// DEXNAV SEARCH ////
///////////////////////
static inline int GetSpecies(const struct WildPokemonInfo* info, int index) { return GetRandomPokemonFromSpecies(info->wildPokemon[index].species); }
...
static void DrawDexNavSearchMonIcon(SpeciesEnum species, u8* dst, bool8 owned) {
    u8 spriteId;
    ...
    *dst = spriteId;
    ...
    if (owned) sDexNavSearchDataPtr->ownedIconSpriteId = CreateSprite(&sOwnedIconTemplate, SPECIES_ICON_X + 6, GetSearchWindowY() + 4, 0);
}
...
static void AddSearchWindow(u8 width) {
    ...
    if (sDexNavSearchDataPtr->tileY > (gSaveBlock1Ptr->pos.y + 7)) y = 1;  // draw at top if chosen tile is below
...
static void AddSearchWindowText(SpeciesEnum species, u8 proximity, u8 searchLevel, bool8 hidden) {
...
static void DrawSearchWindow(SpeciesEnum species, u8 potential, bool8 hidden) {
...
static void RemoveDexNavWindowAndGfx(void) {
    ...
    // try remove sprites
    if (sDexNavSearchDataPtr->iconSpriteId != 0xFF) DestroySprite(&gSprites[sDexNavSearchDataPtr->iconSpriteId]);
    if (sDexNavSearchDataPtr->itemSpriteId != 0xFF) DestroySprite(&gSprites[sDexNavSearchDataPtr->itemSpriteId]);
    if (sDexNavSearchDataPtr->eyeSpriteId != 0xFF) DestroySprite(&gSprites[sDexNavSearchDataPtr->eyeSpriteId]);
    if (sDexNavSearchDataPtr->ownedIconSpriteId != 0xFF) DestroySprite(&gSprites[sDexNavSearchDataPtr->ownedIconSpriteId]);
    if (sDexNavSearchDataPtr->exclamationSpriteId != 0xFF) DestroySprite(&gSprites[sDexNavSearchDataPtr->exclamationSpriteId]);

    for (i = 0; i < NELEMS(sDexNavSearchDataPtr->starSpriteIds); i++) {
        if (sDexNavSearchDataPtr->starSpriteIds[i] != 0xFF) DestroySprite(&gSprites[sDexNavSearchDataPtr->starSpriteIds[i]]);
    }
...
static void DexNavProximityUpdate(void) { sDexNavSearchDataPtr->proximity = GetPlayerDistance(sDexNavSearchDataPtr->tileX, sDexNavSearchDataPtr->tileY); }
...
static bool8 DexnavIsTileUsable(u8 environment) {
    ...
    switch (environment) {
        ...
        case ROW_ROCK_SMASH:
        case ROW_HONEY:
            if (MetatileBehavior_IsLandWildEncounter(tileBehaviour)) return TRUE;
            break;
        case ROW_WATER:
        case ROW_FISHING:
            if (MetatileBehavior_IsSurfableWaterOrUnderwater(tileBehaviour)) return TRUE;
            break;
    }
    return FALSE;
}
...
static bool8 TryStartHiddenMonFieldEffect(u8 environment, u8 xSize, u8 ySize, bool8 smallScan) {
    ...
    if (DexNavPickTile(environment, xSize, ySize, smallScan)) {
        ...
        if (fldEffId != 0) {
            ...
            if (sDexNavSearchDataPtr->fldEffSpriteId == 0xFF) return FALSE;
...
static void DrawDexNavSearchHeldItem(u8* dst) {
    *dst = CreateSprite(&sHeldItemTemplate, SPECIES_ICON_X + 6, GetSearchWindowY() + 18, 0);
    if (*dst != 0xFF) gSprites[*dst].invisible = TRUE;
}
...
static void Task_SetUpDexNavSearch(u8 taskId) {
    SpeciesEnum species = sDexNavSearchDataPtr->species;
...
static void Task_InitDexNavSearch(u8 taskId) {
    ...
    SpeciesEnum species = task->tSpecies;
    ...
    if (gSaveBlock1Ptr->flashLevel > 0) {
        ...
        // Start Encounter
        CreateDexNavWildMon(sDexNavSearchDataPtr->species,
                            sDexNavSearchDataPtr->potential,
                            sDexNavSearchDataPtr->monLevel,
                            sDexNavSearchDataPtr->abilityNum,
                            sDexNavSearchDataPtr->heldItem,
                            sDexNavSearchDataPtr->moves);
...
static void DexNavDrawPotentialStars(u8 potential, u8* dst) {
    ...
    for (i = 0; i < NELEMS(sDexNavSearchDataPtr->starSpriteIds); i++) {
        spriteId = 0xFF;
        if (potential > i) spriteId = CreateSprite(&sPotentialStarTemplate, SPECIES_ICON_X - 20, GetSearchWindowY() + 4 + (i * 8), 0);

        dst[i] = spriteId;
        if (spriteId != 0xFF) gSprites[spriteId].invisible = TRUE;
    }
}
...
bool8 TryStartDexnavSearch(void) {
    u8 taskId;
    SpeciesEnum species = VarGet(VAR_DEXNAV_SPECIES);
    u16 enviorment = VarGet(VAR_DEXNAV_ENVIORMENT);

    if (FlagGet(FLAG_SYS_DEXNAV_SEARCH) || species == SPECIES_NONE) return FALSE;
...
static void Task_DexNavSearch(u8 taskId) {
    ...
    if (sDexNavSearchDataPtr->proximity > MAX_PROXIMITY) {  // out of range
    ...
    if (ScriptContext2_IsEnabled() == TRUE) {  // check if script just executed
        // gSaveBlock1Ptr->dexNavChain = 0;  //issue with reusable repels
        EndDexNavSearch(taskId);
        return;
    }

    if (gTasks[taskId].tFrameCount > DEXNAV_TIMEOUT * 60) {  // player took too long
        if (sDexNavSearchDataPtr->hiddenSearch && !task->tRevealed)
            EndDexNavSearch(taskId);
        else
    ...
    if (sDexNavSearchDataPtr->hiddenSearch && !task->tRevealed && (JOY_NEW(R_BUTTON) || (sDexNavSearchDataPtr->proximity < CREEPING_PROXIMITY))) {
        PlaySE(SE_DEX_SEARCH);
        ClearStdWindowAndFrameToTransparent(sDexNavSearchDataPtr->windowId, FALSE);
        CopyWindowToVram(sDexNavSearchDataPtr->windowId, 3);
        RemoveWindow(sDexNavSearchDataPtr->windowId);
...
static void DexNavUpdateSearchWindow(u8 proximity, u8 searchLevel) {
    ...
    if (species == SPECIES_NONE) hideName = TRUE;  // if a detector mode hidden search and player hasn't seen the mon, hide info
    ...
    // init hidden sprites
    if (sDexNavSearchDataPtr->itemSpriteId != 0xFF) gSprites[sDexNavSearchDataPtr->itemSpriteId].invisible = TRUE;
    if (sDexNavSearchDataPtr->starSpriteIds[0] != 0xFF) gSprites[sDexNavSearchDataPtr->starSpriteIds[0]].invisible = TRUE;
    if (sDexNavSearchDataPtr->starSpriteIds[1] != 0xFF) gSprites[sDexNavSearchDataPtr->starSpriteIds[1]].invisible = TRUE;
    if (sDexNavSearchDataPtr->starSpriteIds[2] != 0xFF) gSprites[sDexNavSearchDataPtr->starSpriteIds[2]].invisible = TRUE;

    if (proximity <= SNEAKING_PROXIMITY) {
        if (searchLevel > 2 && sDexNavSearchDataPtr->heldItem) {
            // toggle item view
            if (sDexNavSearchDataPtr->itemSpriteId != 0xFF) gSprites[sDexNavSearchDataPtr->itemSpriteId].invisible = FALSE;
        }

        if (searchLevel > 4) {
            if (sDexNavSearchDataPtr->starSpriteIds[0] != 0xFF) gSprites[sDexNavSearchDataPtr->starSpriteIds[0]].invisible = FALSE;

            if (sDexNavSearchDataPtr->starSpriteIds[1] != 0xFF) gSprites[sDexNavSearchDataPtr->starSpriteIds[1]].invisible = FALSE;

            if (sDexNavSearchDataPtr->starSpriteIds[2] != 0xFF) gSprites[sDexNavSearchDataPtr->starSpriteIds[2]].invisible = FALSE;
        }
    }
}
...
static void CreateDexNavWildMon(SpeciesEnum species, u8 potential, u8 level, u8 abilityNum, u16 item, u16* moves) {
    ...
    if (DexNavTryMakeShinyMon()) FlagSet(FLAG_SHINY_CREATION);  // just easier this way
    ...
    if (potential > 2 && iv[2] != NUM_STATS) SetMonData(mon, MON_DATA_HP_IV + iv[2], &perfectIv);
    if (potential > 1 && iv[1] != NUM_STATS) SetMonData(mon, MON_DATA_HP_IV + iv[1], &perfectIv);
    if (potential > 0 && iv[0] != NUM_STATS) SetMonData(mon, MON_DATA_HP_IV + iv[0], &perfectIv);
    ...
    for (i = 0; i < MAX_MON_MOVES; i++) {
        if (moves[i] != MOVE_NONE) SetMonMoveSlot(mon, moves[i], i);
    }
...
static u8 DexNavTryGenerateMonLevel(SpeciesEnum species, u8 environment) {
    ...
    if (Random() % 100 < 4) levelBonus += 10;  // 4% chance of having a +10 level
...
static void DexNavGenerateMoveset(SpeciesEnum species, u8 searchLevel, u8 encounterLevel, u16* moveDst) {
    ...
    if (searchLevel < 5) {
        ...
        if (randVal < SEARCHLEVEL0_MOVECHANCE) genMove = TRUE;
#endif
    } else if (searchLevel < 10) {
#if (SEARCHLEVEL5_MOVECHANCE != 0)
        if (randVal < SEARCHLEVEL5_MOVECHANCE) genMove = TRUE;
#endif
    } else if (searchLevel < 25) {
#if (SEARCHLEVEL10_MOVECHANCE != 0)
        if (randVal < SEARCHLEVEL10_MOVECHANCE) genMove = TRUE;
#endif
    } else if (searchLevel < 50) {
#if (SEARCHLEVEL25_MOVECHANCE != 0)
        if (randVal < SEARCHLEVEL25_MOVECHANCE) genMove = TRUE;
#endif
    } else if (searchLevel < 100) {
#if (SEARCHLEVEL50_MOVECHANCE != 0)
        if (randVal < SEARCHLEVEL50_MOVECHANCE) genMove = TRUE;
#endif
    } else {
#if (SEARCHLEVEL100_MOVECHANCE != 0)
        if (randVal < SEARCHLEVEL100_MOVECHANCE) genMove = TRUE;
#endif
    }
    ...
    for (i = 0; i < MAX_MON_MOVES; i++) moveDst[i] = GetMonData(&gEnemyParty[0], MON_DATA_MOVE1 + i, NULL);
    ...
    if (genMove) {
        ...
        if (numEggMoves != 0) moveDst[0] = eggMoveBuffer[Random() % numEggMoves];
    }
}
...
static u16 DexNavGenerateHeldItem(SpeciesEnum species, u8 searchLevel) {
    ...
    if (item2 == ITEM_NONE && item1 == ITEM_NONE) return ITEM_NONE;

    // if only one entry, 50% chance
    if (item2 == ITEM_NONE && item1 != ITEM_NONE) return (randVal < 50) ? item1 : ITEM_NONE;
...
static u8 DexNavGetAbilityNum(SpeciesEnum species, u8 searchLevel) {
    ...
    if (searchLevel < 5) {
        ...
        if (randVal < SEARCHLEVEL0_ABILITYCHANCE) genAbility = TRUE;
#endif
    } else if (searchLevel < 10) {
#if (SEARCHLEVEL5_ABILITYCHANCE != 0)
        if (randVal < SEARCHLEVEL5_ABILITYCHANCE) genAbility = TRUE;
#endif
    } else if (searchLevel < 25) {
#if (SEARCHLEVEL10_ABILITYCHANCE != 0)
        if (randVal < SEARCHLEVEL10_ABILITYCHANCE) genAbility = TRUE;
#endif
    } else if (searchLevel < 50) {
#if (SEARCHLEVEL25_ABILITYCHANCE != 0)
        if (randVal < SEARCHLEVEL25_ABILITYCHANCE) genAbility = TRUE;
#endif
    } else if (searchLevel < 100) {
#if (SEARCHLEVEL50_ABILITYCHANCE != 0)
        if (randVal < SEARCHLEVEL50_ABILITYCHANCE) genAbility = TRUE;
#endif
    } else {
#if (SEARCHLEVEL100_ABILITYCHANCE != 0)
        if (randVal < SEARCHLEVEL100_ABILITYCHANCE) genAbility = TRUE;
#endif
    }
...
static u8 GetEncounterLevelFromMapData(SpeciesEnum species, u8 environment) {
    ...
    switch (environment) {
        case ROW_LAND_TOP:                                           // grass
            if (landMonsInfo == NULL) return MON_LEVEL_NONEXISTENT;  // Hidden pokemon should only appear on walkable tiles or surf tiles
        ...
        case ROW_HONEY:
            ...
            break;
        case ROW_HIDDEN:
            if (hiddenMonsInfo == NULL || !CanFindHiddenPokemon()) return MON_LEVEL_NONEXISTENT;
    ...
    if (max == 0) return MON_LEVEL_NONEXISTENT;
...
/// GUI ///
///////////
static const struct BgTemplate sDexNavMenuBgTemplates[2] = {{.bg = 0, .charBaseIndex = 0, .mapBaseIndex = 31, .priority = 0},
...
static u8 CreateNoDataIcon(s16 x, s16 y) { return CreateSprite(&sNoDataIconTemplate, x, y, 0); }
...
static bool8 CapturedAllLandMons(u16 headerId) {
    ...
    if (landMonsInfo != NULL) {
        for (i = 0; i < LAND_WILD_COUNT; ++i) {
            ...
            if (species != SPECIES_NONE) {
                if (!GetSetPokedexFlag(SpeciesToNationalPokedexNum(species), FLAG_GET_CAUGHT)) break;
...
static bool8 CapturedAllWaterMons(u16 headerId) {
    ...
    if (waterMonsInfo != NULL) {
        ...
        if (i >= WATER_WILD_COUNT && count > 0) return TRUE;
    } else {
        return TRUE;  // technically, no mon data means you caught them all
    }
...
static bool8 CapturedAllHiddenMons(u16 headerId) {
    ...
    if (hiddenMonsInfo != NULL) {
        ...
        if (i >= HIDDEN_WILD_COUNT && count > 0) return TRUE;
    } else {
        return TRUE;  // technically, no mon data means you caught them all
    }
...
static void DexNavLoadCapturedAllSymbols(void) {
    ...
    if (CapturedAllLandMons(headerId)) CreateSprite(&sCaptureAllMonsSpriteTemplate, 152, 58, 0);

    if (CapturedAllWaterMons(headerId)) CreateSprite(&sCaptureAllMonsSpriteTemplate, 139, 17, 0);

    if (CapturedAllHiddenMons(headerId)) CreateSprite(&sCaptureAllMonsSpriteTemplate, 114, 123, 0);
}
...
static void CB1_DexNavSearchCallback(void) { CB1_InitDexNavSearch(); }
...
static bool8 SpeciesInArray(SpeciesEnum species, u8 section) {
    u32 i;

    for (i = 0; i < LAND_WILD_COUNT; i++) {
        if (sDexNavUiDataPtr->routeSpecies[section][i] == species) return TRUE;
    }
        ...
        switch (i) {
                ...
                if (landMonsInfo != NULL && landMonsInfo->encounterRate != 0) {
                    if (!gSaveBlock2Ptr->encounterRandomizedMode && gSaveBlock2Ptr->encounterRandomizedLegendaryMode) {
                        for (j = 0; j < LAND_WILD_COUNT / 2; j++) {
                            ...
                            if (species != SPECIES_NONE && !SpeciesInArray(species, i)) sDexNavUiDataPtr->routeSpecies[i][index++] = species;
                        }
                    } else {
            ...
            case ROW_WATER:
                ...
                if (waterMonsInfo != NULL && waterMonsInfo->encounterRate != 0 && FlagGet(DEXNAV_WATER_MONS_UNLOCK_FLAG)) {
                    ...
                    if (newEnviorment == DEXNAV_ROWS_COUNT) newEnviorment = i;
                }
                break;
            case ROW_FISHING:
                // Fishing mons
...
static u8 TryDrawIconInSlot(u8 enviorment, u8 num, s16 x, s16 y) {
    SpeciesEnum species = sDexNavUiDataPtr->routeSpecies[enviorment][num];
...
static u16 DexNavGetSpecies(void) {
    SpeciesEnum species = sDexNavUiDataPtr->routeSpecies[sDexNavUiDataPtr->currentEnviorment][sDexNavUiDataPtr->cursorCol + (sDexNavUiDataPtr->cursorRow * 3)];

    if (species >= NUM_SPECIES) return SPECIES_NONE;
...
static const u8 sMoveTypeToOamPaletteNum[NUMBER_OF_MON_TYPES] = {
    ...
    [TYPE_DRAGON] = TYPE_ICON_PAL_NUM_2,  [TYPE_DARK] = TYPE_ICON_PAL_NUM_0,
...
u8 getMonPrice(SpeciesEnum species) {
...
static void PrintCurrentSpeciesInfo(void) {
    SpeciesEnum species = DexNavGetSpecies();
    u32 i, j, x, y;
    ...
    for (i = 0; i < DEXNAV_MAX_SHOWN_ROWS; i++) {
        ...
        if ((numRows < 5 && (i == 0 || i == DEXNAV_MAX_SHOWN_ROWS - 1)) || (numRows == 1 && i != DEFAULT_DEXNAV_ROW))
            ...
            if (i == DEFAULT_DEXNAV_ROW) BlitBitmapToWindow(WINDOW_INFO, gDexnavFieldIcon_Selector, 0, (i * 32), 48, 32);
        }
    }
    ...
    if (species == SPECIES_NONE) type1 = type2 = TYPE_MYSTERY;
    ...
    switch (sDexNavUiDataPtr->currentMessage) {
        ...
        case DEXNAV_MESSAGE_BUY_ALL_NOT_OWNED:
            ...
            for (i = 0; i < NUM_POKEMON_ICONS; i++) {
                ...
                if (species != SPECIES_NONE && !GetSetPokedexFlag(SpeciesToNationalPokedexNum(species), FLAG_GET_CAUGHT)) price += getMonPrice(species);
            }
        ...
        case DEXNAV_MESSAGE_BUY_ALL:
            ...
            for (i = 0; i < NUM_POKEMON_ICONS; i++) {
                species = sDexNavUiDataPtr->routeSpecies[sDexNavUiDataPtr->currentEnviorment][i];
                if (species != SPECIES_NONE) price += getMonPrice(species);
            }
...
static bool8 DexNav_DoGfxSetup(void) {
    ...
    switch (gMain.state) {
        ...
        case 4:
            ...
            break;
        case 5:
            if (DexNav_LoadGraphics() == TRUE) gMain.state++;
            break;
        case 6:
            DexNav_InitWindows();
            sDexNavUiDataPtr->currentEnviorment = ROW_LAND_TOP;
...
static void Task_DexNavWaitFadeIn(u8 taskId) {
    if (!gPaletteFade.active) gTasks[taskId].func = Task_DexNavMain;
}
                ...
                case DEXNAV_MESSAGE_BUY:
                    ...
                    if (gSaveBlock2Ptr->frontier.battlePoints < getMonPrice(species) && !DEXNAV_MON_FREE) {
                        ...
                        if (couldGiveMon < 2) {
                            ...
                            if (VarGet(VAR_DEXNAV_SHINY_FLAG) == 2) gotShiny = TRUE;
                            VarSet(VAR_DEXNAV_SHINY_FLAG, 0);

                            if (!DEXNAV_MON_FREE) gSaveBlock2Ptr->frontier.battlePoints = gSaveBlock2Ptr->frontier.battlePoints - getMonPrice(species);
                            if (gotShiny)
                    ...
                    if (gSaveBlock2Ptr->frontier.battlePoints < price && !DEXNAV_MON_FREE) {
                        ...
                        for (i = 0; i < NUM_POKEMON_ICONS; i++) {
                            ...
                            if (species != SPECIES_NONE && !GetSetPokedexFlag(SpeciesToNationalPokedexNum(species), FLAG_GET_CAUGHT) &&
                                !gSaveBlock2Ptr->nuzlockeCaptures) {
                                couldGiveMon = ScriptGiveMon(species, level, ITEM_NONE, 0, 0, 0);
                                if (couldGiveMon < 2) {
                                    VarSet(VAR_DEXNAV_SHINY_FLAG, 1);
...
bool8 TryFindHiddenPokemon(void) {
    ...
    if ((*stepPtr) == 0 && (Random() % 100 < HIDDEN_MON_SEARCH_RATE)) {
        ...
        if (hiddenMonsInfo == NULL) return FALSE;
        ...
        switch (hiddenMonsInfo->encounterRate) {
            case 0:  // land
                ...
                if (Random() % 100 < HIDDEN_MON_PROBABILTY) {
                    ...
                    if (index == 0xFF) return FALSE;  // no hidden info
        ...
        if (species == SPECIES_NONE) return FALSE;
        ...
        while (1) {
            if (TryStartHiddenMonFieldEffect(sDexNavUiDataPtr->currentEnviorment, 8, 8, TRUE)) break;
            if (++attempts > 20) return FALSE;  // cannot find suitable tile
        }
        ...
        ObjectEventGetLocalIdAndMap(
            &gObjectEvents[gPlayerAvatar.objectEventId], &gFieldEffectArguments[0], &gFieldEffectArguments[1], &gFieldEffectArguments[2]);
...
bool8 DexNavTryMakeShinyMon(void) {
    ...
    for (i = 0; i < shinyRolls; i++) {
        if (Random() % 10000 < shinyRate) return TRUE;
    }
...
void ResetDexNavSearch(void) {
    ...
    if (FlagGet(FLAG_SYS_DEXNAV_SEARCH)) EndDexNavSearch(FindTaskIdByFunc(Task_DexNavSearch));  // moving to new map ends dexnav search
}

void IncrementDexNavChain(void) {
    if (gSaveBlock1Ptr->dexNavChain < DEXNAV_CHAIN_MAX) gSaveBlock1Ptr->dexNavChain++;
}
        ...
        case MAP_GROUP_ROUTES_AND_TOWNS:
            if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(ROUTE102)) {  // Route 102
                if (HasTrainerBeenFought(TRAINER_RICK) &&
                    ...
                    HasTrainerBeenFought(TRAINER_CALVIN_1) && HasTrainerBeenFought(TRAINER_ALLEN) && HasTrainerBeenFought(TRAINER_TIANA))
                    CanFindHiddenMon = TRUE;
            } else if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(ROUTE111)) {  // Route 111 (Desert)
                if (HasTrainerBeenFought(TRAINER_DREW) && HasTrainerBeenFought(TRAINER_BEAU) && HasTrainerBeenFought(TRAINER_HEIDI) &&
                    HasTrainerBeenFought(TRAINER_BECKY) && HasTrainerBeenFought(TRAINER_DUSTY_1) && HasTrainerBeenFought(TRAINER_CELIA) &&
                    HasTrainerBeenFought(TRAINER_BRANDEN) && HasTrainerBeenFought(TRAINER_BRYAN))
                    CanFindHiddenMon = TRUE;
            } else if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(ROUTE109)) {  // Route 109 (Beach)
                if (HasTrainerBeenFought(TRAINER_HUEY) && HasTrainerBeenFought(TRAINER_RICKY_1) && HasTrainerBeenFought(TRAINER_LOLA_1) &&
                    HasTrainerBeenFought(TRAINER_CHANDLER) && HasTrainerBeenFought(TRAINER_HAILEY) && HasTrainerBeenFought(TRAINER_EDMOND) &&
                    HasTrainerBeenFought(TRAINER_TIANA))
                    CanFindHiddenMon = TRUE;
            } else if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(ROUTE113)) {  // Route 113
                if (HasTrainerBeenFought(TRAINER_JAYLEN) && HasTrainerBeenFought(TRAINER_DILLON) && HasTrainerBeenFought(TRAINER_MADELINE_1) &&
                    HasTrainerBeenFought(TRAINER_LAO_1) && HasTrainerBeenFought(TRAINER_LUNG) && HasTrainerBeenFought(TRAINER_TORI_AND_TIA) &&
                    HasTrainerBeenFought(TRAINER_SOPHIE) && HasTrainerBeenFought(TRAINER_COBY) && HasTrainerBeenFought(TRAINER_LAWRENCE) &&
                    HasTrainerBeenFought(TRAINER_WYATT))
                    CanFindHiddenMon = TRUE;
            } else if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(ROUTE114)) {  // Route 114
                if (HasTrainerBeenFought(TRAINER_LENNY) && HasTrainerBeenFought(TRAINER_LUCAS_1) && HasTrainerBeenFought(TRAINER_SHANE) &&
                    HasTrainerBeenFought(TRAINER_NANCY) && HasTrainerBeenFought(TRAINER_STEVE_1) && HasTrainerBeenFought(TRAINER_BERNIE_1) &&
                    HasTrainerBeenFought(TRAINER_CLAUDE) && HasTrainerBeenFought(TRAINER_NOLAN) && HasTrainerBeenFought(TRAINER_TYRA_AND_IVY) &&
                    HasTrainerBeenFought(TRAINER_ANGELINA) && HasTrainerBeenFought(TRAINER_CHARLOTTE) && HasTrainerBeenFought(TRAINER_KAI))
                    CanFindHiddenMon = TRUE;
            } else if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(ROUTE115)) {  // Route 115
                if (HasTrainerBeenFought(TRAINER_TIMOTHY_1) && HasTrainerBeenFought(TRAINER_KOICHI) && HasTrainerBeenFought(TRAINER_NOB_1) &&
                    HasTrainerBeenFought(TRAINER_CYNDY_1) && HasTrainerBeenFought(TRAINER_HECTOR) && HasTrainerBeenFought(TRAINER_KYRA) &&
                    HasTrainerBeenFought(TRAINER_JAIDEN) && HasTrainerBeenFought(TRAINER_ALIX) && HasTrainerBeenFought(TRAINER_HELENE) &&
                    HasTrainerBeenFought(TRAINER_MARLENE))
                    CanFindHiddenMon = TRUE;
            } else if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(ROUTE116)) {  // Route 116
                if (HasTrainerBeenFought(TRAINER_JOEY) && HasTrainerBeenFought(TRAINER_JOSE) && HasTrainerBeenFought(TRAINER_JERRY_1) &&
                    HasTrainerBeenFought(TRAINER_CLARK) && HasTrainerBeenFought(TRAINER_JANICE) && HasTrainerBeenFought(TRAINER_KAREN_1) &&
                    HasTrainerBeenFought(TRAINER_SARAH) && HasTrainerBeenFought(TRAINER_DAWSON) && HasTrainerBeenFought(TRAINER_DEVAN) &&
                    HasTrainerBeenFought(TRAINER_JOHNSON))
                    CanFindHiddenMon = TRUE;
            } else if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(ROUTE117)) {  // Route 117
                if (HasTrainerBeenFought(TRAINER_ISAAC_1) && HasTrainerBeenFought(TRAINER_LYDIA_1) && HasTrainerBeenFought(TRAINER_DYLAN_1) &&
                    HasTrainerBeenFought(TRAINER_MARIA_1) && HasTrainerBeenFought(TRAINER_DEREK) && HasTrainerBeenFought(TRAINER_ANNA_AND_MEG_1) &&
                    HasTrainerBeenFought(TRAINER_MELINA) && HasTrainerBeenFought(TRAINER_BRANDI) && HasTrainerBeenFought(TRAINER_AISHA))
                    CanFindHiddenMon = TRUE;
            } else if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(ROUTE118)) {  // Route 118
                if (HasTrainerBeenFought(TRAINER_ROSE_1) && HasTrainerBeenFought(TRAINER_BARNY) && HasTrainerBeenFought(TRAINER_WADE) &&
                    HasTrainerBeenFought(TRAINER_DALTON_1) && HasTrainerBeenFought(TRAINER_PERRY) && HasTrainerBeenFought(TRAINER_CHESTER) &&
                    HasTrainerBeenFought(TRAINER_DEANDRE))
                    CanFindHiddenMon = TRUE;
            } else if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(ROUTE119)) {  // Route 119
                if (HasTrainerBeenFought(TRAINER_BRENT) && HasTrainerBeenFought(TRAINER_DONALD) && HasTrainerBeenFought(TRAINER_TAYLOR) &&
                    HasTrainerBeenFought(TRAINER_DOUG) && HasTrainerBeenFought(TRAINER_GREG) && HasTrainerBeenFought(TRAINER_KENT) &&
                    HasTrainerBeenFought(TRAINER_JACKSON_1) && HasTrainerBeenFought(TRAINER_CATHERINE_1) && HasTrainerBeenFought(TRAINER_HUGH) &&
                    HasTrainerBeenFought(TRAINER_PHIL) && HasTrainerBeenFought(TRAINER_YASU) && HasTrainerBeenFought(TRAINER_TAKASHI) &&
                    HasTrainerBeenFought(TRAINER_HIDEO) && HasTrainerBeenFought(TRAINER_CHRIS) && HasTrainerBeenFought(TRAINER_FABIAN) &&
                    HasTrainerBeenFought(TRAINER_DAYTON) && HasTrainerBeenFought(TRAINER_RACHEL))
                    CanFindHiddenMon = TRUE;
            } else if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(ROUTE120)) {  // Route 120
                if (HasTrainerBeenFought(TRAINER_COLIN) && HasTrainerBeenFought(TRAINER_ROBERT_1) && HasTrainerBeenFought(TRAINER_LORENZO) &&
                    HasTrainerBeenFought(TRAINER_JENNA) && HasTrainerBeenFought(TRAINER_JEFFREY_1) && HasTrainerBeenFought(TRAINER_JENNIFER) &&
                    HasTrainerBeenFought(TRAINER_CHIP) && HasTrainerBeenFought(TRAINER_CLARISSA) && HasTrainerBeenFought(TRAINER_ANGELICA) &&
                    HasTrainerBeenFought(TRAINER_KEIGO) && HasTrainerBeenFought(TRAINER_RILEY) && HasTrainerBeenFought(TRAINER_LEONEL) &&
                    HasTrainerBeenFought(TRAINER_CALLIE))
                    CanFindHiddenMon = TRUE;
            } else if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(ROUTE121)) {  // Route 121
                if (HasTrainerBeenFought(TRAINER_VANESSA) && HasTrainerBeenFought(TRAINER_WALTER_1) && HasTrainerBeenFought(TRAINER_TAMMY) &&
                    HasTrainerBeenFought(TRAINER_KATE_AND_JOY) && HasTrainerBeenFought(TRAINER_JESSICA_1) && HasTrainerBeenFought(TRAINER_CALE) &&
                    HasTrainerBeenFought(TRAINER_MYLES) && HasTrainerBeenFought(TRAINER_PAT) && HasTrainerBeenFought(TRAINER_MARCEL) &&
                    HasTrainerBeenFought(TRAINER_CRISTIN_1))
                    CanFindHiddenMon = TRUE;
            } else if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(ROUTE123)) {  // Route 123
                if (HasTrainerBeenFought(TRAINER_WENDY) && HasTrainerBeenFought(TRAINER_BRAXTON) && HasTrainerBeenFought(TRAINER_VIOLET) &&
                    HasTrainerBeenFought(TRAINER_CAMERON_1) && HasTrainerBeenFought(TRAINER_JACKI_1) && HasTrainerBeenFought(TRAINER_MIU_AND_YUKI) &&
                    HasTrainerBeenFought(TRAINER_KINDRA) && HasTrainerBeenFought(TRAINER_FREDRICK) && HasTrainerBeenFought(TRAINER_ALBERTO) &&
                    HasTrainerBeenFought(TRAINER_ED) && HasTrainerBeenFought(TRAINER_KAYLEY) && HasTrainerBeenFought(TRAINER_JONAS) &&
                    HasTrainerBeenFought(TRAINER_JAZMYN) && HasTrainerBeenFought(TRAINER_DAVIS) && HasTrainerBeenFought(TRAINER_FERNANDO_1))
                    CanFindHiddenMon = TRUE;
            } else if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(ROUTE127)) {  // Route 127
                if (HasTrainerBeenFought(TRAINER_KOJI_1) && HasTrainerBeenFought(TRAINER_ATHENA) && HasTrainerBeenFought(TRAINER_AIDAN) &&
                    HasTrainerBeenFought(TRAINER_ROGER) && HasTrainerBeenFought(TRAINER_HENRY) && HasTrainerBeenFought(TRAINER_JONAH) &&
                    HasTrainerBeenFought(TRAINER_CAMDEN) && HasTrainerBeenFought(TRAINER_DONNY))
                    CanFindHiddenMon = TRUE;
            } else if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(ROUTE128)) {  // Route 128
                if (HasTrainerBeenFought(TRAINER_CARLEE) && HasTrainerBeenFought(TRAINER_HARRISON) && HasTrainerBeenFought(TRAINER_WAYNE) &&
                    HasTrainerBeenFought(TRAINER_RUBEN) && HasTrainerBeenFought(TRAINER_ALEXA) && HasTrainerBeenFought(TRAINER_KATELYN_1) &&
                    HasTrainerBeenFought(TRAINER_ISAIAH_1))
                    CanFindHiddenMon = TRUE;
            } else if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(ROUTE131)) {  // Route 131
                if (HasTrainerBeenFought(TRAINER_RICHARD) && HasTrainerBeenFought(TRAINER_HERMAN) && HasTrainerBeenFought(TRAINER_SUSIE) &&
                    HasTrainerBeenFought(TRAINER_KARA) && HasTrainerBeenFought(TRAINER_RELI_AND_IAN) && HasTrainerBeenFought(TRAINER_TALIA) &&
                    HasTrainerBeenFought(TRAINER_KEVIN))
                    CanFindHiddenMon = TRUE;
            } else if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(ROUTE132)) {  // Route 132
                if (HasTrainerBeenFought(TRAINER_MAKAYLA) && HasTrainerBeenFought(TRAINER_JONATHAN) && HasTrainerBeenFought(TRAINER_DARCY) &&
                    HasTrainerBeenFought(TRAINER_PAXTON) && HasTrainerBeenFought(TRAINER_KIYO) && HasTrainerBeenFought(TRAINER_RONALD) &&
                    HasTrainerBeenFought(TRAINER_DANA) && HasTrainerBeenFought(TRAINER_GILBERT))
                    CanFindHiddenMon = TRUE;
            } else if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(ROUTE133)) {  // Route 133
                if (HasTrainerBeenFought(TRAINER_CONOR) && HasTrainerBeenFought(TRAINER_MOLLIE) && HasTrainerBeenFought(TRAINER_BECK) &&
                    HasTrainerBeenFought(TRAINER_WARREN) && HasTrainerBeenFought(TRAINER_LINDA) && HasTrainerBeenFought(TRAINER_DEBRA))
                    CanFindHiddenMon = TRUE;
            } else if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(ROUTE134)) {  // Route 134
                if (HasTrainerBeenFought(TRAINER_KELVIN) && HasTrainerBeenFought(TRAINER_MARLEY) && HasTrainerBeenFought(TRAINER_REYNA) &&
                    HasTrainerBeenFought(TRAINER_HUDSON) && HasTrainerBeenFought(TRAINER_HITOSHI) && HasTrainerBeenFought(TRAINER_AARON) &&
                    HasTrainerBeenFought(TRAINER_ALEX) && HasTrainerBeenFought(TRAINER_LAUREL) && HasTrainerBeenFought(TRAINER_JACK))
                    CanFindHiddenMon = TRUE;
            }
            break;
        case MAP_GROUP_DUNGEONS:
            if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(VICTORY_ROAD_1F)) {  // Victory Road 1F
                if (HasTrainerBeenFought(TRAINER_WALLY_VR_1) && HasTrainerBeenFought(TRAINER_EDGAR) && HasTrainerBeenFought(TRAINER_ALBERT) &&
                    HasTrainerBeenFought(TRAINER_HOPE) && HasTrainerBeenFought(TRAINER_QUINCY) && HasTrainerBeenFought(TRAINER_KATELYNN))
                    CanFindHiddenMon = TRUE;
            } else if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(VICTORY_ROAD_B1F)) {  // Victory Road B1F
                if (HasTrainerBeenFought(TRAINER_HALLE) && HasTrainerBeenFought(TRAINER_MITCHELL) && HasTrainerBeenFought(TRAINER_SHANNON) &&
                    HasTrainerBeenFought(TRAINER_SAMUEL))
                    CanFindHiddenMon = TRUE;
            } else if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(VICTORY_ROAD_B2F)) {  // Victory Road B2F
                if (HasTrainerBeenFought(TRAINER_JULIE) && HasTrainerBeenFought(TRAINER_CAROLINE) && HasTrainerBeenFought(TRAINER_OWEN) &&
                    HasTrainerBeenFought(TRAINER_VITO) && HasTrainerBeenFought(TRAINER_FELIX) && HasTrainerBeenFought(TRAINER_DIANNE))
                    CanFindHiddenMon = TRUE;
            }
            break;
    }

    if (!FlagGet(FLAG_SYS_GAME_CLEAR)) CanFindHiddenMon = FALSE;
...
bool8 IsRouteDexnavLocked() {
    ...
    switch (gSaveBlock1Ptr->location.mapNum) {
        case MAP_NUM(ROUTE115):
            if (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(ROUTE115) && !FlagGet(DEXNAV_ENABLE_ROUTE_115_FLAG)) disableDexnav = TRUE;
            break;
    }
    if (IsRouteEncountered(loc, locG) && gSaveBlock2Ptr->nuzlockeCaptures) {
        disableDexnav = TRUE;
...
bool8 MapHasMons(void) {
    ...
    for (i = 0;; i++) {
        ...
        if (wildHeader->mapGroup == 0xFF) break;
...
bool8 canOpenDexnav(void) {
    ...
    for (i = 0; i < DEXNAV_ROWS_COUNT; i++) {
        ...
        switch (i) {
            ...
            case ROW_WATER:
                // Water Encounters
                if (waterMonsInfo != NULL && waterMonsInfo->encounterRate != 0 && FlagGet(DEXNAV_WATER_MONS_UNLOCK_FLAG)) return TRUE;
                break;
            case ROW_FISHING:
                // Fishing Encounters
                if (fishingMonsInfo != NULL && fishingMonsInfo->encounterRate != 0 && FlagGet(DEXNAV_FISHING_MONS_UNLOCK_FLAG)) return TRUE;
                break;
            case ROW_ROCK_SMASH:
                // Rock Smash Encounters
                if (rockSmashMonsInfo != NULL && rockSmashMonsInfo->encounterRate != 0 && FlagGet(DEXNAV_HEADBUTT_MONS_UNLOCK_FLAG)) return TRUE;
                break;
            case ROW_HONEY:
                // Honey Encounters
                if (honeyMonsInfo != NULL && honeyMonsInfo->encounterRate != 0 && FlagGet(DEXNAV_HONEY_MONS_UNLOCK_FLAG)) return TRUE;
                break;
            case ROW_HIDDEN:
                // Hidden Encounters
                if (hiddenMonsInfo != NULL && CanFindHiddenPokemon()) return TRUE;
                break;
        }
    }
