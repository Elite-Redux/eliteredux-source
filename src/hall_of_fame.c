...
u16 getHallofFameSpecies(u8 num) {
    ...
    for (i = 0; i < ARRAY_COUNT(gEvolutionTable[species]); i++)
    {
        ...
        if (gEvolutionTable[species][i].method == EVO_MOVE_MEGA_EVOLUTION)
        {
            ...
            {
                if (GetMonData(&gPlayerParty[num], MON_DATA_MOVE1 + j) == gEvolutionTable[species][i].param)
                    return gEvolutionTable[species][i].targetSpecies;
            }
        }
    }
...
{
    u8 numModes = 0;
    static const u8 gText_WelcomeToHOF[] 		  = _("Elite Redux v3.0 - {STR_VAR_1} Mode{COLOR WHITE}{SHADOW DARK_GRAY}, {STR_VAR_2} Caps\n{COLOR WHITE}{SHADOW DARK_GRAY}{STR_VAR_3}");
