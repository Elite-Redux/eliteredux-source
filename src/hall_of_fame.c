...
static const s16 sHallOfFame_MonFullTeamPositions[PARTY_SIZE][4] = {
    {120, 210, 120, 40}, {326, 220, 56, 40}, {-86, 220, 184, 40}, {120, -62, 120, 88}, {-70, -92, 200, 88}, {310, -92, 40, 88}};

static const s16 sHallOfFame_MonHalfTeamPositions[PARTY_SIZE / 2][4] = {{120, 234, 120, 64}, {326, 244, 56, 64}, {-86, 244, 184, 64}};
...
static const struct HallofFameMon sDummyFameMon = {.tid = 0x3EA03EA, .personality = 0, .species = SPECIES_NONE, .lvl = 0, .nick = {0}};
...
u16 getHallofFameSpecies(u8 num) {
    ...
    for (i = 0; evos[i].method; i++) {
        if (evos[i].method == EVO_MEGA_EVOLUTION || evos[i].method == EVO_PRIMAL_REVERSION) {
            if (evos[i].param == item) return evos[i].targetSpecies;
        }

        if (evos[i].method == EVO_MOVE_MEGA_EVOLUTION) {
            for (j = 0; j < MAX_MON_MOVES; j++) {
                if (GetMonData(&gPlayerParty[num], MON_DATA_MOVE1 + j) == evos[i].param) return evos[i].targetSpecies;
            }
        }
    }
...
static void Task_Hof_InitTeamSaveData(u8 taskId) {
    ...
    if (!gHasHallOfFameRecords) {
        ...
        if (LoadGameSave(SAVE_HALL_OF_FAME) != SAVE_STATUS_OK) memset(gDecompressionBuffer, 0, 0x2000);
    }

    for (i = 0; i < HALL_OF_FAME_MAX_TEAMS; i++, lastSavedTeam++) {
        if (lastSavedTeam->mon[0].species == SPECIES_NONE) break;
    }
...
static void Task_Hof_TrySaveData(u8 taskId) {
    ...
    if (TrySavingData(SAVE_HALL_OF_FAME) == SAVE_STATUS_ERROR && gDamagedSaveSectors != 0) {
        ...
        if (sHofGfxPtr != NULL) FREE_AND_SET_NULL(sHofGfxPtr);
        if (sHofMonPtr != NULL) FREE_AND_SET_NULL(sHofMonPtr);
...
static void Task_Hof_SetMonDisplayTask(u8 taskId) { gTasks[taskId].func = Task_Hof_DisplayMon; }
...
static void Task_Hof_DisplayMon(u8 taskId) {
    ...
    if (currMon->species == SPECIES_EGG) destY += 10;
...
static void Task_Hof_PaletteFadeAndPrintWelcomeText(u8 taskId) {
    ...
    for (i = 0; i < PARTY_SIZE; i++) {
        if (gTasks[taskId].tMonSpriteId(i) != SPRITE_NONE) gSprites[gTasks[taskId].tMonSpriteId(i)].oam.priority = 0;
    }
...
static void Task_Hof_DoConfetti(u8 taskId) {
    if (gTasks[taskId].tFrameCount != 0) {
        ...
        if ((gTasks[taskId].tFrameCount & 3) == 0 && gTasks[taskId].tFrameCount > 110) CreateHofConfettiSprite();
    } else {
        u16 i;
        for (i = 0; i < PARTY_SIZE; i++) {
            if (gTasks[taskId].tMonSpriteId(i) != SPRITE_NONE) gSprites[gTasks[taskId].tMonSpriteId(i)].oam.priority = 1;
        }
        BeginNormalPaletteFade(sHofFadePalettes, 0, 12, 12, RGB(16, 29, 24));
...
static void StartCredits(void) { SetMainCallback2(CB2_StartCreditsSequence); }
...
void CB2_DoHallOfFamePC(void) {
    switch (gMain.state) {
        ...
        case 4:
            ...
            BuildOamBuffer();
            UpdatePaletteFade();
            if (!IsComputerScreenOpenEffectActive()) gMain.state++;
            break;
        case 5: {
            u8 taskId, i;
...
static void Task_HofPC_CopySaveData(u8 taskId) {
    ...
    if (LoadGameSave(SAVE_HALL_OF_FAME) != SAVE_STATUS_OK) {
        ...
        for (i = 0; i < HALL_OF_FAME_MAX_TEAMS; i++, savedTeams++) {
            if (savedTeams->mon[0].species == SPECIES_NONE) break;
        }
...
static void Task_HofPC_DrawSpritesPrintText(u8 taskId) {
    ...
    for (i = 0; i < gTasks[taskId].tCurrTeamNo; i++) savedTeams++;
    ...
    for (i = 0; i < PARTY_SIZE; i++, currMon++) {
        if (currMon->species != 0) gTasks[taskId].tMonNo++;
    }
    ...
    for (i = 0; i < PARTY_SIZE; i++, currMon++) {
        if (currMon->species != 0) {
            ...
            if (currMon->species == SPECIES_EGG) posY += 10;
...
static void Task_HofPC_PrintMonInfo(u8 taskId) {
    ...
    for (i = 0; i < PARTY_SIZE; i++) {
        ...
        if (spriteId != SPRITE_NONE) gSprites[spriteId].oam.priority = 1;
    }
...
static void Task_HofPC_HandleInput(u8 taskId) {
    ...
    if (JOY_NEW(A_BUTTON)) {
        if (gTasks[taskId].tCurrTeamNo != 0)  // prepare another team to view
        {
            ...
            if (gTasks[taskId].tCurrPageNo != 0) gTasks[taskId].tCurrPageNo--;
            gTasks[taskId].func = Task_HofPC_DrawSpritesPrintText;
...
static void Task_HofPC_ExitOnButtonPress(u8 taskId) {
    if (JOY_NEW(A_BUTTON)) gTasks[taskId].func = Task_HofPC_HandlePaletteOnExit;
}
...
static void HallOfFame_PrintWelcomeText(u8 unusedPossiblyWindowId, u8 unused2) {
    ...
    static const u8 gText_WelcomeToHOF[] =
        _("Elite Redux v2.5 - {STR_VAR_1} Mode{COLOR WHITE}{SHADOW DARK_GRAY}, {STR_VAR_2} Caps\n{COLOR WHITE}{SHADOW DARK_GRAY}{STR_VAR_3}");
    ...
    if (FlagGet(FLAG_SYS_LOCKED_MODE)) {
        ...
        FlagClear(FLAG_SYS_LOCKED_MODE);  // This seems to not be saved upon reloading the game, the game probably saves before calling this funcition and
                                          // restarts without saving
    } else {
    ...
    if (gSaveBlock2Ptr->encounterRandomizedMode || gSaveBlock2Ptr->encounterRandomizedLegendaryMode || gSaveBlock2Ptr->typeRandomizedMode ||
        gSaveBlock2Ptr->abilityRandomizedMode || gSaveBlock2Ptr->innaterandomizedMode) {
...
static void HallOfFame_PrintPlayerInfo(u8 unused1, u8 unused2) {
    ...
    if (text[0] == CHAR_0) text[0] = CHAR_SPACE;
    if (text[0] == CHAR_SPACE && text[1] == CHAR_0) text[8] = CHAR_SPACE;
...
static bool8 LoadHofBgs(void) {
    switch (sHofGfxPtr->state) {
        case 0:
            ...
            break;
        case 1:
            if (FreeTempTileDataBuffersIfPossible()) return TRUE;
            break;
        case 2:
...
static void SpriteCB_GetOnScreenAndAnimate(struct Sprite* sprite) {
    if (sprite->x != sprite->tDestinationX || sprite->y != sprite->tDestinationY) {
        if (sprite->x < sprite->tDestinationX) sprite->x += 15;
        if (sprite->x > sprite->tDestinationX) sprite->x -= 15;

        if (sprite->y < sprite->tDestinationY) sprite->y += 10;
        if (sprite->y > sprite->tDestinationY) sprite->y -= 10;
    } else {
...
static void StopDomeConfetti(void) {
    ...
    if ((taskId = FindTaskIdByFunc(Task_DoDomeConfetti)) != TASK_NONE) DestroyTask(taskId);
        ...
        case 1:
            if (tTimer != 0 && tTimer % 3 == 0) {
                ...
                id = ConfettiUtil_AddNew(
                    &sOamData_Confetti, TAG_CONFETTI, TAG_CONFETTI, Random() % DISPLAY_WIDTH, -(Random() % 8), Random() % ARRAY_COUNT(sAnims_Confetti), id);
                if (id != 0xFF) {
                    ConfettiUtil_SetCallback(id, UpdateDomeConfetti);

                    // 1/4 of the confetti move an extra y coord every frame
                    if ((Random() % 4) == 0) ConfettiUtil_SetData(id, CONFETTI_EXTRA_Y, 1);
