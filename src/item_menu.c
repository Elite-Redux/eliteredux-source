...
static const struct MenuAction sItemMenuActions[] = {[ACTION_USE] = {gMenuText_Use, ItemMenu_UseOutOfBattle},
...
static const u8 sContextMenuItems_BattleUse[] = {ACTION_BATTLE_USE, ACTION_CANCEL};

static const u8 sContextMenuItems_Give[] = {ACTION_GIVE, ACTION_CANCEL};

static const u8 sContextMenuItems_Cancel[] = {ACTION_CANCEL};

static const u8 sContextMenuItems_BerryBlenderCrush[] = {ACTION_CONFIRM, ACTION_CHECK_TAG, ACTION_DUMMY, ACTION_CANCEL};

static const u8 sContextMenuItems_Apprentice[] = {ACTION_SHOW, ACTION_CANCEL};

static const u8 sContextMenuItems_FavorLady[] = {ACTION_GIVE_FAVOR_LADY, ACTION_CANCEL};

static const u8 sContextMenuItems_QuizLady[] = {ACTION_CONFIRM_QUIZ_LADY, ACTION_CANCEL};
...
static const u8 sRegisteredSelect_Gfx[] = INCBIN_U8("graphics/interface/select_button2.4bpp");  // tx_registered_items_menu

enum { COLORID_NORMAL, COLORID_POCKET_NAME, COLORID_GRAY_CURSOR, COLORID_UNUSED, COLORID_TMHM_INFO, COLORID_NONE = 0xFF };
...
static const struct WindowTemplate sDefaultBagWindows[] = {[WIN_ITEM_LIST] =
                                                               {
                                                                   .bg = 0,
                                                                   .tilemapLeft = 14,
                                                                   .tilemapTop = 2,
...
void CB2_BagMenuFromStartMenu(void) { GoToBagMenu(ITEMMENULOCATION_FIELD, POCKETS_COUNT, CB2_ReturnToUIMenu); }
...
void CB2_ChooseBerry(void) { GoToBagMenu(ITEMMENULOCATION_BERRY_TREE, BERRIES_POCKET, CB2_ReturnToFieldContinueScript); }

void CB2_ChooseItem(void) { GoToBagMenu(ITEMMENULOCATION_BERRY_TREE, ITEMS_POCKET, CB2_ReturnToFieldContinueScript); }
...
static bool8 SetupBagMenu(void) {
    ...
    switch (gMain.state) {
        ...
        case 14:
            ...
            gTasks[taskId].tListTaskId =
                ListMenuInit(&gMultiuseListMenuTemplate, gBagPosition.scrollPosition[gBagPosition.pocket], gBagPosition.cursorPosition[gBagPosition.pocket]);
...
static void LoadBagItemListBuffers(u8 pocketId) {
    u16 added = 0;
    const u16* itemsList = gItemsForPocket[pocketId];
    u16 itemCount = gItemCountsForPocket[pocketId];
    ...
    if (!gBagMenu->hideCloseBagText) {
        for (int i = 0; i < itemCount; i++) {
            ...
            subBuffer[added].id = i;
            if (++added >= gBagMenu->numItemStacks[pocketId] - 1) break;
        }
        StringCopy(sListBuffer2->name[added], gText_CloseBag);
        subBuffer = sListBuffer1->subBuffers;
        subBuffer[added].name = sListBuffer2->name[added];
        subBuffer[added].id = LIST_CANCEL;
    } else {
        for (int i = 0; i < itemCount; i++) {
            u16 item = itemsList[i];
            FILTER(HasItem(item))
            ...
            subBuffer[added].name = sListBuffer2->name[added];
            subBuffer[added].id = i;
            if (++added >= gBagMenu->numItemStacks[pocketId]) break;
        }
    }
...
static void BagMenu_MoveCursorCallback(s32 itemIndex, bool8 onInit, struct ListMenu* list) {
    ...
    if (gBagMenu->toSwapPos == NOT_SWAPPING) {
        ...
        if (!gBagMenu->inhibitItemDescriptionPrint) PrintItemDescription(itemIndex);
    }
}
...
static void BagMenu_PrintCursor(u8 listTaskId, u8 colorIndex) { BagMenu_PrintCursorAtPos(ListMenuGetYCoordForPrintingArrowCursor(listTaskId), colorIndex); }
...
static void CreatePocketScrollArrowPair(void) {
    if (gBagMenu->pocketScrollArrowsTask == TASK_NONE)
        gBagMenu->pocketScrollArrowsTask =
...
void UpdatePocketItemList(u8 pocketId) {
    ...
    u16 count = gItemCountsForPocket[pocketId];
...
static void UpdatePocketItemLists(void) {
    u8 i;
    for (i = 0; i < POCKETS_COUNT; i++) UpdatePocketItemList(i);
}

void UpdatePocketListPosition(u8 pocketId) {
    SetCursorWithinListBounds(
        &gBagPosition.scrollPosition[pocketId], &gBagPosition.cursorPosition[pocketId], gBagMenu->numShownItems[pocketId], gBagMenu->numItemStacks[pocketId]);
}

static void InitPocketListPositions(void) {
    u8 i;
    for (i = 0; i < POCKETS_COUNT; i++) UpdatePocketListPosition(i);
}
...
static void InitPocketScrollPositions(void) {
    ...
    for (i = 0; i < POCKETS_COUNT; i++)
        SetCursorScrollWithinListBounds(
            &gBagPosition.scrollPosition[i], &gBagPosition.cursorPosition[i], gBagMenu->numShownItems[i], gBagMenu->numItemStacks[i], MAX_ITEMS_SHOWN);
}

u8 GetItemListPosition(u8 pocketId) { return gBagPosition.scrollPosition[pocketId] + gBagPosition.cursorPosition[pocketId]; }
...
static void AddItemQuantityWindow(u8 windowType) { PrintItemQuantity(BagMenu_AddWindow(windowType), 1); }
...
static u8 GetSwitchBagPocketDirection(void) {
    u8 LRKeys;
    if (gBagMenu->pocketSwitchDisabled) return SWITCH_POCKET_NONE;
...
static void ChangeBagPocketId(u8* bagPocketId, s8 deltaBagPocketId) {
    if ((deltaBagPocketId == MENU_CURSOR_DELTA_RIGHT && *bagPocketId == BATTLE_POCKET) ||
        (deltaBagPocketId == MENU_CURSOR_DELTA_LEFT && *bagPocketId == BERRIES_POCKET))
...
static void Task_SwitchBagPocket(u8 taskId) {
    ...
    switch (tPocketSwitchState) {
        case 0:
            ...
            if (tPocketSwitchTimer == 16) tPocketSwitchState++;
            break;
        case 1:
            ChangeBagPocketId(&gBagPosition.pocket, tPocketSwitchDir);
            LoadBagItemListBuffers(gBagPosition.pocket);
                ...
                switch (gBagPosition.pocket) {
                    ...
                    case BATTLE_POCKET:
                        ...
                        memcpy(&gBagMenu->contextMenuItemsBuffer,
                               ...
                               sizeof(sContextMenuItems_ItemsPocket));  // Are these memcpys needed?
                        break;
                    case TMHM_POCKET:
                    ...
                    case KEYITEMS_POCKET:
                        ...
                        if (sRegisterSubMenu == FALSE)  // tx_registered_items_menu
                        {
                            ...
                            if (gSaveBlock1Ptr->registeredItemSelect == gSpecialVar_ItemId) gBagMenu->contextMenuItemsBuffer[1] = ACTION_DESELECT;
...
static bool8 IsValidContextMenuPos(s8 cursorPos) {
    ...
    if (cursorPos > gBagMenu->contextMenuNumItems) return FALSE;
    if (gBagMenu->contextMenuItemsPtr[cursorPos] == ACTION_DUMMY) return FALSE;
    return TRUE;
}
...
static void PrintThereIsNoPokemon(u8 taskId) { DisplayItemMessage(taskId, 1, gText_NoPokemon, HandleErrorMessage); }
...
void CB2_ReturnToBagMenuPocket(void) { GoToBagMenu(ITEMMENULOCATION_LAST, POCKETS_COUNT, NULL); }
...
bool8 UseRegisteredKeyItemOnField(u8 button) {
    ...
    if (InUnionRoom() == TRUE || InBattlePyramid() || InBattlePike() || InMultiPartnerRoom() == TRUE) return FALSE;
...
static bool8 IsWallysBag(void) {
    if (gBagPosition.location == ITEMMENULOCATION_WALLY) return TRUE;
    return FALSE;
}
...
// Unused
static u8 BagMenu_GetWindowId(u8 windowType) { return gBagMenu->windowIds[windowType]; }
...
static u8 AddItemMessageWindow(u8 windowType) {
    u8* windowId = &gBagMenu->windowIds[windowType];
    if (*windowId == WINDOW_NONE) *windowId = AddWindow(&sContextMenuWindowTemplates[windowType]);
    return *windowId;
}
...
static void PrintTMHMMoveData(u16 itemId) {
    ...
    if (itemId == ITEM_NONE) {
        for (i = 0; i < 4; i++) BagMenu_Print(WIN_TMHM_INFO, 1, gText_ThreeDashes, 7, i * 12, 0, 0, TEXT_SPEED_FF, COLORID_TMHM_INFO);
