...
static const u8 sRemovedDefaultPokeball[] = _("Random balls will be used!{PAUSE_UNTIL_PRESS}");

void ItemUseOutOfBattle_PokeBall(u8 taskId) {
    if (gSaveBlock1Ptr->defaultPokeball == gSpecialVar_ItemId) {
        gSaveBlock1Ptr->defaultPokeball = ITEM_NONE;
        DisplayItemMessage(taskId, 1, sRemovedDefaultPokeball, CloseItemMessage);
    } else {
...
static bool32 CanFish(void) {
    ...
    if (TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_UNDERWATER)) return FALSE;

    if (!TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_SURFING)) {
        if (IsPlayerFacingSurfableFishableWater()) return TRUE;
    } else {
        if (MetatileBehavior_IsSurfableWaterOrUnderwater(tileBehavior) && !MapGridIsImpassableAt(x, y)) return TRUE;
        if (MetatileBehavior_8089510(tileBehavior) == TRUE) return TRUE;
    }
        ...
        if (tItemfinderBeeps == 4) {
            ...
            if (playerDirToItem != DIR_NONE) {
                ...
                for (i = 0; i < ARRAY_COUNT(sClockwiseDirections); i++) {
                    if (playerDir == sClockwiseDirections[i]) tFacingDir = (i + 1) & 3;
                }
...
static bool8 ItemfinderCheckForHiddenItems(const struct MapEvents *events, u8 taskId) {
    ...
    for (i = 0; i < events->bgEventCount; i++) {
        ...
        if (events->bgEvents[i].kind == BG_EVENT_HIDDEN_ITEM && !FlagGet(events->bgEvents[i].bgUnion.hiddenItem.hiddenItemId + FLAG_HIDDEN_ITEMS_START)) {
            ...
            if ((u16)(distanceX + 7) < 15 && (distanceY >= -5) && (distanceY < 6)) SetDistanceOfClosestHiddenItem(taskId, distanceX, distanceY);
        }
    }
    ...
    for (x = playerX - 7; x <= playerX + 7; x++) {
        for (y = playerY - 5; y <= playerY + 5; y++) {
            if (var1 > x || x >= width || var2 > y || y >= height) {
                ...
                if (conn && IsHiddenItemPresentInConnection(conn, x, y) == TRUE) SetDistanceOfClosestHiddenItem(taskId, x - playerX, y - playerY);
            }
        }
    }
}
...
static u8 GetDirectionToHiddenItem(s16 itemDistanceX, s16 itemDistanceY) {
    ...
    if (itemDistanceX == 0 && itemDistanceY == 0) return DIR_NONE;  // player is standing on the item.
...
static void Task_StandingOnHiddenItem(u8 taskId) {
    ...
    if (ObjectEventCheckHeldMovementStatus(&gObjectEvents[GetObjectEventIdByLocalIdAndMap(OBJ_EVENT_ID_PLAYER, 0, 0)]) == TRUE || tItemFound == FALSE) {
        ...
        if (tCounter == 4) DisplayItemMessageOnField(taskId, gText_ItemFinderOnTop, Task_CloseItemfinderMessage);
    }
}
...
static void CB2_OpenPokeblockFromBag(void) { OpenPokeblockCase(PBLOCK_CASE_FIELD, CB2_ReturnToBagMenuPocket); }
...
static void UseTMHMYesNo(u8 taskId) { BagMenu_YesNo(taskId, ITEMWIN_YESNO_HIGH, &sUseTMHMYesNoFuncTable); }
...
static void ItemUseOnFieldCB_PokeVial(u8 taskId) {
    PlaySE(SE_USE_ITEM);
    HealPlayerParty();
    if (GetBoxMonDataAt(TOTAL_BOXES_COUNT - 1, IN_BOX_COUNT - 1, MON_DATA_SPEED_IV) != 1) VarSet(VAR_POKE_VIAL_CHARGES, VarGet(VAR_POKE_VIAL_CHARGES) - 1);
...
void ItemUseOutOfBattle_CannotUse(u8 taskId) { DisplayDadsAdviceCannotUseItemMessage(taskId, gTasks[taskId].tUsingRegisteredKeyItem); }
