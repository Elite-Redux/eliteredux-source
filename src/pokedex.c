...
enum { SEARCH_NAME, SEARCH_COLOR, SEARCH_TYPE_LEFT, SEARCH_TYPE_RIGHT, SEARCH_ORDER, SEARCH_MODE, SEARCH_OK, SEARCH_COUNT };
...
static const struct SpritePalette sStatBarSpritePal[] =  //{sStatBarPalette, TAG_STAT_BAR};
    {{sStatBarPalette, TAG_STAT_BAR}, {sStatBarPalette, TAG_STAT_BAR_BG}, {0}};

// const rom data
#include "data/pokemon/pokedex_orders.h"

static const struct OamData sOamData_ScrollBar = {.y = DISPLAY_HEIGHT,
                                                  .affineMode = ST_OAM_AFFINE_OFF,
                                                  .objMode = ST_OAM_OBJ_NORMAL,
                                                  .mosaic = 0,
                                                  .bpp = ST_OAM_4BPP,
...
static const struct OamData sOamData_ScrollArrow = {.y = DISPLAY_HEIGHT,
                                                    .affineMode = ST_OAM_AFFINE_OFF,
                                                    .objMode = ST_OAM_OBJ_NORMAL,
                                                    .mosaic = 0,
                                                    .bpp = ST_OAM_4BPP,
...
static const struct OamData sOamData_InterfaceText = {.y = DISPLAY_HEIGHT,
                                                      .affineMode = ST_OAM_AFFINE_OFF,
                                                      .objMode = ST_OAM_OBJ_NORMAL,
                                                      .mosaic = 0,
                                                      .bpp = ST_OAM_4BPP,
...
static const struct OamData sOamData_RotatingPokeBall = {.y = DISPLAY_HEIGHT,
                                                         .affineMode = ST_OAM_AFFINE_OFF,
...
static const struct OamData sOamData_SeenOwnText = {.y = DISPLAY_HEIGHT,
                                                    .affineMode = ST_OAM_AFFINE_OFF,
                                                    .objMode = ST_OAM_OBJ_NORMAL,
                                                    .mosaic = 0,
                                                    .bpp = ST_OAM_4BPP,
...
static const struct OamData sOamData_Dex8x16 = {.y = DISPLAY_HEIGHT,
                                                .affineMode = ST_OAM_AFFINE_OFF,
                                                .objMode = ST_OAM_OBJ_NORMAL,
                                                .mosaic = 0,
                                                .bpp = ST_OAM_4BPP,
...
static const union AnimCmd *const sSpriteAnimTable_HoennNationalText[] = {sSpriteAnim_HoennText, sSpriteAnim_NationalText};

static const union AnimCmd *const sSpriteAnimTable_HoennSeenOwnNumber[] = {sSpriteAnim_HoennSeenOwnDigit0,
...
static const union AnimCmd *const sSpriteAnimTable_NationalSeenOwnNumber[] = {sSpriteAnim_NationalSeenOwnDigit0,
...
static const struct CompressedSpriteSheet sInterfaceSpriteSheet[] = {
    {gPokedexInterface_Gfx, 0x2000, TAG_DEX_INTERFACE}, {gPokedexInterface_DECA_Gfx, 0x2000, TAG_DEX_INTERFACE}, {0}};

static const struct SpritePalette sInterfaceSpritePalette[] = {{gPokedexDefault_Pal, TAG_DEX_INTERFACE}, {0}};
...
static const struct SearchOptionText sDexSearchTypeOptions[NUMBER_OF_MON_TYPES + 1] =  // + 2 for "None" and terminator, - 1 for Mystery
    {
        ...
        {gText_DexEmptyString, gTypeNames[TYPE_FLYING]}, {gText_DexEmptyString, gTypeNames[TYPE_POISON]},   {gText_DexEmptyString, gTypeNames[TYPE_GROUND]},
        {gText_DexEmptyString, gTypeNames[TYPE_ROCK]},   {gText_DexEmptyString, gTypeNames[TYPE_BUG]},      {gText_DexEmptyString, gTypeNames[TYPE_GHOST]},
        {gText_DexEmptyString, gTypeNames[TYPE_STEEL]},  {gText_DexEmptyString, gTypeNames[TYPE_FIRE]},     {gText_DexEmptyString, gTypeNames[TYPE_WATER]},
        {gText_DexEmptyString, gTypeNames[TYPE_GRASS]},  {gText_DexEmptyString, gTypeNames[TYPE_ELECTRIC]}, {gText_DexEmptyString, gTypeNames[TYPE_PSYCHIC]},
        {gText_DexEmptyString, gTypeNames[TYPE_ICE]},    {gText_DexEmptyString, gTypeNames[TYPE_DRAGON]},   {gText_DexEmptyString, gTypeNames[TYPE_DARK]},
        {gText_DexEmptyString, gTypeNames[TYPE_FAIRY]},  {gText_DexEmptyString, gTypeNames[TYPE_STELLAR]},  {},
};
...
static const u8 sDexSearchTypeIds[NUMBER_OF_MON_TYPES] = {
    TYPE_NONE, TYPE_NORMAL, TYPE_FIGHTING, TYPE_FLYING,   TYPE_POISON,  TYPE_GROUND, TYPE_ROCK,   TYPE_BUG,  TYPE_GHOST, TYPE_STEEL,
    TYPE_FIRE, TYPE_WATER,  TYPE_GRASS,    TYPE_ELECTRIC, TYPE_PSYCHIC, TYPE_ICE,    TYPE_DRAGON, TYPE_DARK, TYPE_FAIRY, TYPE_STELLAR,
};
...
static void ResetPokedexView(struct PokedexView *pokedexView) {
    ...
    for (i = 0; i < MAX_MONS_ON_SCREEN; i++) pokedexView->monSpriteIds[i] = 0xFFFF;
    ...
    for (i = 0; i < ARRAY_COUNT(pokedexView->unkArr1); i++) pokedexView->unkArr1[i] = 0;
...
void Task_OpenPokedexMainPage(u8 taskId) {
    ...
    if (LoadPokedexListPage(PAGE_MAIN)) gTasks[taskId].func = Task_HandlePokedexInput;
}
...
static void Task_ClosePokedex(u8 taskId) {
    if (!gPaletteFade.active) {
        ...
        if (!IsNationalPokedexEnabled()) gSaveBlock2Ptr->pokedex.mode = DEX_MODE_HOENN;
...
static void Task_OpenSearchResults(u8 taskId) {
    ...
    if (LoadPokedexListPage(PAGE_SEARCH_RESULTS)) gTasks[taskId].func = Task_HandleSearchResultsInput;
}
...
static bool8 LoadPokedexListPage(u8 page) {
    switch (gMain.state) {
        case 0:
        default:
            if (gPaletteFade.active) return 0;
            SetVBlankCallback(NULL);
...
static void FreeWindowAndBgBuffers(void) {
    ...
    if (tilemapBuffer) Free(tilemapBuffer);
        ...
        case ORDER_NUMERICAL:
            if (temp_isHoennDex) {
                for (i = 0; i < temp_dexCount; i++) {
                    ...
                    if (sPokedexView->pokedexList[i].seen) sPokedexView->pokemonListCount = i + 1;
                }
            } else {
                s16 r5, r10;
                for (i = 0, r5 = 0, r10 = 0; i < temp_dexCount; i++) {
                    temp_dexNum = i + 1;
                    if (GetSetPokedexFlag(temp_dexNum, FLAG_GET_SEEN)) r10 = 1;
                    if (r10) {
                        sPokedexView->pokedexList[r5].dexNum = temp_dexNum;
                        sPokedexView->pokedexList[r5].seen = GetSetPokedexFlag(temp_dexNum, FLAG_GET_SEEN);
                        sPokedexView->pokedexList[r5].owned = GetSetPokedexFlag(temp_dexNum, FLAG_GET_CAUGHT);
                        if (sPokedexView->pokedexList[r5].seen) sPokedexView->pokemonListCount = r5 + 1;
                        r5++;
                    }
                }
            }
...
static void ClearMonListEntry(u8 x, u8 y, u16 unused) { FillWindowPixelRect(0, PIXEL_FILL(0), x * 8, y * 8, 0x60, 16); }
...
static void CreateMonSpritesAtPos(u16 selectedMon, u16 ignored) {
    ...
    for (i = 0; i < MAX_MONS_ON_SCREEN; i++) sPokedexView->monSpriteIds[i] = 0xFFFF;
        ...
        switch (direction) {
            case 1:  // Up
                for (i = 0; i < MAX_MONS_ON_SCREEN; i++) {
                    if (sPokedexView->monSpriteIds[i] != 0xFFFF) gSprites[sPokedexView->monSpriteIds[i]].data[5] += monMoveIncrement;
                }
...
static u16 TryDoPokedexScroll(u16 selectedMon, u16 ignored) {
    ...
    if ((JOY_HELD(DPAD_UP)) && (selectedMon > 0)) {
        ...
        for (i = 0; i < 7; i++) selectedMon = GetNextPosition(1, selectedMon, 0, sPokedexView->pokemonListCount - 1);
        ...
        startingPos = selectedMon;
        for (i = 0; i < 7; i++) selectedMon = GetNextPosition(0, selectedMon, 0, sPokedexView->pokemonListCount - 1);
        sPokedexView->pokeBallRotation += 16 * (selectedMon - startingPos);
        ClearMonSprites();
        CreateMonSpritesAtPos(selectedMon, 0xE);
        sPokedexView->justScrolled = TRUE;  // HGSS_Ui
    ...
    if (sPokedexView->scrollSpeed < 12) sPokedexView->scrollSpeed++;
    return selectedMon;
}
...
static void UpdateSelectedMonSpriteId(void) {
    ...
    for (i = 0; i < MAX_MONS_ON_SCREEN; i++) {
        ...
        if (gSprites[spriteId].x2 == 0 && gSprites[spriteId].y2 == 0 && spriteId != 0xFFFF) sPokedexView->selectedMonSpriteId = spriteId;
    }
}
...
static void CreateInterfaceSprites(u8 page) {
    ...
    if (!IsNationalPokedexEnabled() && page == PAGE_MAIN) {
        ...
        spriteId = CreateSprite(
            &sNationalDexSeenOwnNumberSpriteTemplate, LIST_RIGHT_SIDE_TEXT_X + LIST_RIGHT_SIDE_TEXT_X_OFFSET, 45 - LIST_RIGHT_SIDE_TEXT_Y_OFFSET, 1);
        ...
        spriteId = CreateSprite(
            &sNationalDexSeenOwnNumberSpriteTemplate, LIST_RIGHT_SIDE_TEXT_X + LIST_RIGHT_SIDE_TEXT_X_OFFSET + 8, 45 - LIST_RIGHT_SIDE_TEXT_Y_OFFSET, 1);
        ...
        // Hoenn seen value - 1s
        spriteId = CreateSprite(
            &sNationalDexSeenOwnNumberSpriteTemplate, LIST_RIGHT_SIDE_TEXT_X + LIST_RIGHT_SIDE_TEXT_X_OFFSET + 16, 45 - LIST_RIGHT_SIDE_TEXT_Y_OFFSET, 1);
        digitNum = (sPokedexView->seenCount % 100) % 10;
        StartSpriteAnim(&gSprites[spriteId], digitNum);
        ...
        drawNextDigit = FALSE;
        spriteId = CreateSprite(
            &sNationalDexSeenOwnNumberSpriteTemplate, LIST_RIGHT_SIDE_TEXT_X + LIST_RIGHT_SIDE_TEXT_X_OFFSET, 55 - LIST_RIGHT_SIDE_TEXT_Y_OFFSET, 1);
        ...
        spriteId = CreateSprite(
            &sNationalDexSeenOwnNumberSpriteTemplate, LIST_RIGHT_SIDE_TEXT_X + LIST_RIGHT_SIDE_TEXT_X_OFFSET + 8, 55 - LIST_RIGHT_SIDE_TEXT_Y_OFFSET, 1);
        ...
        spriteId = CreateSprite(
            &sNationalDexSeenOwnNumberSpriteTemplate, LIST_RIGHT_SIDE_TEXT_X + LIST_RIGHT_SIDE_TEXT_X_OFFSET + 16, 55 - LIST_RIGHT_SIDE_TEXT_Y_OFFSET, 1);
        digitNum = (sPokedexView->ownCount % 100) % 10;
        StartSpriteAnim(&gSprites[spriteId], digitNum);
        ...
        drawNextDigit = FALSE;
        spriteId = CreateSprite(
            &sNationalDexSeenOwnNumberSpriteTemplate, LIST_RIGHT_SIDE_TEXT_X + LIST_RIGHT_SIDE_TEXT_X_OFFSET, 78 - LIST_RIGHT_SIDE_TEXT_Y_OFFSET, 1);
        digitNum = sPokedexView->seenCount / 100;
        StartSpriteAnim(&gSprites[spriteId], digitNum);
        if (digitNum != 0)
            drawNextDigit = TRUE;
        ...
        spriteId = CreateSprite(
            &sNationalDexSeenOwnNumberSpriteTemplate, LIST_RIGHT_SIDE_TEXT_X + LIST_RIGHT_SIDE_TEXT_X_OFFSET + 8, 78 - LIST_RIGHT_SIDE_TEXT_Y_OFFSET, 1);
        digitNum = (sPokedexView->seenCount % 100) / 10;
        if (digitNum != 0 || drawNextDigit)
            StartSpriteAnim(&gSprites[spriteId], digitNum);
        else
        ...
        spriteId = CreateSprite(
            &sNationalDexSeenOwnNumberSpriteTemplate, LIST_RIGHT_SIDE_TEXT_X + LIST_RIGHT_SIDE_TEXT_X_OFFSET + 16, 78 - LIST_RIGHT_SIDE_TEXT_Y_OFFSET, 1);
        digitNum = (sPokedexView->seenCount % 100) % 10;
        StartSpriteAnim(&gSprites[spriteId], digitNum);
        ...
        drawNextDigit = FALSE;
        spriteId = CreateSprite(
            &sNationalDexSeenOwnNumberSpriteTemplate, LIST_RIGHT_SIDE_TEXT_X + LIST_RIGHT_SIDE_TEXT_X_OFFSET, 88 - LIST_RIGHT_SIDE_TEXT_Y_OFFSET, 1);
        digitNum = sPokedexView->ownCount / 100;
        StartSpriteAnim(&gSprites[spriteId], digitNum);
        if (digitNum != 0)
            drawNextDigit = TRUE;
        ...
        spriteId = CreateSprite(
            &sNationalDexSeenOwnNumberSpriteTemplate, LIST_RIGHT_SIDE_TEXT_X + LIST_RIGHT_SIDE_TEXT_X_OFFSET + 8, 88 - LIST_RIGHT_SIDE_TEXT_Y_OFFSET, 1);
        digitNum = (sPokedexView->ownCount % 100) / 10;
        if (digitNum != 0 || drawNextDigit)
            StartSpriteAnim(&gSprites[spriteId], digitNum);
        else
        ...
        spriteId = CreateSprite(
            &sNationalDexSeenOwnNumberSpriteTemplate, LIST_RIGHT_SIDE_TEXT_X + LIST_RIGHT_SIDE_TEXT_X_OFFSET + 16, 88 - LIST_RIGHT_SIDE_TEXT_Y_OFFSET, 1);
        digitNum = (sPokedexView->ownCount % 100) % 10;
        StartSpriteAnim(&gSprites[spriteId], digitNum);
    }
...
static void SpriteCB_SeenOwnInfo(struct Sprite *sprite) {
    if (sPokedexView->currentPage != PAGE_MAIN && sPokedexView->currentPage != PAGE_SEARCH_RESULTS) DestroySprite(sprite);
}
...
void SpriteCB_MoveMonForInfoScreen(struct Sprite *sprite) {
    ...
    if (sprite->x != 48 || sprite->y != 56) {
        if (sprite->x > 48) sprite->x -= 4;
        if (sprite->x < 48) sprite->x = 48;

        if (sprite->y > 56) sprite->y -= 4;
        if (sprite->y < 56) sprite->y = 56;
    } else {
...
static void Task_WaitForAreaScreenInput(u8 taskId) {
    ...
    if (sPokedexView->screenSwitchState != 0) gTasks[taskId].func = Task_SwitchScreensFromAreaScreen;
}
...
static void Task_LoadCryScreen(u8 taskId) {
    switch (gMain.state) {
        ...
        case 7: {
            ...
            cryMeter.xPos = 18;
            cryMeter.yPos = 3;
            if (LoadCryMeter(&cryMeter, 3)) gMain.state++;
...
static void HighlightScreenSelectBarItem(u8 selectedScreen, u16 unused)  // HGSS_Ui
{
    ...
    for (i = 0; i < SCREEN_COUNT; i++) {
        ...
        do {
            newPalette = 0x4000;
            if (i == selectedScreen) newPalette = 0x2000;
        } while (0);
...
static void SpriteCB_SlideCaughtMonToCenter(struct Sprite *sprite) {
    if (sprite->x < 0x78) sprite->x += 2;
    if (sprite->x > 0x78) sprite->x -= 2;

    if (sprite->y < 0x50) sprite->y += 1;
    if (sprite->y > 0x50) sprite->y -= 1;
}
...
// Type Icon
static void SetSpriteInvisibility(u8 spriteArrayId, bool8 invisible) { gSprites[sPokedexView->typeIconSpriteIds[spriteArrayId]].invisible = invisible; }
...
static void CreateTypeIconSprites(void) {
    ...
    for (i = 0; i < 2; i++) {
        if (sPokedexView->typeIconSpriteIds[i] == 0xFF) sPokedexView->typeIconSpriteIds[i] = CreateSprite(&sSpriteTemplate_MoveTypes, 10, 10, 2);
...
s8 GetSetPokedexFlag(u16 nationalDexNo, u8 caseID) {
    ...
    switch (caseID) {
        ...
        case FLAG_GET_CAUGHT:
            ...
            break;
        case FLAG_SET_SEEN:
            if (nationalDexNo < POKEMON_SLOTS_NUMBER) gSaveBlock1Ptr->dexSeen[index] |= mask;
            break;
        case FLAG_SET_CAUGHT:
            if (nationalDexNo < POKEMON_SLOTS_NUMBER) gSaveBlock1Ptr->dexCaught[index] |= mask;
            break;
    }
...
bool16 HasAllHoennMons(void) {
    ...
    for (i = 0; i < HOENN_DEX_COUNT - 2; i++) {
        if (!GetSetPokedexFlag(HoennToNationalOrder(i + 1), FLAG_GET_CAUGHT)) return FALSE;
    }
    return TRUE;
}
...
bool8 HasAllKantoMons(void) {
    ...
    for (i = 0; i < KANTO_DEX_COUNT - 1; i++) {
        if (!GetSetPokedexFlag(i + 1, FLAG_GET_CAUGHT)) return FALSE;
    }
    return TRUE;
}
...
static u8 PrintCryScreenSpeciesName(u8 windowId, u16 num, u8 left, u8 top) {
    ...
    for (i = 0; i < ARRAY_COUNT(str); i++) str[i] = EOS;
    num = NationalPokedexNumToSpecies(num);
    switch (num) {
        default:
            for (i = 0; gSpecies[num].name[i] != EOS && i < POKEMON_NAME_LENGTH; i++) str[i] = gSpecies[num].name[i];
            break;
        case 0:
            for (i = 0; i < 5; i++) str[i] = CHAR_HYPHEN;
            break;
    }
    PrintInfoSubMenuText(windowId, str, left, top);
    return i;
...
static void UnusedPrintMonName(u8 windowId, const u8 *name, u8 left, u8 top) {
    ...
    for (i = 0; i < ARRAY_COUNT(str); i++) str[i] = CHAR_SPACE;
    for (nameLength = 0; name[nameLength] != CHAR_SPACE && nameLength < ARRAY_COUNT(str); nameLength++);
    for (i = 0; i < nameLength; i++) str[ARRAY_COUNT(str) - nameLength + i] = name[i];
#ifdef UBFIX
...
static u16 CreateSizeScreenTrainerPic(u16 species, s16 x, s16 y, s8 paletteSlot) { return CreateTrainerPicSprite(species, TRUE, x, y, paletteSlot, 0xFFFF); }
...
bool8 SpeciesCanLearnLvlUpMove(u16 species, u16 move)  // Move search PokedexPlus HGSS_Ui
{
    ...
    for (j = 0; gSpecies[species].levelUpMoves[j].move; j++) {
        if (move == (gSpecies[species].levelUpMoves[j].move & LEVEL_UP_MOVE_ID)) {
            return TRUE;
        }
    }
#else
    for (j = 0; gSpecies[species].levelUpMoves[j]; j++) {
        if (move == (gSpecies[species].levelUpMoves[j] & LEVEL_UP_MOVE_ID)) {
            return TRUE;
        }
    }
#endif
...
static void ClearSearchMenuRect(u32 x, u32 y, u32 width, u32 height) { FillWindowPixelRect(0, PIXEL_FILL(0), x, y, width, height); }
...
static void DrawOrEraseSearchParameterBox(bool8 erase) {
    ...
    if (!erase) {
        *(ptr + 0x11) = 0xC0B;
        for (i = 0x12; i < 0x1F; i++) *(ptr + i) = 0x80D;
        for (j = 1; j < 13; j++) {
            *(ptr + 0x11 + j * 32) = 0x40A;
            for (i = 0x12; i < 0x1F; i++) *(ptr + j * 32 + i) = 2;
        }
        *(ptr + 0x1B1) = 0x40B;
        for (i = 0x12; i < 0x1F; i++) *(ptr + 0x1A0 + i) = 0xD;
    } else {
        for (j = 0; j < 14; j++) {
            for (i = 0x11; i < 0x1E; i++) {
                *(ptr + j * 32 + i) = 0x4F;
...
static void PrintSearchParameterText(u8 taskId) {
    ...
    for (i = 0, j = *scrollOffset; i < MAX_SEARCH_PARAM_ON_SCREEN && texts[j].title != NULL; i++, j++) PrintSearchParameterTitle(i, texts[j].title);
...
static void EraseSelectorArrow(u32 y) { ClearSearchMenuRect(144, y * 16 + 8, 8, 16); }

static void PrintSelectorArrow(u32 y) { PrintSearchText(gText_SelectorArrow, 144, y * 16 + 9); }

static void PrintSearchParameterTitle(u32 y, const u8 *str) { PrintSearchText(str, 152, y * 16 + 9); }

static void ClearSearchParameterBoxText(void) { ClearSearchMenuRect(144, 8, 96, 96); }
...
static void Task_LoadStatsScreen(u8 taskId) {
    switch (gMain.state) {
        ...
        case 5:
            if (gTasks[taskId].data[1] == 0) {
                ...
                gTasks[taskId].data[6] =
                    CreateMonIcon(NationalPokedexNumToSpecies(sPokedexListItem->dexNum), SpriteCB_MonIcon, 18, 31, 4, 0);  // Create pokemon sprite
        ...
        case 8:
            ...
            break;
        case 9:
            if (!gPaletteFade.active) gMain.state++;
            break;
        case 10:
            gMain.state++;
            break;
...
static void PrintStatsScreen_MoveNameAndInfo(u8 taskId) {
    ...
    if (selected < numEggMoves) {
        ...
        level = gSpecies[species].levelUpMoves[(selected - numEggMoves)].level;
...
// Physical/Special Split from BE
        ...
        if (contest_effectValue != 0xFF) contest_effectValue /= 10;
        for (contest_i = 0; contest_i < 8; contest_i++) {
            if (contest_effectValue != 0xFF && contest_i < contest_effectValue) contest_appeal += 1;
        }
        ...
        if (contest_effectValue != 0xFF) contest_effectValue /= 10;
        for (contest_i = 0; contest_i < 8; contest_i++) {
            if (contest_effectValue != 0xFF && contest_i < contest_effectValue) contest_jam += 1;
        }
...
static void PrintStatsScreen_Left(u8 taskId) {
    ...
    u16 ability0;
    u8 differentEVs = 0;
    u8 EVs[6] = {gSpecies[species].evYield_HP,
                 gSpecies[species].evYield_Speed,
                 gSpecies[species].evYield_Attack,
                 gSpecies[species].evYield_SpAttack,
                 gSpecies[species].evYield_Defense,
                 gSpecies[species].evYield_SpDefense};
    ...
    if (gTasks[taskId].data[5] == 0) {
        ...
        // Egg cycles
        if (gSpecies[species].eggGroup1 == EGG_GROUP_UNDISCOVERED ||
            gSpecies[species].eggGroup2 == EGG_GROUP_UNDISCOVERED)  // Species without eggs (legendaries etc)
        {
...
static u8 ShowSplitIcon(u32 split) {
    if (sPokedexView->splitIconSpriteId == 0xFF) sPokedexView->splitIconSpriteId = CreateSprite(&sSpriteTemplate_SplitIcons, 139, 90, 0);
...
static void DestroySplitIcon(void) {
    if (sPokedexView->splitIconSpriteId != 0xFF) DestroySprite(&gSprites[sPokedexView->splitIconSpriteId]);
        ...
        case 3:
            if (gTasks[taskId].data[1] == 0) {
                ...
                gTasks[taskId].data[4] =
                    CreateMonIcon(NationalPokedexNumToSpecies(sPokedexListItem->dexNum), SpriteCB_MonIcon, 18, 31, 4, 0, TRUE);  // Create pokemon sprite
#endif
#ifdef POKEMON_EXPANSION
                gTasks[taskId].data[4] =
                    CreateMonIcon(NationalPokedexNumToSpecies(sPokedexListItem->dexNum), SpriteCB_MonIcon, 18, 31, 4, 0);  // Create pokemon sprite
...
#ifndef POKEMON_EXPANSION
        if (gSpecies[species].evos[i].method != 0) times += 1;
#endif
#ifdef POKEMON_EXPANSION
        if (gSpecies[species].evos[i].method != 0 && gSpecies[species].evos[i].method != EVO_MEGA_EVOLUTION) times += 1;
#endif
    }
    ...
    for (i = 0; i < times; i++) {
        ...
        switch (gSpecies[species].evos[i].method) {
            ...
            case EVO_ITEM:
                ...
                item = gSpecies[species].evos[i].param;
            ...
            case EVO_LEVEL_SHEDINJA:
                ...
                break;
            case EVO_BEAUTY:
                ConvertIntToDecimalStringN(gStringVar2, gSpecies[species].evos[i].param, STR_CONV_MODE_LEADING_ZEROS, 3);  // beauty
                targetSpecies = gSpecies[species].evos[i].targetSpecies;
                CreateCaughtBallEvolutionScreen(targetSpecies, base_x + depth_x * depth - 9, base_y + base_y_offset * base_i, 0);
                handleTargetSpeciesPrint(taskId, targetSpecies, base_x + depth_x * depth, base_y, base_y_offset, base_i);  // evolution mon name
...
static bool32 IsMonInfoBeingLoaded(void) { return (gSprites[sPokedexView->selectedMonSpriteId].callback == SpriteCB_MoveMonForInfoScreen); }
static void SpriteCB_StatBars(struct Sprite *sprite) {
    if (IsMonInfoBeingLoaded()) sprite->invisible = TRUE;
    if (sPokedexView->currentPage != PAGE_MAIN && sPokedexView->currentPage != PAGE_SEARCH_RESULTS) {
        FreeSpriteTilesByTag(TAG_STAT_BAR);
        FreeSpriteOamMatrix(&gSprites[sPokedexView->statBarsSpriteId]);
        DestroySprite(&gSprites[sPokedexView->statBarsSpriteId]);
...
static void PrintForms(u8 taskId, u16 species) {
    ...
    if (species == SPECIES_UNOWN) y_offset_icons = 8;
    ...
    for (i = 0; i < 11; i++) {
        ...
        if (speciesForm == FORM_SPECIES_END)
            ...
            if (times < 7)
                ...
                gTasks[taskId].data[4 + times] =
                    CreateMonIcon(speciesForm, SpriteCB_MonIcon, 18 + 34 * (times - 7), 70 - y_offset_icons, 4, 0);  // Create pokemon sprite
            // else if (times < 21)
            //     gTasks[taskId].data[4+times] = CreateMonIcon(speciesForm, SpriteCB_MonIcon, 18 + 34*(times-14), 104 - y_offset_icons, 4, 0); //Create pokemon
            //     sprite
            // else
            //     gTasks[taskId].data[4+times] = CreateMonIcon(speciesForm, SpriteCB_MonIcon, 18 + 34*(times-21), 138 - y_offset_icons, 4, 0); //Create pokemon
            //     sprite
