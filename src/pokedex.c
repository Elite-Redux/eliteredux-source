...
static u8 PrintCryScreenSpeciesName(u8 windowId, u16 num, u8 left, u8 top) {
    ...
    switch (num) {
        default:
            for (i = 0; gSpeciesNames[num][i] != EOS && i < POKEMON_NAME_LENGTH; i++) str[i] = gSpeciesNames[num][i];
            break;
        case 0:
...
static void PrintStatsScreen_MoveNameAndInfo(u8 taskId) {
    ...
    if (selected < numEggMoves) {
        ...
        while (((gLevelUpLearnsets[species][(selected - numEggMoves)] & LEVEL_UP_MOVE_LV) != (level << 9)) && level < 0xFF) {
            level++;
            if (gLevelUpLearnsets[species][(selected - numEggMoves)] == LEVEL_UP_END) level = 0xFF;
        }
#endif
...
static void PrintStatsScreen_Left(u8 taskId) {
    ...
    u16 ability0;
    u8 differentEVs = 0;
    u8 EVs[6] = {gBaseStats[species].evYield_HP,
                 gBaseStats[species].evYield_Speed,
                 gBaseStats[species].evYield_Attack,
                 gBaseStats[species].evYield_SpAttack,
                 gBaseStats[species].evYield_Defense,
                 gBaseStats[species].evYield_SpDefense};
    ...
    if (gTasks[taskId].data[5] == 0) {
        ...
        if (gBaseStats[species].eggGroup1 == EGG_GROUP_UNDISCOVERED ||
            gBaseStats[species].eggGroup2 == EGG_GROUP_UNDISCOVERED)  // Species without eggs (legendaries etc)
        {
