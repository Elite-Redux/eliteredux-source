...
bool8 PokemonUseItemEffects(struct Pokemon *mon, u16 item, u8 partyIndex, u8 moveIndex, bool8 usedByAI)
{
    ...
    for (i = 0; i < ITEM_EFFECT_ARG_START; i++)
    {
        ...
        case 3:
            ...
            if ((itemEffect[i] & ITEM3_BLEED) && HealStatusConditions(mon, partyIndex, STATUS1_BLEED, battlerId) == 0)
                retVal = FALSE;
            break;
    ...
    case EVO_MODE_NORMAL:
        ...
        for (i = 0; i < EVOS_PER_MON; i++)
        {
            ...
            case EVO_FRIENDSHIP_DAY:
                RtcCalcLocalTime();
                if (IsCurrentlyDay() < NIGHT_START && friendship >= 160)
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                break;
            case EVO_LEVEL_DAY:
                RtcCalcLocalTime();
            ...
            case EVO_FRIENDSHIP_NIGHT:
                RtcCalcLocalTime();
                if (!IsCurrentlyDay() && friendship >= 160)
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                break;
...
u32 getMask(s8 loc, s8 locG) {
    ...
    if (	(MAP_NUM(LITTLEROOT_TOWN) == loc && MAP_GROUP(LITTLEROOT_TOWN) == locG) ||
        ...
        (MAP_NUM(VERDANTURF_TOWN) == loc && MAP_GROUP(VERDANTURF_TOWN) == locG) ||
    ...
    if (	(MAP_NUM(ROUTE107) == loc && MAP_GROUP(ROUTE107) == locG) ||
        ...
        (MAP_NUM(ROUTE126) == loc && MAP_GROUP(ROUTE126) == locG) ||
        (MAP_NUM(ROUTE127) == loc && MAP_GROUP(ROUTE127) == locG) ||
        (MAP_NUM(ROUTE128) == loc && MAP_GROUP(ROUTE128) == locG) ||
        (MAP_NUM(ROUTE129) == loc && MAP_GROUP(ROUTE129) == locG) ||
        (MAP_NUM(ROUTE130) == loc && MAP_GROUP(ROUTE130) == locG) ||
    ...
    if (	(MAP_NUM(EMBER_PATH) == loc && MAP_GROUP(EMBER_PATH) == locG) ||
        ...
        (MAP_NUM(ARTISAN_CAVE_B1F) == loc && MAP_GROUP(ARTISAN_CAVE_B1F) == locG) ||
...
u16 getBaseSpeciesFromMega(u16 species) {
    ...
    case SPECIES_FIDOUGH_PARTNER_MEGA: return SPECIES_FIDOUGH_PARTNER;
    ...
    case SPECIES_DURALUDON_PARTNER_MEGA: return SPECIES_DURALUDON_PARTNER;
...
u16 GetFormShiftSpecies(u16 species)
{
    ...
    {
        if (gFormChangeTable[baseSpecies][i].targetSpecies == species)
            return baseSpecies;
    }
...
#define RETURN_SPECIES_NAME_LONG(species) case species: return gSpeciesNameLong ## species;
