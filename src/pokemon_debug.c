...
static void BattleLoadOpponentMonSpriteGfxCustom(u16 species, bool8 isFemale, u8 isShiny, u8 battlerId)
{
    ...
    {
        if (SpeciesHasGenderDifference[species] && isFemale)
            lzPaletteData = gSpecies[species].shinyPaletteFemale.data;
        else
...
static void SetConstSpriteValues(struct PokemonDebugMenu *data)
{
    ...
    data->constSpriteValues.frontPicCoords = gSpecies[species].frontCoords.y_offset;
    data->constSpriteValues.frontElevation = gEnemyMonElevation[species];
    data->constSpriteValues.backPicCoords  = gSpecies[species].backCoords.y_offset;
}
...
static u8 GetBattlerSpriteFinal_YCustom(u16 species, s8 offset_picCoords, s8 offset_elevation)
{
    ...
    //FrontPicCoords
    offset = gSpecies[species].frontCoords.y_offset + offset_picCoords;
...
void CB2_Debug_Pokemon(void)
{
    ...
    switch (gMain.state)
    {
        ...
        case 3:
            ...
            //Front
            HandleLoadSpecialPokePic(&gSpecies[species].frontPic, gMonSpritesGfxPtr->sprites.ptr[1], species, (data->isFemale ? FEMALE_PERSONALITY : MALE_PERSONALITY));
            data->isShiny = FALSE;
            data->isFemale = FALSE;
            ...
            offset_y = gSpecies[species].backCoords.y_offset;
...
static void ApplyOffsetSpriteValues(struct PokemonDebugMenu *data)
{
    u16 species = data->currentmonId;
    //Back
    gSprites[data->backspriteId].y = DEBUG_MON_BACK_Y + gSpecies[species].backCoords.y_offset + data->offsetsSpriteValues.offset_back_picCoords;
    //Front
...
static void UpdateSubmenuTwoOptionValue(u8 taskId, bool8 increment)
{
    ...
    case 0: //Back picCoords
        ...
        gSprites[data->backspriteId].y = DEBUG_MON_BACK_Y + gSpecies[species].backCoords.y_offset + offset;
        break;
...
static void Handle_Input_Debug_Pokemon(u8 taskId)
{
    ...
    else if (data->currentSubmenu == 1) //Submenu 1
    {
        ...
        else if (JOY_NEW(DPAD_DOWN))
        {
            ...
            if (data->submenuYpos[1] >= 3)
            {
                if ((hasForms) || (data->submenuYpos[1] >= 4))
