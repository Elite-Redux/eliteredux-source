...
bool8 ScrCmd_goto_if(struct ScriptContext *ctx) {
    ...
    if (sScriptConditionTable[condition][ctx->comparisonResult] == 1) ScriptJump(ctx, ptr);
    return FALSE;
}
...
bool8 ScrCmd_call_if(struct ScriptContext *ctx) {
    ...
    if (sScriptConditionTable[condition][ctx->comparisonResult] == 1) ScriptCall(ctx, ptr);
    return FALSE;
}
...
bool8 ScrCmd_gotostd(struct ScriptContext *ctx) {
    ...
    if (ptr < gStdScripts_End) ScriptJump(ctx, *ptr);
    return FALSE;
}
...
bool8 ScrCmd_callstd(struct ScriptContext *ctx) {
    ...
    if (ptr < gStdScripts_End) ScriptCall(ctx, *ptr);
    return FALSE;
}
...
bool8 ScrCmd_playbgm(struct ScriptContext *ctx) {
    ...
    if (val == TRUE) Overworld_SetSavedMusic(songId);
...
static bool8 WaitForMovementFinish(void) { return ScriptMovement_IsObjectMovementFinished(sMovingNpcId, sMovingNpcMapId, sMovingNpcMapBank); }
...
bool8 ScrCmd_release(struct ScriptContext *ctx) {
    ...
    HideFieldMessageBox();
    if (gObjectEvents[gSelectedObjectEvent].active) ObjectEventClearHeldMovementIfFinished(&gObjectEvents[gSelectedObjectEvent]);
    playerObjectId = GetObjectEventIdByLocalIdAndMap(OBJ_EVENT_ID_PLAYER, 0, 0);
    ObjectEventClearHeldMovementIfFinished(&gObjectEvents[playerObjectId]);
    ScriptMovement_UnfreezeObjectEvents();
    UnfreezeObjectEvents();
...
bool8 ScrCmd_showcontestpainting(struct ScriptContext *ctx) {
    ...
    if (contestWinnerId != CONTEST_WINNER_ARTIST) SetContestWinnerForPainting(contestWinnerId);
...
bool8 ScrCmd_braillemessage(struct ScriptContext *ctx) {
    ...
    for (i = 0, height = 4; gStringVar4[i] != EOS;) {
        if (gStringVar4[i++] == CHAR_NEWLINE) height += 3;
    }
...
bool8 ScrCmd_addmoney(struct ScriptContext *ctx) {
    ...
    if (!ignore) AddMoney(&gSaveBlock1Ptr->money, amount);
    return FALSE;
}
...
bool8 ScrCmd_removemoney(struct ScriptContext *ctx) {
    ...
    switch (VarGet(VAR_SHOP_MONEY_TYPE)) {
        case MART_MONEY_TYPE_NORMAL:
            if (!ignore) RemoveMoney(&gSaveBlock1Ptr->money, amount);
            break;
        case MART_MONEY_TYPE_BATTLE_POINTS:
            if (!ignore) gSaveBlock2Ptr->frontier.battlePoints = gSaveBlock2Ptr->frontier.battlePoints - amount;
            break;
        case MART_MONEY_TYPE_CASINO_COINS:
            if (!ignore) gSaveBlock1Ptr->coins = gSaveBlock1Ptr->coins - amount;
            break;
    }
    return FALSE;
}
...
bool8 ScrCmd_checkmoney(struct ScriptContext *ctx) {
    ...
    switch (VarGet(VAR_SHOP_MONEY_TYPE)) {
        case MART_MONEY_TYPE_NORMAL:
            if (!ignore) gSpecialVar_Result = IsEnoughMoney(&gSaveBlock1Ptr->money, amount);
            break;
        case MART_MONEY_TYPE_BATTLE_POINTS:
            if (!ignore) gSpecialVar_Result = gSaveBlock2Ptr->frontier.battlePoints >= amount;
            break;
        case MART_MONEY_TYPE_CASINO_COINS:
            if (!ignore) gSpecialVar_Result = gSaveBlock1Ptr->coins >= amount;
            break;
    }
...
bool8 ScrCmd_showmoneybox(struct ScriptContext *ctx) {
    ...
    switch (VarGet(VAR_SHOP_MONEY_TYPE)) {
        case MART_MONEY_TYPE_NORMAL:
            ...
            break;
        case MART_MONEY_TYPE_BATTLE_POINTS:
            if (!ignore) DrawMoneyBox(gSaveBlock2Ptr->frontier.battlePoints, x, y);
            break;
        case MART_MONEY_TYPE_CASINO_COINS:
            if (!ignore) DrawMoneyBox(gSaveBlock1Ptr->coins, x, y);
            break;
    }
...
bool8 ScrCmd_updatemoneybox(struct ScriptContext *ctx) {
    ...
    switch (VarGet(VAR_SHOP_MONEY_TYPE)) {
        case MART_MONEY_TYPE_NORMAL:
            if (!ignore) ChangeAmountInMoneyBox(GetMoney(&gSaveBlock1Ptr->money));
            break;
        case MART_MONEY_TYPE_BATTLE_POINTS:
            if (!ignore) ChangeAmountInMoneyBox(gSaveBlock2Ptr->frontier.battlePoints);
            break;
        case MART_MONEY_TYPE_CASINO_COINS:
            if (!ignore) ChangeAmountInMoneyBox(gSaveBlock1Ptr->coins);
            break;
    }
    return FALSE;
}
...
bool8 ScrCmd_setmonmetlocation(struct ScriptContext *ctx) {
    ...
    if (partyIndex < PARTY_SIZE) SetMonData(&gPlayerParty[partyIndex], MON_DATA_MET_LOCATION, &location);
    return FALSE;
}
...
bool8 ScrCmd_checkPartyHasSpecies(struct ScriptContext *ctx) {
    ...
    for (i = 0; i < partyCount; i++) {
        ...
        if (gSpecies[partySpecies].forms) partySpecies = gSpecies[partySpecies].forms[0];
        if (partySpecies == wantedSpecies) {
            gSpecialVar_Result = TRUE;
        }
    }
    return FALSE;
...
bool8 ScrCmd_isChosenMonSpecies(struct ScriptContext *ctx) {
    ...
    gSpecialVar_Result = FALSE;
    if (gSpecies[chosenSpecies].forms) chosenSpecies = gSpecies[chosenSpecies].forms[0];
    if (chosenSpecies == wantedSpecies) {
        gSpecialVar_Result = TRUE;
    }
    return FALSE;
}
...
bool8 ScrCmd_multichoice2(struct ScriptContext *ctx) {
    ...
    if ((u32)choices < 0x1000000) {  // choices is a multichoiceId
        ...
        if (defaultChoice >= count) defaultChoice = 0;
        if (count > 0) {
            if (columns > 1)
...
bool8 ScrCmd_givecustommon(struct ScriptContext *ctx) {
    ...
    if (level == 0) level = GetHighestLevelInPlayerParty();
