...
static void DrawMultichoiceMenu(u8 left, u8 top, u8 multichoiceId, u8 ignoreBPress, u8 cursorPos) {
    DrawMultichoiceMenuCustom2(
        left, top, multichoiceId, ignoreBPress, cursorPos, sMultichoiceLists[multichoiceId].list, sMultichoiceLists[multichoiceId].count);
}
...
void TryDrawRepelMenu(void) {
    ...
    if (count > 1) DrawMultichoiceMenuCustom2(0, 0, 0, FALSE, 0, menuItems, count);
...
static void CreatePCMultichoice(void) {
    ...
    numChoices = (FlagGet(FLAG_SYS_GAME_CLEAR) != 0) + (FlagGet(FLAG_SYS_POKEMON_GET) != 0) + 1;
    windowId = CreateWindowFromRect(0, 0, width, numChoices * 2);
    SetStandardWindowBorderStyle(windowId, 0);

    if (FlagGet(FLAG_SYS_POKEMON_GET)) {
        if (FlagGet(FLAG_SYS_PC_LANETTE))
            AddTextPrinterParameterized(windowId, 1, gText_LanettesPC, x, 1, TEXT_SPEED_FF, NULL);
        else
            AddTextPrinterParameterized(windowId, 1, gText_SomeonesPC, x, 1, TEXT_SPEED_FF, NULL);

        y += 16;

        // Include Hall of Fame option if player is champion
        if (FlagGet(FLAG_SYS_GAME_CLEAR)) {
            AddTextPrinterParameterized(windowId, 1, gText_HallOfFame, x, y, TEXT_SPEED_FF, NULL);
            y += 16;
        }
    }

    AddTextPrinterParameterized(windowId, 1, gText_LogOff, x, y, TEXT_SPEED_FF, NULL);
        ...
        for (selectionCount = 0, i = 0; i < SSTIDAL_SELECTION_COUNT; i++) {
            if (sLilycoveSSTidalSelections[i] != 0xFF) {
                AddTextPrinterParameterized(
                    windowId, 1, sLilycoveSSTidalDestinations[sLilycoveSSTidalSelections[i]], 8, selectionCount * 16 + 1, TEXT_SPEED_FF, NULL);
                selectionCount++;
            }
        }
    ...
    if (count == PKMNCENTER_MOVE_TUTOR_SELECTION_COUNT) {
        ...
        for (selectionCount = 0, i = 0; i < PKMNCENTER_MOVE_TUTOR_SELECTION_COUNT; i++) {
            if (sPKMNCenterMoveTutorLists[i] != 0xFF) {
                AddTextPrinterParameterized(
                    windowId, 1, sPKMNCenterTutorListOptions[sPKMNCenterMoveTutorLists[i]], 8, selectionCount * 16 + 1, TEXT_SPEED_FF, NULL);
                selectionCount++;
            }
        }
...
bool8 ScriptMenu_ShowPokemonPic(SpeciesEnum species, u8 x, u8 y) {
    u8 taskId;
    u8 spriteId;
...
bool8 (*ScriptMenu_GetPicboxWaitFunc(void))(void) {
    ...
    if (taskId == TASK_NONE) return NULL;
...
u8 CreateWindowFromRect(u8 x, u8 y, u8 width, u8 height) {
    struct WindowTemplate spriteTemplate = CreateWindowTemplate(0, x + 1, y + 1, width, height, 15, 100);
    u8 windowId = AddWindow(&spriteTemplate);
...
int ConvertPixelWidthToTileWidth(int width) { return (((width + 9) / 8) + 1) > MAX_MULTICHOICE_WIDTH ? MAX_MULTICHOICE_WIDTH : (((width + 9) / 8) + 1); }
...
static const struct ListMenuTemplate sMultichoiceListTemplate = {.header_X = 0,
                                                                 .item_X = 8,
                                                                 .cursor_X = 0,
                                                                 .upText_Y = 1,
                                                                 .cursorPal = 2,
...
void ScriptMenu_ScrollingMultichoice2(void) {
    ...
    for (i = 0; i < sScrollingSets[setId].count; i++) width = DisplayTextAndGetWidth(sScrollingSets[setId].set[i].name, width);
