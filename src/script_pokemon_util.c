...
u8 ScriptGiveEgg(SpeciesEnum species) {
    struct Pokemon mon;
    u8 isEgg;
...
static bool8 CheckPartyMonHasHeldItem(u16 item) {
    ...
    for (i = 0; i < PARTY_SIZE; i++) {
        ...
        if (species != SPECIES_NONE && species != SPECIES_EGG && GetMonData(&gPlayerParty[i], MON_DATA_HELD_ITEM) == item) return TRUE;
    }
    return FALSE;
}
...
bool8 DoesPartyHaveEnigmaBerry(void) {
    ...
    if (hasItem == TRUE) GetBerryNameByBerryType(ItemIdToBerryType(ITEM_ENIGMA_BERRY), gStringVar1);

    return hasItem;
}

void CreateScriptedWildMon(SpeciesEnum species, u8 level, u16 item) {
    u8 heldItem[2];
...
void ScriptSetMonMoveSlot(u8 monIndex, u16 move, u8 slot) {
    if (monIndex > PARTY_SIZE) monIndex = gPlayerPartyCount - 1;
...
void ReducePlayerPartyToSelectedMons(void) {
    ...
    for (i = 0; i < MAX_FRONTIER_PARTY_SIZE; i++) gPlayerParty[i] = party[i];
...
u8 ScriptGiveCustomMon(SpeciesEnum species, u8 level, u16 item, u8 ball, u8 nature, u8 abilityNum, u8 *evs, u8 *ivs, u16 *moves, u8 isShiny) {
    ...
    for (i = 0; i < NUM_STATS; i++) {
        ...
        if (evs[i] != 0xFF && evTotal < 510) {
            // only up to 510 evs
            if ((evTotal + evs[i]) > 510) evs[i] = (510 - evTotal);
        ...
        // iv
        if (ivs[i] != 32 && ivs[i] != 0xFF) SetMonData(&mon, MON_DATA_HP_IV + i, &ivs[i]);
    }
    ...
    if (!ball || ball > POKEBALL_COUNT) ball = GetDefaultPokeball();
