...
static const struct MenuAction sShopMenuActions_BuySellQuit[] = {{gText_ShopBuy, {.void_u8 = Task_HandleShopMenuBuy}},
                                                                 {gText_ShopSell, {.void_u8 = Task_HandleShopMenuSell}},
                                                                 {gText_ShopQuit, {.void_u8 = Task_HandleShopMenuQuit}}};

static const struct MenuAction sShopMenuActions_BuyQuit[] = {{gText_ShopBuy, {.void_u8 = Task_HandleShopMenuBuy}},
                                                             {gText_ShopQuit, {.void_u8 = Task_HandleShopMenuQuit}}};
...
#define ADOPTION_CENTER_ZERO_BADGES                                                                                                        \
    SPECIES_PANSAGE_REDUX, SPECIES_PANSEAR_REDUX, SPECIES_PANPOUR_REDUX, SPECIES_RATTATA_REDUX, SPECIES_SHINX_REDUX, SPECIES_NOIBAT_REDUX, \
        SPECIES_SPEAROW_REDUX, SPECIES_BELLSPROUT_REDUX, SPECIES_KNAIVER_REDUX, SPECIES_RALTS_REDUX
...
#define ADOPTION_CENTER_FOUR_BADGES ADOPTION_CENTER_THREE_BADGES, SPECIES_BUIZEL_REDUX, SPECIES_TOXEL_REDUX, SPECIES_HOUNDOUR_REDUX
...
static void Task_HandleShopMenuQuit(u8 taskId) {
    ...
    if (sMartInfo.callback) sMartInfo.callback();
}
...
static void CB2_InitBuyMenu(void) {
    ...
    switch (gMain.state) {
        case 0:
            ...
            break;
        case 1:
            if (!FreeTempTileDataBuffersIfPossible()) gMain.state++;
            break;
        default:
            BuyMenuDrawGraphics();
...
static void BuyMenuBuildListMenuTemplate(void) {
    ...
    for (i = 0; i < sMartInfo.itemCount; i++) BuyMenuSetListEntry(&sListMenuItems[i], sMartInfo.itemList[i], sItemNames[i]);
        ...
        switch (sMartInfo.martType) {
            case MART_TYPE_NORMAL:
                if (CheckBagHasItem(itemId, 1)) {
                    StringCopy(gStringVar1, gText_ShopPurchasedItemPrice);
                    StringExpandPlaceholders(gStringVar4, gText_StrVar1);
                } else {
                    if (VarGet(VAR_SHOP_MONEY_TYPE) == MART_MONEY_TYPE_BATTLE_POINTS) {
                        u16 price = ItemId_GetBPPrice(itemId);
                        if (price == 0 && (ItemId_GetPocket(itemId) == POCKET_MEGA_STONES))
                            price = DEFAULT_MEGA_STONE_PRICE;
                        else if (price == 0)
            ...
            case MART_TYPE_DECOR2:
                if (VarGet(VAR_SHOP_MONEY_TYPE) == MART_MONEY_TYPE_BATTLE_POINTS) {
                    ...
                    if (price == 0) price = DEFAULT_DECORATION_PRICE;
            ...
            case MART_TYPE_MONS: {
                ...
                if (price == 0) price = DEFAULT_POKEMON_PRICE;
                ConvertIntToDecimalStringN(gStringVar1, price, STR_CONV_MODE_LEFT_ALIGN, 5);
                if (VarGet(VAR_SHOP_MONEY_TYPE) == MART_MONEY_TYPE_NORMAL)
                    StringExpandPlaceholders(gStringVar4, gText_PokedollarVar1);
                else
...
static void BuyMenuAddScrollIndicatorArrows(void) {
    if (sShopData->scrollIndicatorsTaskId == TASK_NONE && sMartInfo.itemCount + 1 > 8) {
        sShopData->scrollIndicatorsTaskId = AddScrollIndicatorArrowPairParameterized(
            SCROLL_ARROW_UP, 172, 12, 148, sMartInfo.itemCount - 7, TAG_SCROLL_ARROW, TAG_SCROLL_ARROW, &sShopData->scrollOffset);
    }
}
...
static void BuyMenuAddItemIcon(u16 item, u8 iconSlot) {
    ...
    switch (sMartInfo.martType) {
        ...
        case MART_TYPE_DECOR2:
            ...
            if (spriteId != MAX_SPRITES) *spriteIdPtr = spriteId;
            break;
        case MART_TYPE_MONS:
...
static void BuyMenuCollectObjectEventData(void) {
    ...
    for (y = 0; y < OBJECT_EVENTS_COUNT; y++) sShopData->viewportObjects[y][OBJ_EVENT_ID] = OBJECT_EVENTS_COUNT;
    for (y = 0; y < 5; y++) {
        for (x = 0; x < 7; x++) {
...
static void BuyMenuDrawObjectEvents(void) {
    ...
    for (i = 0; i < OBJECT_EVENTS_COUNT; i++) {
        ...
        spriteId = AddPseudoObjectEvent(gObjectEvents[sShopData->viewportObjects[i][OBJ_EVENT_ID]].graphicsId,
                                        SpriteCallbackDummy,
...
static bool8 BuyMenuCheckForOverlapWithMenuBg(int x, int y) {
    ...
    if (metatile[offset2 + offset1] == 0 && metatile[offset2 + offset1 + 32] == 0 && metatile[offset2 + offset1 + 1] == 0 &&
                ...
                if (VarGet(VAR_SHOP_MONEY_TYPE) == MART_MONEY_TYPE_NORMAL) {
                    ...
                    if (!IsEnoughMoney(&money, sShopData->totalCost)) {
                        if (CheckBagHasItem(itemId, 1) == TRUE) {
                            BuyMenuDisplayMessage(taskId, gText_ShopAlreadyHaveItem, BuyMenuReturnToItemList);
                        } else {
                        ...
                        // There are no normal shops that sell mons, so this is ToDo
                    }
...
static void BuyMenuTryMakePurchase(u8 taskId) {
    ...
    switch (sMartInfo.martType) {
        ...
        case MART_TYPE_MONS: {
            ...
            if (Random() < ALPHA_ODDS && GetNumberOfBadges() >= 6) FlagSet(FLAG_ALPHA_CREATION);
