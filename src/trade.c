...
static void CB2_CreateTradeMenu(void) {
    ...
    switch (gMain.state) {
        case 0:
            ...
            for (i = 0; i < GFXTAG_MENU_TEXT_COUNT; i++) sMenuTextTileBuffers[i] = &sMenuTextAllocBuffer[i * 256];
        ...
        case 1:
            ...
            for (i = 0; i < PARTY_SIZE; i++) CreateMon(&gEnemyParty[i], SPECIES_NONE, 0, USE_RANDOM_IVS, FALSE, 0, OT_ID_PLAYER_ID, 0);
        ...
        case 10:
            ...
            break;
        case 11:
            if (LoadTradeMenuSpriteSheetsAndPalettes()) gMain.state++;
            break;
        case 12:
...
static void DrawIsThisTradeOkay(void) { DrawBottomRowText(sText_IsThisTradeOkay, (void *)(OBJ_VRAM0 + (sTradeMenuData->bottomTextTileStart * 32)), 24); }
...
static void ReactToLinkTradeData(u8 mpId, u8 status) {
    ...
    if (status & 2) TradeResetReceivedFlag(1);
}

static void QueueLinkTradeData(void) {
    if (sTradeMenuData->playerLinkFlagChoseAction && sTradeMenuData->partnerLinkFlagChoseAction) {
        if (sTradeMenuData->playerLinkFlagChoseAction == WANTS_TO_TRADE && sTradeMenuData->partnerLinkFlagChoseAction == WANTS_TO_TRADE) {
        ...
        } else if (sTradeMenuData->playerLinkFlagChoseAction == WANTS_TO_TRADE && sTradeMenuData->partnerLinkFlagChoseAction == WANTS_TO_CANCEL) {
            PrintTradeMessage(TRADE_MSG_CANCELED);
            ...
            sTradeMenuData->playerLinkFlagChoseAction = sTradeMenuData->partnerLinkFlagChoseAction = 0;
            sTradeMenuData->tradeMenuFunc = TRADEMENUFUNC_REDRAW_MAIN_MENU;
        } else if (sTradeMenuData->playerLinkFlagChoseAction == WANTS_TO_CANCEL && sTradeMenuData->partnerLinkFlagChoseAction == WANTS_TO_TRADE) {
            PrintTradeMessage(TRADE_MSG_FRIEND_WANTS_TO_TRADE);
            ...
            sTradeMenuData->playerLinkFlagChoseAction = sTradeMenuData->partnerLinkFlagChoseAction = 0;
            sTradeMenuData->tradeMenuFunc = TRADEMENUFUNC_REDRAW_MAIN_MENU;
        } else if (sTradeMenuData->playerLinkFlagChoseAction == WANTS_TO_CANCEL && sTradeMenuData->partnerLinkFlagChoseAction == WANTS_TO_CANCEL) {
    ...
    if (sTradeMenuData->playerLinkFlagStatus && sTradeMenuData->partnerLinkFlagStatus) {
        if (sTradeMenuData->playerLinkFlagStatus == INITIATE_TRADE && sTradeMenuData->partnerLinkFlagStatus == INITIATE_TRADE) {
        ...
        if (sTradeMenuData->playerLinkFlagStatus == CANCEL_TRADE || sTradeMenuData->partnerLinkFlagStatus == CANCEL_TRADE) {
            PrintTradeMessage(TRADE_MSG_CANCELED);
            sTradeMenuData->linkData[0] = LINKCMD_PLAYER_CANCEL_TRADE;
            sTradeMenuData->linkData[1] = 0;
            QueueAction(QUEUE_DELAY_DATA, QUEUE_SEND_DATA);
...
static void SetLinkData(u16 linkCmd, u16 cursorPosition) { _SetLinkData(sTradeMenuData->linkData, linkCmd, cursorPosition); }
    ...
    if (!gPaletteFade.active) {
        ...
        if (sTradeMenuData->cursorPosition < PARTY_SIZE)
            ShowPokemonSummaryScreen(
                SUMMARY_MODE_LOCK_MOVES, gPlayerParty, sTradeMenuData->cursorPosition, sTradeMenuData->partyCounts[TRADE_PLAYER] - 1, CB2_ReturnToTradeMenu);
        // Partner's party
        else
...
static u8 CheckValidityOfTradeMons(u8 *aliveMons, u8 playerPartyCount, u8 playerMonIdx, u8 partnerMonIdx) {
    ...
    for (i = 0; i < playerPartyCount; i++) {
        if (playerMonIdx != i) hasLiveMon += aliveMons[i];
    }
    ...
    if (partnerSpecies == SPECIES_DEOXYS || partnerSpecies == SPECIES_MEW) {
        if (!GetMonData(&gEnemyParty[partnerMonIdx], MON_DATA_EVENT_LEGAL)) return PARTNER_MON_INVALID;
    }

    // Partner cant trade Egg or non-Hoenn mon if player doesn't have National Dex
    if (!IsNationalPokedexEnabled()) {
        if (sTradeMenuData->isEgg[TRADE_PARTNER][partnerMonIdx] || !IsSpeciesInHoennDex(partnerSpecies)) return PARTNER_MON_INVALID;
    }

    if (hasLiveMon) hasLiveMon = BOTH_MONS_VALID;
...
static bool32 CheckMonsBeforeTrade(void) {
    ...
    switch (
        CheckValidityOfTradeMons(aliveMons, sTradeMenuData->partyCounts[TRADE_PLAYER], sTradeMenuData->cursorPosition, sTradeMenuData->partnerCursorPosition)) {
...
static void ConfirmOrCancelTrade(void) {
    switch (Menu_ProcessInputNoWrapClearOnChoose()) {
        ...
        case MENU_B_PRESSED:
            QueueAction(QUEUE_DELAY_MSG, QUEUE_STANDBY);
            if (IsLinkTradeTaskFinished()) SetLinkData(LINKCMD_READY_CANCEL_TRADE, 0);
            sTradeMenuData->tradeMenuFunc = TRADEMENUFUNC_STANDBY;
            PutWindowTilemap(17);
            break;
    }
...
static void ConfirmTradePrompt(void) {
    if (sTradeMenuData->drawPartyState[TRADE_PLAYER] == DRAW_PARTY_FINISH && sTradeMenuData->drawPartyState[TRADE_PARTNER] == DRAW_PARTY_FINISH) {
        DrawIsThisTradeOkay();
...
static void DrawTradeMenuParty(u8 whichParty) {
    ...
    if (sTradeMenuData->selectedMonIdx[whichParty] < PARTY_SIZE) selectedMonParty = TRADE_PLAYER;
    ...
    switch (sTradeMenuData->drawPartyState[whichParty]) {
        case 1:
            ...
            gSprites[sTradeMenuData->partySpriteIds[0][partyIdx + (selectedMonParty * PARTY_SIZE)]].data[2] =
                (sTradeMonSpriteCoords[selectedMonParty * PARTY_SIZE][0] + sTradeMonSpriteCoords[selectedMonParty * PARTY_SIZE + 1][0]) / 2 * 8 + 14;
            ...
            if (selectedMonParty == TRADE_PLAYER) PrintNicknamesForTradeMenu();
            break;
        case 2:
...
static void DrawTradeMenuPartyMonInfo(u8 whichParty, u8 monIdx, u8 x, u8 y, u8 width, u8 height) {
    ...
    if (!sTradeMenuData->isEgg[whichParty][monIdx]) {
        if (level / 10 != 0) sTradeMenuData->tilemapBuffer[x + (y * 32)] = (level / 10) + 0x60;
...
static void DrawBottomRowText(const u8 *str, u8 *dest, u8 unused) { DrawTextWindowAndBufferTiles(str, dest, 0, 0, 6); }
...
static void SaveTradeGiftRibbons(void) {
    ...
    for (i = 0; i < (int)ARRAY_COUNT(sTradeMenuData->giftRibbons); i++) {
        if (gSaveBlock1Ptr->giftRibbons[i] == 0 && sTradeMenuData->giftRibbons[i] != 0) {
            if (sTradeMenuData->giftRibbons[i] < 64) gSaveBlock1Ptr->giftRibbons[i] = sTradeMenuData->giftRibbons[i];
        }
    }
}
...
static u32 CanTradeSelectedMon(struct Pokemon *playerParty, int partyCount, int monIdx) {
    ...
    if (!IsNationalPokedexEnabled()) {
        if (species2[monIdx] == SPECIES_EGG) return CANT_TRADE_EGG_YET;

        if (!IsSpeciesInHoennDex(species2[monIdx])) return CANT_TRADE_NATIONAL;
    }
    ...
    if ((player->version & 0xFF) != VERSION_RUBY && (player->version & 0xFF) != VERSION_SAPPHIRE) {
        ...
        if (!(player->progressFlagsCopy & 0xF)) {
            if (species2[monIdx] == SPECIES_EGG) return CANT_TRADE_EGG_YET2;

            if (!IsSpeciesInHoennDex(species2[monIdx])) return CANT_TRADE_INVALID_MON;
        }
    }

    if (species[monIdx] == SPECIES_DEOXYS || species[monIdx] == SPECIES_MEW) {
        if (!GetMonData(&playerParty[monIdx], MON_DATA_EVENT_LEGAL)) return CANT_TRADE_INVALID_MON;
    }
    ...
    for (numMonsLeft = 0, i = 0; i < partyCount; i++) {
        if (i != monIdx) numMonsLeft += species2[i];
    }
...
int CanRegisterMonForTradingBoard(struct GFtgtGnameSub rfuPlayer, SpeciesEnum species2, SpeciesEnum species, u8 isEventLegal) {
    ...
    if (IsDeoxysOrMewUntradable(species, isEventLegal)) return CANT_REGISTER_MON;

    if (hasNationalDex) return CAN_REGISTER_MON;

    // Eggs can only be traded if the player has the National Dex
    if (species2 == SPECIES_EGG) return CANT_REGISTER_EGG;

    if (IsSpeciesInHoennDex(species2)) return CAN_REGISTER_MON;
...
int CanSpinTradeMon(struct Pokemon *mon, u16 monIdx) {
    ...
    for (i = 0; i < GetLinkPlayerCount(); i++) {
        ...
        do {
            if (!(player->progressFlags & 0xF)) canTradeAnyMon = FALSE;

            if (versions && (player->progressFlags / 16)) canTradeAnyMon = FALSE;
        } while (0);
    }

    if (canTradeAnyMon == FALSE) {
        if (!IsSpeciesInHoennDex(speciesArray[monIdx])) return CANT_TRADE_NATIONAL;

        if (speciesArray[monIdx] == SPECIES_NONE) return CANT_TRADE_EGG_YET;
    }
...
static void SetTradeBGAffine(void) {
    ...
    DoBgAffineSet(
        &affine, sTradeData->texX * 0x100, sTradeData->texY * 0x100, sTradeData->scrX, sTradeData->scrY, sTradeData->sXY, sTradeData->sXY, sTradeData->alpha);
...
static u32 TradeGetMultiplayerId(void) {
    if (gReceivedRemoteLinkPlayers) return GetMultiplayerId();
    return 0;
}
...
static void TradeMons(u8 playerPartyIdx, u8 partnerPartyIdx) {
    ...
    *partnerMon = sTradeData->mon;
    ...
    friendship = 70;
    if (!GetMonData(playerMon, MON_DATA_IS_EGG)) SetMonData(playerMon, MON_DATA_FRIENDSHIP, &friendship);

    UpdatePokedexForReceivedMon(playerPartyIdx);
    if (gReceivedRemoteLinkPlayers) TryEnableNationalDexFromLinkPartner();
}
...
static bool8 AnimateTradeSequenceCable(void) {
    ...
    switch (sTradeData->state) {
        ...
        case TS_STATE_SEND_MSG:
            ...
            if (sTradeData->monSpecies[TRADE_PLAYER] != SPECIES_EGG) PlayCry1(sTradeData->monSpecies[TRADE_PLAYER], 0);
        ...
        case TS_STATE_BYE_BYE:
            if (++sTradeData->timer == 80) {
                sTradeData->releasePokeballSpriteId =
                    CreateTradePokeballSprite(sTradeData->monSpriteIds[0], gSprites[sTradeData->monSpriteIds[0]].oam.paletteNum, 120, 32, 2, 1, 0x14, 0xfffff);
                sTradeData->state++;
        ...
        case TS_STATE_WAIT_FADE_IN_TO_GBA_SEND:
            if (!gPaletteFade.active) sTradeData->state = TS_STATE_GBA_ZOOM_OUT;
            break;
        ...
        case TS_STATE_PAN_AWAY_GBA:
            if (--sTradeData->bg1vofs == 316) sTradeData->state++;

            if (sTradeData->bg1vofs == 328) sTradeData->cableEndSpriteId = CreateSprite(&sSpriteTemplate_CableEnd, 128, 65, 0);
            break;
        ...
        case TS_STATE_LINK_MON_TRAVEL_OUT:
            if ((sTradeData->bg1vofs -= 2) == 166) sTradeData->state = TS_STATE_LINK_MON_TRAVEL_OFFSCREEN;
        ...
        case TS_STATE_LINK_MON_TRAVEL_OFFSCREEN:
            ...
            if (gSprites[sTradeData->connectionSpriteId1].y < -8) sTradeData->state = TS_STATE_FADE_OUT_TO_CROSSING;
            break;
        ...
        case TS_STATE_WAIT_FADE_OUT_TO_GBA_RECV:
            SetGpuReg(REG_OFFSET_DISPCNT, DISPCNT_MODE_0 | DISPCNT_OBJ_1D_MAP | DISPCNT_BG1_ON | DISPCNT_OBJ_ON);
            if (!gPaletteFade.active) sTradeData->state++;
            break;
        ...
        case TS_STATE_LINK_MON_ARRIVED_DELAY:
            if (++sTradeData->timer == 10) sTradeData->state++;
            break;
        ...
        case TS_STATE_SHOW_NEW_MON:
            ...
            CreatePokeballSpriteToReleaseMon(sTradeData->monSpriteIds[TRADE_PARTNER],
                                             gSprites[sTradeData->monSpriteIds[TRADE_PARTNER]].oam.paletteNum,
                                             120,
                                             84,
                                             2,
                                             1,
        ...
        case TS_STATE_WAIT_FOR_MON_CRY:
            if (IsCryFinished()) sTradeData->state = TS_STATE_TAKE_CARE_OF_MON;
            break;
        case TS_STATE_TAKE_CARE_OF_MON:
            if (++sTradeData->timer == 10) PlayFanfare(MUS_EVOLVED);
        ...
        case TS_STATE_AFTER_NEW_MON_DELAY:
            if (++sTradeData->timer == 60) sTradeData->state++;
            break;
        ...
        case TS_STATE_TRY_EVOLUTION:  // Only if in-game trade, link trades use CB2_TryLinkTradeEvolution
            ...
            evoTarget = GetEvolutionTargetSpecies(&gPlayerParty[gSelectedTradeMonPositions[TRADE_PLAYER]],
                                                  EVO_MODE_TRADE,
                                                  ITEM_NONE,
                                                  GetMonData(&gPlayerParty[gSelectedTradeMonPositions[TRADE_PARTNER]], MON_DATA_SPECIES));
            if (evoTarget != SPECIES_NONE) {
                TradeEvolutionScene(&gPlayerParty[gSelectedTradeMonPositions[TRADE_PLAYER]],
                                    evoTarget,
                                    sTradeData->monSpriteIds[TRADE_PARTNER],
...
static bool8 AnimateTradeSequenceWireless(void) {
    ...
    switch (sTradeData->state) {
        ...
        case TS_STATE_WAIT_WIRELESS_SIGNAL_SEND:
            if (!FuncIsActiveTask(Task_AnimateWirelessSignal)) sTradeData->state = TS_STATE_PAN_AWAY_GBA;
            break;
        case TS_STATE_PAN_AWAY_GBA:
            if (--sTradeData->bg1vofs == 316) sTradeData->state++;
            break;
        case TS_STATE_CREATE_LINK_MON_LEAVING:
            ...
            break;
        case TS_STATE_LINK_MON_TRAVEL_OUT:
            if ((sTradeData->bg1vofs -= 3) == 166) sTradeData->state = TS_STATE_LINK_MON_TRAVEL_OFFSCREEN;
...
static void CB2_TryLinkTradeEvolution(void) {
    ...
    if (!HasLinkErrorOccurred()) RunTasks();
    AnimateSprites();
    BuildOamBuffer();
    UpdatePaletteFade();
}
...
static void UpdateTradeFinishFlags(void) {
    ...
    if (blockReceivedStatus & 0x01) {
        if (gBlockRecvBuffer[0][0] == LINKCMD_CONFIRM_FINISH_TRADE) SetMainCallback2(CB2_TryLinkTradeEvolution);

        if (gBlockRecvBuffer[0][0] == LINKCMD_READY_FINISH_TRADE) sTradeData->playerLinkFlagFinishTrade = READY_FINISH_TRADE;

        ResetBlockReceivedFlag(0);
    }
    if (blockReceivedStatus & 0x02) {
        if (gBlockRecvBuffer[1][0] == LINKCMD_READY_FINISH_TRADE) sTradeData->partnerLinkFlagFinishTrade = READY_FINISH_TRADE;
...
static void SpriteCB_BouncingPokeball(struct Sprite *sprite) {
    ...
    if (sprite->x == 0x78) sprite->data[1] = 0;
...
static void SpriteCB_BouncingPokeballDepart(struct Sprite *sprite) {
    ...
    if (sprite->data[0] == 22) PlaySE(SE_BALL_BOUNCE_1);
    if (++sprite->data[0] == 44) {
...
static void SpriteCB_BouncingPokeballDepartEnd(struct Sprite *sprite) {
    if (sprite->data[1] == 20) StartSpriteAffineAnim(sprite, 1);
    if (++sprite->data[1] > 20) {
        sprite->y2 -= sTradeBallVerticalVelocityTable[sprite->data[0]];
        if (++sprite->data[0] == 23) {
...
static void SpriteCB_BouncingPokeballArrive(struct Sprite *sprite) {
    if (sprite->data[2] == 0) {
        ...
        if (sprite->data[0] == 0x42) PlaySE(SE_BALL_BOUNCE_2);
        if (sprite->data[0] == 0x5c) PlaySE(SE_BALL_BOUNCE_3);
        if (sprite->data[0] == 0x6b) PlaySE(SE_BALL_BOUNCE_4);
        sprite->y2 += sTradeBallVerticalVelocityTable[sprite->data[0]];
        if (++sprite->data[0] == 0x6c) sprite->callback = SpriteCallbackDummy;
    }
}
...
u16 GetTradeSpecies(void) {
    if (GetMonData(&gPlayerParty[gSpecialVar_0x8005], MON_DATA_IS_EGG)) return SPECIES_NONE;
    return GetMonData(&gPlayerParty[gSpecialVar_0x8005], MON_DATA_SPECIES);
}

void CreateInGameTradePokemon(void) { _CreateInGameTradePokemon(gSpecialVar_0x8005, gSpecialVar_0x8004); }
...
static void CB2_TryFinishTrade(void) {
    ...
    if (IsWirelessTrade()) {
        ...
        UpdateTradeFinishFlags();
        if (mpId == 0 && sTradeData->playerLinkFlagFinishTrade == READY_FINISH_TRADE && sTradeData->partnerLinkFlagFinishTrade == READY_FINISH_TRADE) {
...
static void CB2_SaveAndEndTrade(void) {
    switch (gMain.state) {
        ...
        case 2:
            ...
            break;
        case 50:
            if (!InUnionRoom()) IncrementGameStat(GAME_STAT_POKEMON_TRADES);
            if (gWirelessCommType) {
...
static void CheckPartnersMonForRibbons(void) {
    ...
    if (numRibbons != 0) FlagSet(FLAG_SYS_RIBBON_GET);
}
...
static void Task_AnimateWirelessSignal(u8 taskId) {
    ...
    if (sWirelessSignalTiming[idx][0] == 0 && counter == 0) PlaySE(SE_M_HEAL_BELL);
...
static void Task_NarrowWindowForCrossing_Wireless(u8 taskId) {
    ...
    if (sTradeData->wirelessWinLeft < 80) DestroyTask(taskId);
}
...
static void Task_NarrowWindowForCrossing_Cable(u8 taskId) {
    ...
    if (sTradeData->wirelessWinLeft != DISPLAY_WIDTH / 2) {
        ...
        if (sTradeData->wirelessWinLeft > DISPLAY_WIDTH / 2 - 5) BlendPalettes(0x8, 0, RGB_WHITEALPHA);
    } else {
...
static void CB2_SaveAndEndWirelessTrade(void) {
    switch (gMain.state) {
        ...
        case 2:
            ...
            break;
        case 3:
            if (++sTradeData->timer == 5) gMain.state = 4;
            break;
        case 4:
            if (LinkFullSave_WriteSector()) {
                gMain.state = 5;
