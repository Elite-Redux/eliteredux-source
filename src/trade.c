...
int GetUnionRoomTradeMessageId(struct GFtgtGnameSub rfuPlayer, struct GFtgtGnameSub rfuPartner, u16 playerSpecies2, u16 partnerSpecies, u8 requestedType, u16 playerSpecies, u8 isEventLegal)
{
    ...
    {
        if (gSpecies[playerSpecies2].type1 != requestedType && gSpecies[playerSpecies2].type2 != requestedType)
        {
...
static void LoadTradeMonPic(u8 whichParty, u8 state)
{
    ...
    case 0:
        ...
        HandleLoadSpecialPokePic(&gSpecies[species].frontPic, gMonSpritesGfxPtr->sprites.ptr[whichParty * 2 + 1], species, personality);
...
static bool8 AnimateTradeSequenceCable(void)
{
    ...
    case TS_STATE_POKEBALL_ARRIVE_WAIT:
        ...
        {
            HandleLoadSpecialPokePic(&gSpecies[sTradeData->monSpecies[TRADE_PARTNER]].frontPic, gMonSpritesGfxPtr->sprites.ptr[3], sTradeData->monSpecies[TRADE_PARTNER], sTradeData->monPersonalities[TRADE_PARTNER]);
            sTradeData->state++;
        }
        break;
...
static bool8 AnimateTradeSequenceWireless(void)
{
    ...
    case TS_STATE_POKEBALL_ARRIVE_WAIT:
        if (gSprites[sTradeData->bouncingPokeballSpriteId].callback == SpriteCallbackDummy)
        {
            HandleLoadSpecialPokePic(&gSpecies[sTradeData->monSpecies[TRADE_PARTNER]].frontPic, 
