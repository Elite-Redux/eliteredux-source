...
static void SpriteCallback_Inventory_UpArrow(struct Sprite *sprite)
{
    ...
    //sprite->pos.y2 = gSineTable[val] / 128;
    sprite->data[0] += 8;
...
static u8 ShowSpeciesIcon(u8 slot, u8 x, u8 y)
{
    ...
    u8 palette        = 0; //LoadMonIconPaletteWithAilment(species, personality, currentStatus, currentHP, slot + 2);
...
static void PrintToWindow(u8 windowId, u8 colorIdx)
{
	...
	u8 season    = SEASON_SPRING; //getCurrentSeason();
	u8 hours     = gLocalTime.hours;
	u8 minutes   = gLocalTime.minutes;
	u8 font      = FONT_SMALL_NARROW;
	u8 fontColor = FONT_WHITE_2;
    u16 year     = 0;//RtcGetCurrentYear();
    u8 month     = 0;//RtcGetCurrentMonth();
    u8 day       = 0;//RtcGetCurrentDay();
