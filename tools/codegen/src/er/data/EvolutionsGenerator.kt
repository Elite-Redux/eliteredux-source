...
object EvolutionsGenerator : Generator {
  ...
  override fun generate(writer: OutputStreamWriter) {
    ...
    val allEvoPairs =
      (SPECIES_LIST.flatMap {
          it.evoList.filter { evo -> evo.gender == GENDER_NONE }.map { evo -> it.id to evo.to }
        } +
          SPECIES_LIST.flatMap { it.megaList.map { mega -> mega.from to it.id } } +
          SPECIES_LIST.flatMap { it.primalList.map { primal -> primal.from to it.id } })
        .filter { it.second !in INCONSISTENT_GENDER_MONS }

    val inconsistentGenderEvos =
      allEvoPairs.filter {
        it.first.data.genderless != it.second.data.genderless ||
          it.first.data.percentFemale != it.second.data.percentFemale
      }

    check(inconsistentGenderEvos.isEmpty()) {
      "Gender ratios don't match in evos, is this intentional?: $inconsistentGenderEvos"
    }

    val deevos =
      SPECIES_LIST.filter { it.allowDeevolutionTo }
        .flatMap { species -> species.evoList.map { it.to to species.id } }
        .toMap()
        .mapValues { Evo(EvoOrFormType.EVO_DEEVOLUTION, "TRUE", it.value) }

    // De-evolutions are expected to come first
    val (evoIds, speciesEvoIds) =
      NO_EGG_LIST.map { species ->
          listOfNotNull(deevos[species.id]) +
            species.evoList
              .filter { it.to in SPECIES_MAP }
              .map {
                Evo(
                  when {
    ...
    val megas =
      SPECIES_LIST.flatMap { it.megaList.map { mega -> mega to it.id } }
        .groupBy { it.first.from }
        .mapValues { (_, values) ->
          values.map { (mega, to) ->
            if (mega.hasMove()) {
    ...
    val primals =
      SPECIES_LIST.flatMap { it.primalList.map { primal -> primal to it.id } }
        .groupBy { it.first.from }
        .mapValues { (_, values) ->
          values.map { (primal, to) ->
            Evo(EvoOrFormType.EVO_PRIMAL_REVERSION, primal.item.toString(), to)
          }
        }

    val reverseForms =
      (NO_EGG_LIST.map { it.formShiftOf to it.id }.groupBy({ it.first }, { it.second }) -
          SpeciesEnum.SPECIES_NONE)
        .mapValues { (base, forms) ->
          listOf(Evo(EvoOrFormType.EVO_FORM_SHIFT, "TRUE", base)) +
            forms.map {
              when (it.data.formShiftGender) {
    ...
    val allForms =
      NO_EGG_LIST.map {
        ...
        val unmega =
          listOfNotNull(
              (it.megaList.map { m -> m.from } + it.primalList.map { p -> p.from }).firstOrNull()
            )
            .map { from -> Evo(EvoOrFormType.EVO_UNMEGA, "TRUE", from) }
        // Unmegas are expected to come first
        (unmega +
          reverseForms[formId].orEmpty() +
          megas[it.id].orEmpty() +
          primals[it.id].orEmpty()) to it.id
      }

    val (formIds, speciesFormIds) = allForms.createDedupMaps()

    writer.appendLine(
      formIds.entries.joinToString("\n") {
          """
