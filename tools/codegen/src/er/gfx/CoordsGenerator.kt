    ...
    override fun generate(writer: OutputStreamWriter) {
        writer.appendLine(
            ...
            |$IND${SPECIES_LIST.joinToString("\n$IND") { 
                ...
                "[${it.id}] = {.size = ${it.visuals.front.coords.size}, .y_offset = ${it.visuals.front.coords.yOffset}},"
            }}
            |};
            |
            ...
            |$IND${SPECIES_LIST.joinToString("\n$IND") {
                val mon = if (it.hasReuseVisuals()) SPECIES_MAP[it.reuseVisuals] else it
                "[${it.id}] = {.size = ${it.visuals.back.coords.size}, .y_offset = ${it.visuals.back.coords.yOffset}},"
            }}
            |};
            |""".trimMargin())
    }
