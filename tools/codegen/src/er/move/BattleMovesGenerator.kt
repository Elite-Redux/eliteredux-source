...
import er.Generator
import er.GeneratorUtils.CUSTOM_BEHAVIORS
...
object BattleMovesGenerator : Generator {
  private val PROTECT_AFFECTED_TARGETS =
    setOf(
      MoveTarget.BOTH,
      MoveTarget.DEPENDS,
  ...
  override fun generate(writer: OutputStreamWriter) {
    ...
    check(badHitCounts.isEmpty()) {
      "Unsupported hit counts for moves: ${badHitCounts.map { it.id }}"
    }
    ...
    for (move in MOVES_LIST) {
      writer.appendLine(
        """
                |$IND[${move.id}] = {
                |$IND$IND.effect = ${if (move.hasCustomBehavior()) CUSTOM_BEHAVIORS[move.customBehavior]!! else move.effect},
                |$IND$IND.split = SPLIT_${move.split},"""
          .trimMargin()
      )
      ...
      if (move.effectChance > 0)
        writer.appendLine("$IND$IND.secondaryEffectChance = ${move.effectChance},")
      ...
      val noParentalBond =
        move.noParentalBond || move.effect.getOption(noParentalBond) || move.hitCount > 1
      ...
      val bitFlags =
        listOfNotNull(
            ...
            "FLAG_SHEER_FORCE_BOOST".takeIf { sheerForce },
          )
          .toMutableList()
      val fieldFlags =
        listOfNotNull(
            "parentalBondBanned".takeIf { noParentalBond },
            "twoTurnMove".takeIf { twoTurn },
          )
          .toMutableList()
        ...
        val flag =
          when (val type = descriptor.type) {
            ...
            Descriptors.FieldDescriptor.Type.ENUM ->
              (value as EnumValueDescriptor).getOption(enumFlagCodeValue)
      ...
      if (bitFlags.isNotEmpty())
        writer.appendLine("$IND$IND.flags = ${bitFlags.joinToString(" | ")},")

      fieldFlags.forEach { writer.appendLine("$IND$IND.$it = TRUE,") }

      val argument =
        when (move.argument.argumentCase) {
          ...
          Argument.ArgumentCase.ARGUMENT_NOT_SET,
          null -> ""
        }
