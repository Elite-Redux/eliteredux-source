...
object TrainerPartyGenerator : Generator {
    ...
    fun TrainerMon.validate() {
            if (nonstandard.isEmpty()) {
                ...
                check(invalidMoves.isEmpty()) { "Invalid moves: $invalidMoves\n$this" }

                check(((hpEv / 4) + (atkEv / 4) + (defEv / 4) + (spatkEv / 4) + (spdefEv / 4) + (speEv / 4)) == 508 / 4) {
                    "Invalid EV total\n$this"
                }

                check(item != ItemEnum.ITEM_NONE) { "Missing item\n$this" }

                when (getNegativeNatureStat(nature)) {
                    NEUTRAL -> error("Neutral nature\n$this")
                    ATK -> check(atkEv < 4) { "Positive EV for negative nature\n$this" }
                    DEF -> check(defEv < 4) { "Positive EV for negative nature\n$this" }
                    SPATK -> check(spatkEv < 4) { "Positive EV for negative nature\n$this" }
                    SPDEF -> check(spdefEv < 4) { "Positive EV for negative nature\n$this" }
                    SPE -> check(speEv < 4) { "Positive EV for negative nature\n$this" }
                }

                if (ironPill) {
                    check(getNegativeNatureStat(nature) == SPE) { "Iron pill should use negative speed nature\n$this" }
                }
            }
    }
